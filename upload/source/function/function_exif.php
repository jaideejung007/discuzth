...
function getexif($img) {
	...
	$imgtype = ['', 'GIF', 'JPG', 'PNG', 'SWF', 'PSD', 'BMP', 'WEBP', 'TIFF(intel byte order)', 'TIFF(motorola byte order)', 'JPC', 'JP2', 'JPX', 'JB2', 'SWC', 'IFF', 'WBMP', 'XBM'];
	$Orientation = ['', 'top left side', 'top right side', 'bottom right side', 'bottom left side', 'left side top', 'right side top', 'right side bottom', 'left side bottom'];
	$ResolutionUnit = exif_lang('resolutionunit');
	$YCbCrPositioning = ['', 'the center of pixel array', 'the datum point'];
	...
	if($exif === false) {
		...
		foreach($exif as $type => $typearr) {
			foreach($typearr as $key => $kval) {
				...
				} elseif(!in_array($key, ['ComponentsConfiguration', 'FileSource', 'SceneType'])) {
		...
		$new_img_info = [
			...
			exif_lang('FlashPixVersion') => 'Ver. '.number_format(dintval($exif['EXIF']['FlashPixVersion']) / 100, 2),
