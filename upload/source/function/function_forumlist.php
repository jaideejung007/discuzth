...
function forumselect($groupselectable = FALSE, $arrayformat = 0, $selectedfid = 0, $showhide = FALSE, $evalue = FALSE, $special = 0) {
	...
	$forumlist = $arrayformat ? [] : '<optgroup label="&nbsp;">';
...
function recommendupdate($fid, &$modrecommend, $force = '', $position = 0) {
	...
	if($modrecommend['sort'] && (TIMESTAMP - $modrecommend['updatetime'] > $modrecommend['cachelife'] || $force)) {
		foreach(table_forum_forumrecommend::t()->fetch_all_by_fid($fid) as $row) {
			if($modrecommend['sort'] == 2 && $row['moderatorid']) {
				$modedtids[] = $row['tid'];
			}
		}
		table_forum_forumrecommend::t()->delete_by_fid($fid, $modrecommend['sort'] == 2 ? 0 : false);
		$orderby = 'dateline';
		...
		if($tids && $imagenum) {
			...
			foreach($attachtables as $attachtable => $tids) {
				$attachmentpost = [];
				$postlist = table_forum_post::t()->fetch_all_by_tid(0, $tids, false, '', 0, 0, 1);
				if($postlist) {
					$pids = [];
					foreach($postlist as $post) {
						$pids[] = $post['pid'];
					}
					$attachmentlist = table_forum_attachment_n::t()->fetch_all_by_pid_width('tid:'.$tids[0], $pids, $imgw);
					if($attachmentlist) {
		...
		if($addthread) {
			...
			table_forum_forumfield::t()->update($fid, ['modrecommend' => $modrecommendnew]);
		}
	}

	$recommendlists = $recommendlist = $recommendimagelist = [];
	foreach(table_forum_forumrecommend::t()->fetch_all_by_fid($fid, $position) as $recommend) {
...
function showstars($num) {
	...
	if(empty($_G['setting']['starthreshold'])) {
		$return .= str_repeat('<i class="fico-star1 fic4 fc-l" '.$alt.'></i>', $num);
	} else {
		for($i = 3; $i > 0; $i--) {
			$numlevel = intval($num / pow($_G['setting']['starthreshold'], ($i - 1)));
			$num = ($num % pow($_G['setting']['starthreshold'], ($i - 1)));
			$return .= str_repeat('<i class="fico-star'.$i.' fic4 fc-l" '.$alt.'></i>', $numlevel);
		}
	}
	return $return;
}
...
function forumleftside() {
	global $_G;
	$leftside = ['favorites' => [], 'forums' => []];
	$leftside['forums'] = forumselect(FALSE, 1);
	if($_G['uid']) {
		foreach(table_home_favorite::t()->fetch_all_by_uid_idtype($_G['uid'], 'fid') as $id => $forum) {
			if($_G['fid'] == $forum['id']) {
				$_G['forum_fidinfav'] = $forum['favid'];
			}
			$leftside['favorites'][$forum['id']] = [$forum['title'], $forum['favid']];
		}
	}
...
function threadclasscount($fid, $id = 0, $idtype = '', $count = null) {
	...
	$typeflag = ($id && $idtype && in_array($idtype, ['typeid', 'sortid']));
	$threadclasscount = table_common_cache::t()->fetch('threadclasscount_'.$fid);
...
function get_attach($list, $video = false, $audio = false) {
	...
	foreach($posttableids as $id) {
		...
		foreach($posts as $value) {
			if(!$_G['forum']['ismoderator'] && $value['status'] & 1) {
			...
			} elseif(str_contains($value['message'], '[/password]')) {
				...
				if(!empty($value['summary'])) {
					$threadlist_data[$value['tid']]['message'] = $value['summary'];
				} else {
					...
					if($value['message'] && ($video || $audio)) {
						$value['media'] = '';
						$value['message'] = preg_replace(['/\[hide=?\d*\](.*?)\[\/hide\]/is'], [''], $value['message']);
						$value['msglower'] = strtolower($value['message']);
						if(str_contains($value['msglower'], '[/media]') && $video) {
						...
						} elseif(str_contains($value['msglower'], '[/audio]') && $audio) {
