...
$groupids = $typelist = [];
$groupnav = '';
$selectorder = ['default' => '', 'thread' => '', 'membernum' => '', 'dateline' => '', 'activity' => ''];
...
$seodata = ['first' => $nav['first']['name'], 'second' => $nav['second']['name']];
list($navtitle, $metadescription, $metakeywords) = get_seosetting('group', $seodata);

$data = $randgrouplist = $randgroupdata = $grouptop = $newgrouplist = $fids = [];
...
if(empty($_G['cache']['groupindex']) || $cachetimeupdate > 3600 || empty($lastupdategroup)) {
	$data['randgroupdata'] = $randgroupdata = grouplist('lastupdate', ['ff.membernum', 'ff.icon'], 80);
	$data['topgrouplist'] = $topgrouplist = grouplist('activity', ['f.commoncredits', 'ff.membernum', 'ff.icon'], 10);
	...
	foreach($first as $id => $toptype) {
		...
		$query = table_forum_forum::t()->fetch_all_sub_group_by_fup($toptype['secondlist']);
		...
		if(empty($data['lastupdategroup'][$id])) $data['lastupdategroup'][$id] = [];
	}
	$lastupdategroupdetail = grouplist('activity', ['ff.membernum', 'ff.icon'], count($fids), $fids);
...
$list = [];
if($groupids) {
	$orderby = in_array(getgpc('orderby'), ['membernum', 'dateline', 'thread', 'activity']) ? getgpc('orderby') : 'displayorder';
	$page = intval(getgpc('page')) ? intval($_GET['page']) : 1;
	$page = $page > 65535 ? 1 : $page;
	$start = ($page - 1) * $perpage;
	$getcount = grouplist('', '', '', $groupids, 1, 1);
	if($getcount) {
		$list = grouplist($orderby, '', [$start, $perpage], $groupids, 1);
