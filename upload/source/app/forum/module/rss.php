...
if(empty($rssfid)) {
	...
	$forum = isset($_G['cache']['forums'][$rssfid]) && $_G['cache']['forums'][$rssfid]['type'] != 'group' ? $_G['cache']['forums'][$rssfid] : [];
	if(!isset($_G['cache']['forums'][$rssfid])) {
		$forum = $_G['cache']['forums'][$rssfid] = [];
		$subforum = table_forum_forum::t()->fetch_info_by_fid($rssfid);
...
if($fidarray) {
	$alldata = table_forum_rsscache::t()->fetch_all_by_fid($fidarray, $num);
	if($alldata) {
		foreach($alldata as $thread) {
			if(TIMESTAMP - $thread['lastupdate'] > $ttl * 60) {
				...
				[$thread['description'], $attachremote, $attachfile, $attachsize] = explode("\t", $thread['description']);
				if($attachfile) {
					if($attachremote) {
						$filename = $_G['setting']['ftp']['attachurl'].'forum/'.$attachfile;
					} else {
						$filename = (!str_contains($_G['setting']['attachurl'], '://') ? $_G['siteurl'] : '').$_G['setting']['attachurl'].'forum/'.$attachfile;
					}
				}
				echo "    <item>\n".
...
function updatersscache($num) {
	...
	table_forum_rsscache::t()->truncate();
