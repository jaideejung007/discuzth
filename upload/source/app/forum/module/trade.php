...
if(!empty($orderid) && empty($_GET['apitype'])) {
	$orderinfo = table_forum_tradelog::t()->fetch($orderid);
...
if(!empty($orderid)) {
	...
	$limit = 6;
	$query = table_forum_trade::t()->fetch_all_for_seller($tradelog['sellerid'], $limit);
	$usertrades = [];
	$usertradecount = 0;
	...
	if(!empty($_GET['pay']) && !$tradelog['offline'] && $tradelog['status'] == 0 && $tradelog['buyerid'] == $_G['uid']) {
		if($_G['setting']['creditstransextra'][5] != -1 && $tradelog['credit']) {
			...
			updatemembercount($tradelog['buyerid'], [$_G['setting']['creditstransextra'][5] => -$tradelog['credit']]);
		}
		$trade = table_forum_trade::t()->fetch_goods($tradelog['tid'], $tradelog['pid']);

		if($_G['uid'] && $currentcredit < $discountcredit && $tradelog['discount']) {
			showmessage('trade_credits_no_enough', '', ['credittitle' => $_G['setting']['extcredits'][$_G['setting']['creditstrans']]['title']]);
		}
		$pay = [];
		$pay['commision'] = 0;
		$transport = $tradelog['transport'];
		$transportfee = 0;
		trade_setprice(['fee' => $fee, 'trade' => $trade, 'transport' => $transport], $price, $pay, $transportfee);
		...
		table_forum_tradelog::t()->update($orderid, ['paytype' => $paytype]);
	...
	if(submitcheck('offlinesubmit') && in_array($_GET['offlinestatus'], trade_offline($tradelog, 0))) {
		...
		if($_GET['offlinestatus'] == 4) {
			...
			$data = ['lastbuyer' => $tradelog['buyer'], 'lastupdate' => $_G['timestamp']];
			table_forum_trade::t()->update_trade($tradelog['tid'], $tradelog['pid'], $data);
			...
			if($_G['setting']['creditstransextra'][5] != -1 && $tradelog['basecredit']) {
				updatemembercount($tradelog['buyerid'], [$_G['setting']['creditstransextra'][5] => $tradelog['number'] * $tradelog['basecredit']]);
			}
		}
	...
	if(submitcheck('tradesubmit')) {
		...
		if($tradelog['status'] == 0) {
			...
			if($tradelog['sellerid'] == $_G['uid']) {
				...
				notification_add($tradelog['buyerid'], 'goods', 'trade_order_update_sellerid', [
					'seller' => $tradelog['seller'],
					'sellerid' => $tradelog['sellerid'],
					'orderid' => $orderid,
					'subject' => $tradelog['subject']
			...
			if($tradelog['buyerid'] == $_G['uid']) {
				...
				table_forum_trade::t()->update_trade($tradelog['tid'], $tradelog['pid'], ['amount' => $amount]);
				...
				notification_add($tradelog['sellerid'], 'goods', 'trade_order_update_buyerid', [
					'buyer' => $tradelog['buyer'],
					'buyerid' => $tradelog['buyerid'],
					'orderid' => $orderid,
					'subject' => $tradelog['subject']
			...
			if($update) {
				...
				table_forum_tradelog::t()->update($orderid, $update);
				$tradelog = table_forum_tradelog::t()->fetch($orderid);
			}
		}
	...
	if(empty($_GET['pid'])) {
		$pid = table_forum_post::t()->fetch_threadpost_by_tid_invisible($_G['tid']);
		$pid = $pid['pid'];
	} else {
		$pid = $_GET['pid'];
	}
	...
	$trade = table_forum_trade::t()->fetch_goods($_G['tid'], $pid);
	if(empty($trade)) {
	...
	$limit = 6;
	$query = table_forum_trade::t()->fetch_all_for_seller($trade['sellerid'], $limit);
	$usertrades = [];
	$usertradecount = 0;
	foreach($query as $usertrade) {
		$usertradecount++;
	...
	if($_GET['action'] != 'trade' && !submitcheck('tradesubmit')) {
		$lastbuyerinfo = dhtmlspecialchars(table_forum_tradelog::t()->fetch_last($_G['uid']));
		...
		$transportfee = 0;
		trade_setprice(['fee' => $fee, 'trade' => $trade, 'transport' => $_GET['transport']], $price, $pay, $transportfee);
		...
		table_forum_trade::t()->update_counter($trade['tid'], $trade['pid'], 0, 0, 0, '-'.$_GET['number']);
