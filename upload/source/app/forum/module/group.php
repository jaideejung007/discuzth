...
$actionarray = ['join', 'out', 'create', 'viewmember', 'manage', 'index', 'memberlist', 'recommend'];
$action = getgpc('action') && in_array($_GET['action'], $actionarray) ? $_GET['action'] : 'index';
if(in_array($action, ['join', 'out', 'create', 'manage', 'recommend'])) {
	if(empty($_G['uid'])) {
		showmessage('not_loggedin', '', '', ['login' => 1]);
	}
}
...
if($_G['fid']) {
	...
	$groupcache = getgroupcache($_G['fid'], ['replies', 'views', 'digest', 'lastpost', 'ranking', 'activityuser', 'newuserlist'], 604800);
	...
	$groupuser = table_forum_groupuser::t()->fetch_userinfo($_G['uid'], $_G['fid']);
	$onlinemember = grouponline($_G['fid'], 1);
	$groupmanagers = $_G['forum']['moderators'];
	$nav = get_groupnav($_G['forum']);
	$groupnav = $nav['nav'];

	$seodata = ['forum' => $_G['forum']['name'], 'first' => $nav['first']['name'], 'second' => $nav['second']['name'], 'gdes' => $_G['forum']['description']];
...
if(in_array($action, ['out', 'viewmember', 'manage', 'index', 'memberlist'])) {
...
if($action == 'index' && $status != 2) {

	$newuserlist = $activityuserlist = [];
...
if($action == 'index') {
	...
	if($status != 2) {
		...
		if($forumstickytids) {
			foreach(table_forum_thread::t()->fetch_all_by_tid_or_fid($_G['fid'], $forumstickytids) as $row) {
		...
		$newthreadlist = getgroupcache($_G['fid'], ['dateline'], 0, 10, 0, 1);
		foreach($newthreadlist['dateline']['data'] as $key => $thread) {
			...
			} elseif(empty($_G['cookie']['oldtopics']) || !str_contains($_G['cookie']['oldtopics'], 'D'.$thread['tid'].'D')) {
		...
		if(!IS_ROBOT) {
			...
			if($activityuser) {
				$query = table_home_feed::t()->fetch_all_by_uid_dateline($activityuser);
	...
	$oparray = ['card', 'address', 'alluser'];
	$op = getgpc('op') && in_array($_GET['op'], $oparray) ? $_GET['op'] : 'alluser';
	$page = intval(getgpc('page')) ? intval($_GET['page']) : 1;
	$perpage = 50;
	$start = ($page - 1) * $perpage;

	$alluserlist = $adminuserlist = [];
	$staruserlist = $page < 2 ? table_forum_groupuser::t()->groupuserlist($_G['fid'], 'lastupdate', 0, 0, ['level' => '3'], ['uid', 'username', 'level', 'joindateline', 'lastupdate']) : '';
	$adminlist = $groupmanagers && $page < 2 ? $groupmanagers : [];

	if($op == 'alluser') {
		$alluserlist = table_forum_groupuser::t()->groupuserlist($_G['fid'], 'lastupdate', $perpage, $start, "AND level='4'", '', $onlinemember['list']);
	...
	if(!empty($membermaximum)) {
		$curnum = table_forum_groupuser::t()->fetch_count_by_fid($_G['fid']);
		if($curnum >= $membermaximum) {
			showmessage('group_member_maximum', '', ['membermaximum' => $membermaximum]);
		}
	}
	if($groupuser['uid']) {
		...
		$confirmjoin = TRUE;
		$inviteuid = table_forum_groupinvite::t()->fetch_uid_by_inviteuid($_G['fid'], $_G['uid']);
		...
		if($confirmjoin) {
			...
			if($inviteuid) {
				table_forum_groupinvite::t()->delete_by_inviteuid($_G['fid'], $_G['uid']);
			}
			if($modmember == 4) {
				table_forum_forumfield::t()->update_membernum($_G['fid']);
			}
			table_forum_forumfield::t()->update($_G['fid'], ['lastupdate' => TIMESTAMP]);
		}
		include_once libfile('function/stat');
		updatestat('groupjoin');
		delgroupcache($_G['fid'], ['activityuser', 'newuserlist']);
	...
	table_forum_forumfield::t()->update_membernum($_G['fid'], -1);
	...
	if($_G['group']['buildgroupcredits']) {
		if(empty($creditstransextra)) {
			...
			if($_G['member']['extcredits'.$creditstransextra] < $_G['group']['buildgroupcredits']) {
				showmessage('group_create_usergroup_credits_failed', '', ['buildgroupcredits' => $_G['group']['buildgroupcredits'].$_G['setting']['extcredits'][$creditstransextra]['unit'].$_G['setting']['extcredits'][$creditstransextra]['title']]);
			}
		}
	}

	$groupnum = table_forum_forumfield::t()->fetch_groupnum_by_founderuid($_G['uid']);
	...
	if(!submitcheck('createsubmit')) {
		...
		if(table_forum_forum::t()->fetch_fid_by_name($name)) {
		...
		if(empty($_G['setting']['groupmod']) || $_G['adminid'] == 1) {
			$levelinfo = table_forum_grouplevel::t()->fetch_by_credits();
			$levelid = $levelinfo['levelid'];
		} else {
			$levelid = -1;
		}
		$newfid = table_forum_forum::t()->insert_group($fup, 'sub', $name, '3', $levelid);
		if($newfid) {
		...
		if($creditstransextra && $_G['group']['buildgroupcredits']) {
			updatemembercount($_G['uid'], [$creditstransextra => -$_G['group']['buildgroupcredits']], 1, 'BGR', $newfid);
		}
		include_once libfile('function/stat');
		updatestat('group');
		if($levelid == -1) {
			showmessage('group_create_mod_succeed', 'group.php?mod=my&view=manager', [], ['alert' => 'right', 'showdialog' => 1, 'showmsg' => true, 'locationtime' => true]);
		}
		showmessage('group_create_succeed', "forum.php?mod=group&action=manage&fid=$newfid", [], ['showdialog' => 1, 'showmsg' => true, 'locationtime' => true]);
	}
	...
	$oparray = ['group', 'checkuser', 'manageuser', 'threadtype', 'demise'];
	$_GET['op'] = getgpc('op') && in_array($_GET['op'], $oparray) ? $_GET['op'] : 'group';
	if(empty($groupmanagers[$_G['uid']]) && !in_array($_GET['op'], ['group', 'threadtype', 'demise']) && $_G['adminid'] != 1) {
		showmessage('group_admin_noallowed');
	}
	$page = intval(getgpc('page')) ? intval($_GET['page']) : 1;
	$perpage = 50;
	...
	if($_GET['op'] == 'group') {
		...
		if(submitcheck('groupmanage')) {
			...
			if(isset($_GET['domain']) && $_G['forum']['domain'] != $_GET['domain']) {
				...
				if(empty($domainlength) || empty($domain)) {
					...
					if(domaincheck($domain, $_G['setting']['domain']['root']['group'], $domainlength)) {
						deletedomain($_G['fid'], 'group');
						table_common_domain::t()->insert(['domain' => $domain, 'domainroot' => $_G['setting']['domain']['root']['group'], 'id' => $_G['fid'], 'idtype' => 'group']);
					}
			...
			if(($_GET['name'] && !empty($specialswitch['allowchangename'])) || ($_GET['fup'] && !empty($specialswitch['allowchangetype']))) {
				...
				if(!empty($_GET['name']) && $_GET['name'] != $_G['forum']['name']) {
					if(table_forum_forum::t()->fetch_fid_by_name($_GET['name'])) {
			...
			if($forumarr) {
				table_forum_forum::t()->update($_G['fid'], $forumarr);
				if($forumarr['fup']) {
					table_forum_forumfield::t()->update_groupnum($forumarr['fup'], 1);
					table_forum_forumfield::t()->update_groupnum($_G['forum']['fup'], -1);
					require_once libfile('function/cache');
					updatecache('grouptype');
				}
			}
			...
			if($iconnew) {
				...
				if($group_recommend[$_G['fid']]) {
					...
					table_common_setting::t()->update_setting('group_recommend', $group_recommend);
					require_once libfile('function/cache');
					updatecache('setting');
				}
			}
			...
			table_forum_forumfield::t()->update($_G['fid'], $setarr);
			...
			$gviewpermselect = $jointypeselect = ['', '', ''];
		...
		if($checkusers) {
			foreach($checkusers as $uid) {
				...
				notification_add($uid, 'group', $notification, ['fid' => $_G['fid'], 'groupname' => $_G['forum']['name'], 'url' => $_G['siteurl'].'forum.php?mod=group&fid='.$_G['fid']], 1);
			}
			if($checktype == 1) {
			...
			$checkusers = [];
			$userlist = table_forum_groupuser::t()->groupuserlist($_G['fid'], 'joindateline', $perpage, $start, ['level' => 0]);
		...
		$mtype = [1 => lang('group/template', 'group_moderator'), 2 => lang('group/template', 'group_moderator_vice'), 3 => lang('group/template', 'group_star_member_title'), 4 => lang('group/misc', 'group_normal_member'), 5 => lang('group/misc', 'group_goaway')];
		if(!submitcheck('manageuser')) {
			$userlist = [];
			if(empty($_GET['srchuser'])) {
				$staruserlist = $page < 2 ? table_forum_groupuser::t()->groupuserlist($_G['fid'], '', 0, 0, ['level' => '3'], ['uid', 'username', 'level', 'joindateline', 'lastupdate']) : '';
				$adminuserlist = $groupmanagers && $page < 2 ? $groupmanagers : [];
				$multipage = multi($_G['forum']['membernum'], $perpage, $page, $url);
			} else {
				$start = 0;
			}
			$userlist = table_forum_groupuser::t()->groupuserlist($_G['fid'], '', $perpage, $start, $_GET['srchuser'] ? "AND username like '".addslashes($_GET['srchuser'])."%'" : "AND level='4'");
		} else {
			$muser = getgpc('muid');
			...
			if($muser && is_array($muser)) {
				foreach($muser as $muid => $mlevel) {
					...
					if($_G['adminid'] == 1 || ($muid != $_G['uid'] && ($_G['forum']['founderuid'] == $_G['uid'] || !$groupmanagers[$muid] || $groupmanagers[$muid]['level'] > $groupuser['level']))) {
						if($targetlevel != 5) {
							table_forum_groupuser::t()->update_for_user($muid, $_G['fid'], null, null, $targetlevel);
						} else {
							if(!$groupmanagers[$muid] || count($groupmanagers) > 1) {
								table_forum_groupuser::t()->delete_by_fid($_G['fid'], $muid);
								table_forum_forumfield::t()->update_membernum($_G['fid'], -1);
							} else {
		...
		if(!submitcheck('groupthreadtype')) {
			...
			foreach(table_forum_threadclass::t()->fetch_all_by_fid($_G['fid']) as $type) {
				...
				if($newname) {
					foreach($newname as $key => $val) {
						...
						if($_GET['newenable'][$key] && $val) {
							$newtype = table_forum_threadclass::t()->fetch_by_fid_name($_G['fid'], $val);
							$newtypeid = $newtype['typeid'];
							if(!$newtypeid) {
								$typenum = table_forum_threadclass::t()->count_by_fid($_G['fid']);
								if($typenum < $typenumlimit) {
									$threadtypes_newdisplayorder = intval($_GET['newdisplayorder'][$key]);
									$newtypeid = table_forum_threadclass::t()->insert(['fid' => $_G['fid'], 'name' => $val, 'displayorder' => $threadtypes_newdisplayorder], true);
								}
							}
							if($newtypeid) {
			...
			if($threadtypesnew['status']) {
				if(is_array($threadtypesnew['options']) && $threadtypesnew['options']) {
					...
					if(!empty($threadtypesnew['options']['delete'])) {
						table_forum_threadclass::t()->delete_by_typeid_fid($threadtypesnew['options']['delete'], $_G['fid']);
					}
					foreach(table_forum_threadclass::t()->fetch_all_by_typeid_fid($typeids, $_G['fid']) as $type) {
						if($threadtypesnew['options']['name'][$type['typeid']] != $type['name'] || $threadtypesnew['options']['displayorder'][$type['typeid']] != $type['displayorder']) {
							...
							table_forum_threadclass::t()->update_by_typeid_fid($type['typeid'], $_G['fid'], [
			...
			table_forum_forumfield::t()->update($_G['fid'], ['threadtypes' => $threadtypesnew]);
		...
		if((!empty($_G['forum']['founderuid']) && $_G['forum']['founderuid'] == $_G['uid']) || $_G['adminid'] == 1) {
			...
			if(submitcheck('groupdemise')) {
				...
				if($allowbuildgroup > 0) {
					$groupnum = table_forum_forumfield::t()->fetch_groupnum_by_founderuid($suid);
				}
				if(empty($allowbuildgroup) || $allowbuildgroup - $groupnum < 1) {
					showmessage('group_demise_receiver_cannot_do');
				}
				table_forum_forumfield::t()->update($_G['fid'], ['founderuid' => $suid, 'foundername' => $user['username']]);
				table_forum_groupuser::t()->update_for_user($suid, $_G['fid'], NULL, NULL, 1);
				update_groupmoderators($_G['fid']);
				sendpm($suid, lang('group/misc', 'group_demise_message_title', ['forum' => $_G['forum']['name']]), lang('group/misc', 'group_demise_message_body', ['forum' => $_G['forum']['name'], 'siteurl' => $_G['siteurl'], 'fid' => $_G['fid']]), $_G['uid']);
	...
	if(!$_G['forum']['ismoderator'] || !in_array($_G['adminid'], [1, 2])) {
		showmessage('group_admin_noallowed');
	}
	if(submitcheck('grouprecommend')) {
		if($_GET['recommend'] != $_G['forum']['recommend']) {
			table_forum_forum::t()->update($_G['fid'], ['recommend' => intval($_GET['recommend'])]);
			require_once libfile('function/cache');
			updatecache('forumrecommend');
		}
		showmessage('grouprecommend_succeed', '', [], ['alert' => 'right', 'closetime' => true, 'showdialog' => 1]);
	} else {
