...
class extend_thread_rushreply extends extend_thread_base {
	...
	public function after_newthread() {
		if($this->group['allowpostrushreply']) {
			$rushdata = ['tid' => $this->tid, 'stopfloor' => $_GET['stopfloor'], 'starttimefrom' => $_GET['rushreplyfrom'], 'starttimeto' => $_GET['rushreplyto'], 'rewardfloor' => $_GET['rewardfloor'], 'creditlimit' => $_GET['creditlimit'], 'replylimit' => $_GET['replylimit']];
			table_forum_threadrush::t()->insert($rushdata);
		}
	}
	...
	public function before_newreply() {
		...
		if(getstatus($this->thread['status'], 3) && $rushinfo['replylimit'] > 0) {
			$replycount = table_forum_post::t()->count_by_tid_invisible_authorid($this->thread['tid'], $_G['uid']);
	...
	public function after_newreply() {
		...
		if(getstatus($this->thread['status'], 3) && $this->param['maxposition']) {
			...
			if($rushstopfloor > 0 && $this->thread['closed'] == 0 && $this->param['maxposition'] >= $rushstopfloor) {
				$this->param['updatethreaddata'] = array_merge((array)$this->param['updatethreaddata'], ['closed' => 1]);
			}
		}
	}
	...
	public function before_editpost($parameters) {
		...
		if($isfirstpost) {
			if($rushreply) {
				...
				$maxposition = table_forum_post::t()->fetch_maxposition_by_tid($this->thread['posttableid'], $this->thread['tid']);
				...
				$rushdata = ['stopfloor' => $_GET['stopfloor'], 'starttimefrom' => $_GET['rushreplyfrom'], 'starttimeto' => $_GET['rushreplyto'], 'rewardfloor' => $_GET['rewardfloor'], 'creditlimit' => $_GET['creditlimit'], 'replylimit' => $_GET['replylimit']];
				table_forum_threadrush::t()->update($this->thread['tid'], $rushdata);
			}
		}
	}
