...
use table_forum_grouplevel;
use table_forum_threadimage;
...
class extend_thread_image extends extend_thread_base {
	...
	public function after_newthread() {
		...
		if($threadimageaid) {
			if(!$threadimage) {
				$threadimage = table_forum_attachment_n::t()->fetch_attachment('tid:'.$tid, $threadimageaid);
			}
	...
	private function mobile_upload() {
		...
		if(getgpc('mobile') == 'yes' && !empty($_FILES['Filedata'])) {
			...
			if($upload) {
				$_GET['attachnew'][$upload->getaid] = ['description' => ''];
			}
		}
	}
	...
	public function before_editpost($parameters) {
		...
		if($isfirstpost && $attachupdate) {
			if(!$this->param['threadimageaid']) {
				$this->param['threadimage'] = table_forum_attachment_n::t()->fetch_max_image('tid:'.$this->thread['tid'], 'pid', $this->post['pid']);
			...
			if($this->param['threadimageaid']) {
				if(!$this->param['threadimage']) {
					$this->param['threadimage'] = table_forum_attachment_n::t()->fetch_max_image('tid:'.$this->thread['tid'], 'tid', $this->thread['tid']);
				}
				table_forum_threadimage::t()->delete_by_tid($this->thread['tid']);
				table_forum_threadimage::t()->insert([
					'tid' => $this->thread['tid'],
	...
	public function before_deletepost($parameters) {
		...
		$this->param['updatefieldarr']['attachment'] = [$thread_attachment];
