...
use DB;
use table_common_credit_log;
use table_forum_post;
use table_forum_replycredit;
...
class extend_thread_replycredit extends extend_thread_base {
	...
	public function after_newthread() {
		...
		if($this->group['allowreplycredit']) {
			if($this->param['replycredit'] > 0 && $this->replycredit_real > 0) {
				updatemembercount($this->member['uid'], ['extcredits'.$this->setting['creditstransextra']['10'] => -$this->replycredit_real], 1, 'RCT', $this->tid);
				$insertdata = [
					'tid' => $this->tid,
				...
				table_forum_replycredit::t()->insert($insertdata);
			}
		}
	...
	public function after_newreply() {
		if($this->thread['replycredit'] > 0 && !$this->param['modnewreplies'] && $this->thread['authorid'] != $this->member['uid'] && $this->member['uid']) {
			...
			$replycredit_rule = table_forum_replycredit::t()->fetch($this->thread['tid']);
			if(!empty($replycredit_rule['times'])) {
				$have_replycredit = table_common_credit_log::t()->count_by_uid_operation_relatedid($this->member['uid'], 'RCA', $this->thread['tid']);
				if($replycredit_rule['membertimes'] - $have_replycredit > 0 && $this->thread['replycredit'] - $replycredit_rule['extcredits'] >= 0) {
					...
					if($replycredit_rule['random'] > 0) {
						$rand = rand(1, 100);
						$rand_replycredit = $rand <= $replycredit_rule['random'];
					} else {
						$rand_replycredit = true;
					}
					if($rand_replycredit) {
						updatemembercount($this->member['uid'], [$replycredit_rule['extcreditstype'] => $replycredit_rule['extcredits']], 1, 'RCA', $this->thread['tid']);
						table_forum_post::t()->update_post('tid:'.$this->thread['tid'], $this->pid, ['replycredit' => $replycredit_rule['extcredits']]);
						table_forum_thread::t()->update($this->thread['tid'], (array)DB::field('replycredit', $this->thread['replycredit'] - $replycredit_rule['extcredits']), false, false, 0, true);
					}
				}
			}
		}
	...
	public function before_editpost($parameters) {
		...
		if($isfirstpost) {
			if($isorigauthor && $this->group['allowreplycredit']) {
				$replycredit_rule = isset($parameters['replycredit_rule']) && $parameters['replycredit_rule'] ? $parameters['replycredit_rule'] : [];
				...
				if($_POST['replycredit_extcredits'] > 0 && $_POST['replycredit_times'] > 0) {
					...
					if($replycredit_diff) {
						updatemembercount($this->thread['authorid'], [$replycredit_rule['extcreditstype'] => ($replycredit_diff > 0 ? -$replycredit_diff : abs($replycredit_diff))], 1, ($replycredit_diff > 0 ? 'RCT' : 'RCB'), $this->thread['tid']);
					}
				} elseif(($_POST['replycredit_extcredits'] == 0 || $_POST['replycredit_times'] == 0) && $this->thread['replycredit'] > 0) {
					$replycredit = 0;
					table_forum_replycredit::t()->delete($this->thread['tid']);
					updatemembercount($this->thread['authorid'], [$replycredit_rule['extcreditstype'] => $this->thread['replycredit']], 1, 'RCB', $this->thread['tid']);
				...
				if($replycredit) {
					...
					table_forum_replycredit::t()->insert($replydata, false, true);
				}
			}
		}
	}
	...
	public function before_deletepost() {
		...
		if($this->thread['replycredit'] && $isfirstpost && !$this->param['isanonymous']) {
			updatemembercount($this->post['authorid'], [$replycredit_rule['extcreditstype'] => $this->thread['replycredit']], true, 'RCB', $this->thread['tid']);
			table_forum_replycredit::t()->delete($this->thread['tid']);
