...
use table_forum_poll;
use table_forum_polloption;
use table_forum_polloption_image;
...
class extend_thread_poll extends extend_thread_base {
	...
	public function before_newthread($parameters) {
		...
		if(count($polloption) > $maxpolloptions) {
			showmessage('post_poll_option_toomany', '', ['maxpolloptions' => $maxpolloptions]);
		...
		if(!empty($_GET['pollimage']) && count($_GET['pollimage']) != $curpolloption) {
			showmessage('poll_imgoptions_invalid');
		}
	...
	public function after_newthread() {
		foreach($this->pollarray['options'] as $ppkey => $polloptvalue) {
			...
			$polloptionid = table_forum_polloption::t()->insert(['tid' => $this->tid, 'polloption' => $polloptvalue], true);
			if($this->pollarray['pollimage'][$ppkey]) {
				table_forum_polloption_image::t()->update($this->pollarray['pollimage'][$ppkey], ['poid' => $polloptionid, 'tid' => $this->tid, 'pid' => $this->pid]);
				$this->pollarray['isimage'] = 1;
			}
		}
		$this->polloptionpreview = '';
		$query = table_forum_polloption::t()->fetch_all_by_tid($this->tid, 1, 2);
		...
		$data = ['tid' => $this->tid, 'multiple' => $this->pollarray['multiple'], 'visible' => $this->pollarray['visible'], 'maxchoices' => $this->pollarray['maxchoices'], 'expiration' => $this->pollarray['expiration'], 'overt' => $this->pollarray['overt'], 'pollpreview' => $this->polloptionpreview, 'isimage' => $this->pollarray['isimage']];
		table_forum_poll::t()->insert($data);
	}
			...
			if($this->thread['special'] == 1 && ($this->group['alloweditpoll'] || $isorigauthor) && !empty($_GET['polls'])) {
				...
				if($pollarray['options']) {
					if(count($pollarray['options']) > $this->setting['maxpolloptions']) {
						showmessage('post_poll_option_toomany', '', ['maxpolloptions' => $this->setting['maxpolloptions']]);
					}
					foreach($pollarray['options'] as $key => $value) {
						$pollarray['options'][$key] = censor($pollarray['options'][$key]);
						if(!trim($value)) {
							table_forum_polloption::t()->delete_safe_tid($this->thread['tid'], $key);
					...
					$optid = [];
					$query = table_forum_polloption::t()->fetch_all_by_tid($this->thread['tid']);
					...
					foreach($pollarray['options'] as $key => $value) {
						...
						if(in_array($_GET['polloptionid'][$key], $optid)) {
							if($this->group['alloweditpoll']) {
								table_forum_polloption::t()->update_safe_tid($_GET['polloptionid'][$key], $this->thread['tid'], $pollarray['displayorder'][$key], $value);
							} else {
								table_forum_polloption::t()->update_safe_tid($_GET['polloptionid'][$key], $this->thread['tid'], $pollarray['displayorder'][$key]);
							}
						} else {
							$polloptionid = table_forum_polloption::t()->insert(['tid' => $this->thread['tid'], 'displayorder' => $pollarray['displayorder'][$key], 'polloption' => $value], true);
							if($pollarray['pollimage'][$key]) {
								table_forum_polloption_image::t()->update($pollarray['pollimage'][$key], ['poid' => $polloptionid, 'tid' => $this->thread['tid'], 'pid' => $this->post['pid']]);
								$pollarray['isimage'] = 1;
							}
						}
					}
					$polloptionpreview = '';
					$query = table_forum_polloption::t()->fetch_all_by_tid($this->thread['tid'], 1, 2);
					foreach($query as $option) {
						$polloptvalue = preg_replace("/\[url=(https?){1}:\/\/([^\[\"']+?)\](.+?)\[\/url\]/i", "<a href=\"\\1://\\2\" target=\"_blank\">\\3</a>", $option['polloption']);
					...
					$data = ['multiple' => $pollarray['multiple'], 'visible' => $pollarray['visible'], 'maxchoices' => $pollarray['maxchoices'], 'expiration' => $pollarray['expiration'], 'overt' => $pollarray['overt'], 'pollpreview' => $polloptionpreview];
					if($pollarray['isimage']) {
						$data['isimage'] = 1;
					}
					table_forum_poll::t()->update($this->thread['tid'], $data);
				} else {
					$this->param['threadupdatearr']['special'] = 0;
					table_forum_poll::t()->delete($this->thread['tid']);
					table_forum_polloption::t()->delete_safe_tid($this->thread['tid']);
				}
			}
		}
	}
