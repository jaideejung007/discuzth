...
class extend_thread_trade extends extend_thread_base {
	...
	public function before_newthread($parameters) {
		...
		if(!trim($_GET['item_name'])) {
			...
			return $this->showmessage('trade_price_between', '', ['mintradeprice' => $this->group['mintradeprice'], 'maxtradeprice' => $this->group['maxtradeprice']]);
		} elseif($this->group['maxtradeprice'] && $item_credit > 0 && ($this->group['mintradeprice'] > $item_credit || $this->group['maxtradeprice'] < $item_credit)) {
			return $this->showmessage('trade_credit_between', '', ['mintradeprice' => $this->group['mintradeprice'], 'maxtradeprice' => $this->group['maxtradeprice']]);
	...
	public function before_feed() {
		if(!empty($_GET['addfeed']) && $this->forum['allowfeed'] && !$this->param['isanonymous']) {
			...
			if($_GET['tradeaid']) {
				$this->feed['images'] = [getforumimg($_GET['tradeaid'])];
				$this->feed['image_links'] = ['forum.php?mod=viewthread&do=tradeinfo&tid='.$this->tid."&pid=$pid"];
			}
			if($_GET['tradeaid']) {
				$attachment = table_forum_attachment_n::t()->fetch_attachment('tid:'.$this->tid, $_GET['tradeaid']);
				if(in_array($attachment['filetype'], ['image/gif', 'image/jpeg', 'image/png'])) {
	...
	public function after_feed() {
		global $extra;
		$values = ['fid' => $this->forum['fid'], 'tid' => $this->tid, 'pid' => $this->pid, 'coverimg' => ''];
	...
	public function before_replyfeed() {
		if($this->forum['allowfeed'] && !$this->param['isanonymous']) {
			if($this->param['special'] == 2 && !empty($_GET['trade'])) {
				...
				if($_GET['tradeaid']) {
					$this->feed['images'] = [getforumimg($_GET['tradeaid'])];
					$this->feed['image_links'] = ['forum.php?mod=viewthread&do=tradeinfo&tid='.$this->thread['tid'].'&pid='.$this->pid];
				}
			}
		}
	}
	...
	public function before_editpost($parameters) {
		...
		if($parameters['special'] == 2 && $this->group['allowposttrade']) {
			...
			if($trade = table_forum_trade::t()->fetch_goods($this->thread['tid'], $this->post['pid'])) {
				...
				if(!trim($item_name)) {
					...
					showmessage('trade_price_between', '', ['mintradeprice' => $this->group['mintradeprice'], 'maxtradeprice' => $this->group['maxtradeprice']]);
				} elseif($this->group['maxtradeprice'] && $item_credit > 0 && ($this->group['mintradeprice'] > $item_credit || $this->group['maxtradeprice'] < $item_credit)) {
					showmessage('trade_credit_between', '', ['mintradeprice' => $this->group['mintradeprice'], 'maxtradeprice' => $this->group['maxtradeprice']]);
				} elseif(!$this->group['maxtradeprice'] && $item_price > 0 && $this->group['mintradeprice'] > $item_price) {
				...
				if($trade['aid'] && $_GET['tradeaid'] && $trade['aid'] != $_GET['tradeaid']) {
					$attach = table_forum_attachment_n::t()->fetch_attachment('tid:'.$this->thread['tid'], $trade['aid']);
					table_forum_attachment::t()->delete($trade['aid']);
					table_forum_attachment_n::t()->delete_attachment('tid:'.$this->thread['tid'], $trade['aid']);
					dunlink($attach);
				...
				$data = ['aid' => $_GET['tradeaid'], 'account' => $seller, 'tenpayaccount' => $_GET['tenpay_account'], 'subject' => $item_name, 'price' => $item_price, 'amount' => $item_number, 'quality' => $item_quality, 'locus' => $item_locus, 'transport' => $item_transport, 'ordinaryfee' => $postage_mail, 'expressfee' => $postage_express, 'emsfee' => $postage_ems, 'itemtype' => $item_type, 'expiration' => $expiration, 'closed' => $closed, 'costprice' => $item_costprice, 'credit' => $item_credit, 'costcredit' => $_GET['item_costcredit']];
				table_forum_trade::t()->update_trade($this->thread['tid'], $this->post['pid'], $data);
				if(!empty($_GET['infloat'])) {
					$viewpid = table_forum_post::t()->fetch_threadpost_by_tid_invisible($this->thread['tid']);
					$viewpid = $viewpid['pid'];
	...
	public function after_deletepost() {
		if($this->thread['special'] == 2) {
			table_forum_trade::t()->delete_by_id_idtype($this->post['pid'], 'pid');
		}
	}
}
