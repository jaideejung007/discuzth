...
class model_post extends discuz_model {
	public $forum;
	...
	public function newreply($parameters) {
		...
		if(trim($this->param['subject']) == '' && ((in_array($this->param['contentType'], ['text', '']) && empty(trim($this->param['message']))) || (!empty($this->param['contentType']) && $this->param['contentType'] != 'text' && empty(trim($this->param['content'])))) && $this->thread['special'] != 2) {
			...
			return $this->showmessage('post_flood_ctrl_posts_per_hour', '', ['posts_per_hour' => $this->group['maxpostsperhour']]);
		...
		$this->param['maxposition'] = table_forum_post::t()->fetch_maxposition_by_tid($this->thread['posttableid'], $this->thread['tid']);
		...
		if(!empty($notice_funcs)) {
			if(!$this->param['modnewreplies']) {
				foreach($notice_funcs as $notice_func) {
					call_user_func_array($notice_func[0], $notice_func[1]);
				}
			} else {
				foreach($notice_funcs as $notice_func) {
					delay_task('set', 'replyNotice_'.$this->pid, $notice_func, 7 * 86400);
				}
			}
		}
		...
		if($this->param['modnewreplies']) {
			...
			table_forum_forum::t()->update_forum_counter($this->forum['fid'], 0, 0, 1, 1);
			...
			if($this->thread['lastpost'] < getglobal('timestamp')) {
				$fieldarr['lastpost'] = [getglobal('timestamp')];
			}
			$row = table_forum_threadaddviews::t()->fetch($this->thread['tid']);
			if(!empty($row)) {
				table_forum_threadaddviews::t()->update($this->thread['tid'], ['addviews' => 0]);
				$fieldarr['views'] = $row['addviews'];
			}
			$this->param['updatethreaddata'] = array_merge($this->param['updatethreaddata'], table_forum_thread::t()->increase($this->thread['tid'], $fieldarr, false, 0, true));
			if($this->thread['displayorder'] != -4) {
				updatepostcredits('+', $this->member['uid'], 'reply', $this->forum['fid']);
				if($this->forum['status'] == 3) {
					if($this->forum['closed'] > 1) {
						table_forum_thread::t()->increase($this->forum['closed'], $fieldarr, true);
					}
					table_forum_groupuser::t()->update_counter_for_user($this->member['uid'], $this->forum['fid'], 0, 1);
			...
			if($this->forum['allowfeed'] && !$this->param['isanonymous']) {
				if($this->thread['authorid'] != $this->member['uid']) {
					...
					if(!empty($forum_attachexist)) {
						$imgattach = table_forum_attachment_n::t()->fetch_max_image('tid:'.$this->thread['tid'], 'pid', $this->pid);
						$firstaid = $imgattach['aid'];
						unset($imgattach);
						if($firstaid) {
							$this->feed['images'] = [getforumimg($firstaid)];
							$this->feed['image_links'] = [$post_url];
						}
					}
				}
			}
	...
	public function editpost($parameters) {
		...
		if($isfirstpost) {
			...
			if(!empty($summary)) {
				table_forum_thread::t()->update($this->thread['tid'], ['summary' => $summary], true);
			}

			if($this->thread['tid'] > 1) {
				if($this->thread['closed'] > 1) {
					table_forum_thread::t()->update($this->thread['closed'], ['subject' => $this->param['subject']], true);
				} elseif(empty($this->thread['isgroup'])) {
					$threadclosed = table_forum_threadclosed::t()->fetch($this->thread['tid']);
					if($threadclosed['redirect']) {
						table_forum_thread::t()->update($threadclosed['redirect'], ['subject' => $this->param['subject']], true);
					}
				}
			}
			$class_tag = new tag();
		...
		if(getglobal('forum_auditstatuson') && $this->param['audit'] == 1) {
			if(getstatus($this->post['status'], 3) == 0) {
				table_forum_post::t()->update_post($this->thread['posttableid'], $this->post['pid'], ['status' => 4], false, false, null, -2, null, 0);
				updatepostcredits('+', $this->post['authorid'], ($isfirstpost ? 'post' : 'reply'), $this->forum['fid']);
			}
			if(!$isfirstpost) {
				table_forum_thread::t()->increase($this->thread['tid'], ['replies' => 1]);
			}
		...
		if(!getglobal('forum_auditstatuson') || $this->param['audit'] != 1) {
			if($isfirstpost && $this->param['modnewthreads']) {
				table_forum_thread::t()->update($this->thread['tid'], ['displayorder' => -2]);
		...
		if($this->thread['lastpost'] == $this->post['dateline'] && ((!$this->post['anonymous'] && $this->thread['lastposter'] == $this->post['author']) || ($this->post['anonymous'] && $this->thread['lastposter'] == '')) && $this->post['anonymous'] != $this->param['isanonymous']) {
			table_forum_thread::t()->update($this->thread['tid'], ['lastposter' => $this->param['isanonymous'] ? '' : $this->post['author']], true);
		}
	...
	public function deletepost($parameters) {
		...
		if(!$this->param['handlereplycredit']) {
			if(!$isfirstpost && !$this->param['isanonymous']) {
				$postreplycredit = table_forum_post::t()->fetch_post('tid:'.$this->thread['tid'], $this->post['pid']);
		...
		if($isfirstpost) {
			...
			$lastpost = table_forum_post::t()->fetch_visiblepost_by_tid('tid:'.$this->thread['tid'], $this->thread['tid'], 0, 1);
