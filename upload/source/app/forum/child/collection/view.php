...
$oplist = ['comment', 'followers', 'related'];
...
$collectionfollowdata = table_forum_collectionfollow::t()->fetch_by_ctid_uid($ctid, $_G['uid']);
$collectionteamworker = table_forum_collectionteamworker::t()->fetch_all_by_ctid($_G['collection']['ctid']);
...
if(!$op || $op == 'related') {
	...
	if(!$op && $op != 'related') {
		if($_G['collection']['uid'] == $_G['uid']) {
			...
			if($_G['collection']['lastupdate'] >= $lastvisit) {
				table_forum_collection::t()->update($ctid, ['lastvisit' => TIMESTAMP], true, true);
			}
		} elseif($isteamworkers) {
			$lastvisit = $collectionteamworker[$_G['uid']]['lastvisit'];
			if($_G['collection']['lastupdate'] >= $lastvisit) {
				table_forum_collectionteamworker::t()->update_teamworker($ctid, $_G['uid'], ['lastvisit' => TIMESTAMP], true, true);
			}
		} elseif($collectionfollowdata['ctid']) {
			$lastvisit = $collectionfollowdata['lastvisit'];
			if($_G['collection']['lastupdate'] >= $lastvisit) {
				table_forum_collectionfollow::t()->update_collectionfollow($ctid, $_G['uid'], ['lastvisit' => TIMESTAMP], true, true);
			}
		} else {
			$lastvisit = null;
		}

		$collectiontids = table_forum_collectionthread::t()->fetch_all_by_ctid($_G['collection']['ctid'], $start, $tpp);
		$tids = array_keys($collectiontids);
		$threadlist = table_forum_thread::t()->fetch_all_by_tid($tids);
		...
		$userCollections = table_forum_collection::t()->fetch_all_by_uid($_G['collection']['uid'], 0, 5, $_G['collection']['ctid']);
	}

	if($_G['collection']['commentnum'] > 0) {
		$commentlist = table_forum_collectioncomment::t()->fetch_all_by_ctid($_G['collection']['ctid'], 0, 5);
	...
	$followers = table_forum_collectionfollow::t()->fetch_all_collectionfollow($ctid, true, 0, 6);
	...
	if($_G['collection']['commentnum'] > 0) {
		...
		$commentlist = table_forum_collectioncomment::t()->fetch_all_by_ctid($_G['collection']['ctid'], $start, $_G['setting']['postperpage']);
		foreach($commentlist as &$curvalue) {
			$curvalue['dateline'] = dgmdate($curvalue['dateline'], 'u', '9999', getglobal('setting/dateformat'));
			$curvalue['rateimg'] = imgdisplayrate($curvalue['rate']);
		}
	...
	$followers = table_forum_collectionfollow::t()->fetch_all_collectionfollow($ctid, true, $start, $cmemberperpage);
