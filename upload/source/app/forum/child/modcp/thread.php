...
$op = !in_array($op, ['thread', 'post']) ? 'thread' : $op;
	...
	if($_G['fid'] && $_G['forum']['ismoderator']) {
		...
		if($do == 'search' && submitcheck('submit', 1)) {
			...
			if(!empty($conditions)) {
				...
				$tids = $comma = '';
				$count = 0;
				foreach(table_forum_thread::t()->fetch_all_search($conditions, 0, 0, 1000, 'displayorder DESC, lastpost') as $thread) {
		...
		if(empty($do)) {
			...
			$total = table_forum_thread::t()->count_by_fid_typeid_displayorder($_G['fid'], $_GET['typeid'], 0, '>=');
			...
			if($total) {
				$start = ($page - 1) * $_G['tpp'];
				$threads = table_forum_thread::t()->fetch_all_by_fid_typeid_displayorder($_G['fid'], $_GET['typeid'], 0, '>=', $start, $_G['tpp']);
			}
			...
			if($result['fid'] == $_G['fid']) {
				...
				if($total) {
					$start = ($page - 1) * $_G['tpp'];
					$threads = table_forum_thread::t()->fetch_all_by_tid_fid_displayorder(explode(',', $result['tids']), null, null, 'lastpost', $start, $_G['tpp']);
				}
			}
		}
...
if($op == 'post') {
	...
	if($do == 'delete' && submitcheck('deletesubmit')) {
		...
		if($pids = dimplode($_GET['delete'])) {
			...
			foreach(table_forum_post::t()->fetch_all_post($posttableid, $_GET['delete'], false) as $post) {
		...
		if($pidsdelete) {
			...
			$forums = table_forum_forum::t()->fetch_all($prune['forums']);
		...
		showmessage('modcp_thread_delete_succeed', '', [], ['break' => 1]);
	}
	...
	if($do == 'list') {
		...
		if($result['fid'] == $_G['fid']) {
			...
			if($total && $result['pids']) {
				$start = ($page - 1) * $_G['tpp'];
				$tids = [];
				$postlist = table_forum_post::t()->fetch_all_by_pid($result['posttableid'], explode(',', $result['pids']), true, 'DESC', $start, $_G['tpp']);
