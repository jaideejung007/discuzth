...
$imglist = $albumpayaids = $attachmentlist = $freeattachids = [];
...
$validpids = table_forum_post::t()->fetch_all_pid_by_tid($_G['tid'], 0);
foreach(table_forum_attachment_n::t()->fetch_all_by_id('tid:'.$_G['tid'], 'tid', $_G['tid'], 'aid') as $attach) {
...
if($albumpayaids) {
	foreach(table_common_credit_log::t()->fetch_all_by_uid_operation_relatedid($_G['uid'], 'BAC', array_keys($albumpayaids)) as $creditlog) {
