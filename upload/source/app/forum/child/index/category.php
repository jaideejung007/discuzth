...
$gquery = table_forum_forum::t()->fetch_all_info_by_fids($gid);
$query = table_forum_forum::t()->fetch_all_info_by_fids(0, 1, 0, $gid, 1, 0, 0, 'forum');
...
foreach($query as $forum) {
	...
	if($forum['type'] != 'group') {
		...
		if(forum($forum)) {
			$forum['orderid'] = $catlist[$forum['fup']]['forumscount'] ?? 0;
...
$query = table_forum_forum::t()->fetch_all_subforum_by_fup($fids);
foreach($query as $forum) {
