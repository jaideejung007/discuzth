...
if($operation == 'members') {
	...
	require_once childfile('moderate/member');
	exit;
	...
	if($modfid && $modfid != '-1') {
		$fidadd = ['fids' => $modfid, 'and' => ' AND ', 't' => 't.', 'p' => 'p.'];
	} else {
		$fidadd = [];
	}
	...
	var cookiepre = "{$_G['config']['cookie']['cookiepre']}";
        function mod_fast_tid(obj, operation, value, fid, tid, page) {
            obj.href = '{$adminscript}?action=moderate&operation=' + operation +
             '&fast=1&fid=' + fid + '&tid=' + tid +
             '&moderate[' + tid + ']=' + value + '&page=' + page + '&frame=no&pm_'+tid+'='+$('pm_'+tid).value;
        }
        function mod_fast_pid(obj, operation, value, fid, tid, pid, page, posttableid) {
            obj.href = '{$adminscript}?action=moderate&operation=' + operation +
             '&fast=1&fid=' + fid + '&tid=' + tid + '&pid='+pid+
             '&moderate[' + pid + ']=' + value + '&page=' + page + '&posttableid='+posttableid+'&frame=no&pm_'+pid+'='+$('pm_'+pid).value;
        }
...
$submenu = [
	[['menu' => 'moderate_m_forum', 'submenu' => [
		'threads' => ['nav_moderate_threads', 'moderate&operation=threads', $operation == 'threads'],
		'replies' => ['nav_moderate_replies', 'moderate&operation=replies', $operation == 'replies'],
	]], in_array($operation, ['threads', 'replies'])],
	[['menu' => 'moderate_m_home', 'submenu' => [
		'blogs' => ['nav_moderate_blogs', 'moderate&operation=blogs', $operation == 'blogs'],
		'pictures' => ['nav_moderate_pictures', 'moderate&operation=pictures', $operation == 'pictures'],
		'doings' => ['nav_moderate_doings', 'moderate&operation=doings', $operation == 'doings'],
		'shares' => ['nav_moderate_shares', 'moderate&operation=shares', $operation == 'shares'],
		'comments' => ['nav_moderate_comments', 'moderate&operation=comments', $operation == 'comments'],
	]], in_array($operation, ['blogs', 'pictures', 'doings', 'shares', 'comments'])],
	[['menu' => 'moderate_m_portal', 'submenu' => [
		'articles' => ['nav_moderate_articles', 'moderate&operation=articles', $operation == 'articles'],
		'articlecomments' => ['nav_moderate_articlecomments', 'moderate&operation=articlecomments', $operation == 'articlecomments'],
		'topiccomments' => ['nav_moderate_topiccomments', 'moderate&operation=topiccomments', $operation == 'topiccomments'],
	]], in_array($operation, ['articles', 'articlecomments', 'topiccomments'])]
];

if($operation == 'threads') {

	require_once childfile('moderate/thread');

} elseif($operation == 'replies') {

	require_once childfile('moderate/reply');

} elseif($operation == 'blogs') {

	require_once childfile('moderate/blog');

} elseif($operation == 'pictures') {

	require_once childfile('moderate/picture');

} elseif($operation == 'doings') {

	require_once childfile('moderate/doing');

} elseif($operation == 'shares') {

	require_once childfile('moderate/share');

} elseif($operation == 'comments') {

	require_once childfile('moderate/comment');

} elseif($operation == 'articles') {

	require_once childfile('moderate/article');

} elseif($operation == 'articlecomments' || $operation == 'topiccomments') {

	require_once childfile('moderate/portalcomment');
...
function moderateswipe($type, $ids) {
	if($type == 'pid') {
		$exist_ids = array_keys(table_forum_post::t()->fetch_all_post(0, $ids));
	} elseif($type == 'tid') {
		$exist_ids = array_keys(table_forum_thread::t()->fetch_all($ids));
	}
	$remove_ids = array_diff($ids, $exist_ids);
	if($remove_ids) {
		return table_common_moderate::t()->delete_moderate($remove_ids, $type);
	} else {
		return 0;
	}
}
