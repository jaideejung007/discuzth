...
class optimizer_forumstatus {
	...
	public function check() {
		$forumlist = table_forum_forum::t()->fetch_all_by_status(0, 0);
		if(empty($forumlist)) {
			$return = ['status' => 0, 'type' => 'none', 'lang' => lang('optimizer', 'optimizer_forumstatus_no_need')];
		} else {
			$fids = [];
			foreach($forumlist as $forum) {
				$fids[] = $forum['fid'];
			}
			$forumfieldlist = table_forum_forumfield::t()->fetch_all_by_fid($fids);
			$forumdesc = '';
			...
			if(!$forumdesc) {
				...
				$return = ['status' => 1, 'type' => 'view', 'lang' => lang('optimizer', 'optimizer_forumstatus_need', ['forumdesc' => $forumdesc]), 'extraurl' => $extraurl];
			}
		}
		return $return;
	}
