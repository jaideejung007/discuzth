...
class optimizer_usergroup9 {
	...
	public function check() {
		$usergroup = table_common_usergroup::t()->fetch(9);
		$usergroupfield = table_common_usergroup_field::t()->fetch(9);
		if(!$usergroup['allowsendpm'] && !$usergroupfiled['allowposturl'] && !$usergroupfield['allowgroupposturl'] && !$usergroupfield['allowpost'] && !$usergroupfield['allowreply'] && !$usergroupfiled['allowdirectpost'] && !$usergroupfield['allowgroupdirectpost']) {
			$return = ['status' => 0, 'type' => 'none', 'lang' => lang('optimizer', 'optimizer_usergroup9_no_need')];
		} else {
			$option = [
			...
			$return = ['status' => 1, 'type' => 'header', 'lang' => lang('optimizer', 'optimizer_usergroup9_need', ['desc' => implode(',', $desc)])];
		}
		return $return;
	}
