...
class optimizer_loginpwcheck {
	...
	public function check() {
		...
		if(!$accountguard['loginpwcheck']) {
			$return = ['status' => 1, 'type' => 'header', 'lang' => lang('optimizer', 'optimizer_loginpwcheck_need')];
		} else {
			$return = ['status' => 0, 'type' => 'none', 'lang' => lang('optimizer', 'optimizer_loginpwcheck_no_need')];
		}
		return $return;
	}
