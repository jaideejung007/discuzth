...
class optimizer_recyclebin {
	...
	public function check() {
		$forumlist = table_forum_forum::t()->fetch_all_by_recyclebin();
		if(empty($forumlist)) {
			$return = ['status' => 0, 'type' => 'none', 'lang' => lang('optimizer', 'optimizer_recyclebin_no_need')];
		} else {
			$forumdesc = '';
			...
			$return = ['status' => 1, 'type' => 'view', 'lang' => lang('optimizer', 'optimizer_recyclebin_need', ['forumdesc' => $forumdesc]), 'extraurl' => $extraurl];
		}
		return $return;
	}
