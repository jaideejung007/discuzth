...
list($_GET['key'], $_GET['type'], $_GET['rid']) = explode('.', $addonids[$addoni] ?? $addonids[0]);
if($step == 0) {
	cpmsg('cloudaddons_downloading', "action=cloudaddons&operation=download&addonids={$_GET['addonids']}&i=$addoni&step=1&md5hash=".$_GET['md5hash'].'&timestamp='.$_GET['timestamp'], 'loading', ['addonid' => $_GET['key'].'.'.$_GET['type']], '<div>0%</div>', FALSE);
} elseif($step == 1) {
	$packnum = $_GET['num'] ?? 0;
	$tmpdir = DISCUZ_DATA.'./download/'.$_GET['rid'];
	$end = '';
	$md5tmp = DISCUZ_DATA.'./download/'.$_GET['rid'].'.md5';
	if($packnum) {
	...
	if(empty($data)) {
		cpmsg('cloudaddons_downloading', "action=cloudaddons&operation=download&addonids={$_GET['addonids']}&i=$addoni&step=1&md5hash=".$_GET['md5hash'].'&timestamp='.$_GET['timestamp'].'&num='.$packnum, 'loading', ['addonid' => $_GET['key'].'.'.$_GET['type']], '<div></div>', FALSE);
		exit;
	}
	$_GET['importtxt'] = $data;
	...
	if(!$array['Status']) {
		...
		if($array['type'] != $_GET['type'] || $array['key'] != $_GET['key'] || !$array['files']) {
			...
			cpmsg('cloudaddons_download_error', '', 'error', ['ErrorCode' => 100]);
		}
		...
		if(file_put_contents($md5tmp, serialize([$md5total, $md5s])) === false) {
			dir_clear($tmpdir);
			@unlink($md5tmp);
			cloudaddons_faillog($_GET['rid'], 101);
		...
		cpmsg('cloudaddons_install_error', '', 'error', ['ErrorCode' => $array['ErrorCode']]);
	} else {
	...
	if(!$end) {
		$packnum++;
		cpmsg('cloudaddons_downloading', "action=cloudaddons&operation=download&addonids={$_GET['addonids']}&i=$addoni&step=1&md5hash=".$_GET['md5hash'].'&timestamp='.$_GET['timestamp'].'&num='.$packnum, 'loading', ['addonid' => $_GET['key'].'.'.$_GET['type']], '<div>'.$percent.'%</div>', FALSE);
	} else {
	...
	if(!file_exists($tmpdir)) {
		...
		cpmsg('cloudaddons_download_error', '', 'error', ['ErrorCode' => 103]);
	}
	$typedir = [
		'plugin' => PLUGIN_ROOT,
	...
	if(!$typedir[$_GET['type']]) {
		...
		cpmsg('cloudaddons_download_error', '', 'error', ['ErrorCode' => 104]);
	}
	if($_GET['type'] != 'pack') {
	...
	if($unwriteabledirs) {
		if(!submitcheck('settingsubmit')) {
			...
			showtips(cplang('cloudaddons_unwriteabledirs', ['basedir' => $typedir[$_GET['type']] != './' ? $typedir[$_GET['type']] : '/', 'unwriteabledirs' => implode(', ', $unwriteabledirs)]));
	...
	foreach($addonids as $addonid) {
		...
		if(empty($batch[$key.'.'.$type][1])) {
			if($type == 'plugin') {
				...
				$plugindir = DISCUZ_PLUGIN($key);
				if($f = getimportfilename($plugindir.'/discuz_plugin_'.$key.$extra)) {
					$importtxt = @implode('', file($f));
				} elseif($f = getimportfilename($plugindir.'/discuz_plugin_'.$key)) {
					$importtxt = @implode('', file($f));
				}
				if(!empty($importtxt)) {
				...
				$plugin = table_common_plugin::t()->fetch_by_identifier($key);
				if(!$plugin['pluginid']) {
					$batch[$key.'.'.$type] = [
				...
				$pluginarray = [];
				$entrytitle = $importtxt = '';
				$templatedir = DISCUZ_TEMPLATE('./template/'.$key);
				$searchdir = dir($templatedir);
				while($searchentry = $searchdir->read()) {
					if(str_starts_with($searchentry, 'discuz_style_') &&
						(fileext($searchentry) == 'xml' || fileext($searchentry) == 'json')) {
				...
				if(file_exists(DISCUZ_DATA.'./addonpack/'.$key.'.php')) {
					$batch[$key.'.'.$type] = [
