...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
	...
	$pagetmp = $page;
	$sqlwhere = '';
	$modcount = table_common_moderate::t()->fetch_all_for_article($moderatestatus, $_GET['catid'], $_GET['username'], $dateline, 1);
	do {
		$start_limit = ($pagetmp - 1) * $tpp;
		$query = table_common_moderate::t()->fetch_all_for_article($moderatestatus, $_GET['catid'], $_GET['username'], $dateline, 0, $start_limit, $tpp);
		$pagetmp = $pagetmp - 1;
	...
	foreach($query as $article) {
		...
		showtablerow("id=\"mod_{$article['aid']}_row1\"", ["id=\"mod_{$article['aid']}_row1_op\" rowspan=\"3\" class=\"rowform threadopt\" style=\"width:80px;\"", '', 'width="120"', 'width="55"'], [
	...
	if($validate_aids = dimplode($moderation['validate'])) {
		$validates = table_portal_article_title::t()->update($moderation['validate'], ['status' => '0']);
	...
	if($ignore_aids = dimplode($moderation['ignore'])) {
		$ignores = table_portal_article_title::t()->update($moderation['ignore'], ['status' => '2']);
	...
	if($_GET['fast']) {
		...
		cpmsg('moderate_articles_succeed', "action=moderate&operation=articles&page=$page&filter=$filter&catid={$_GET['catid']}&dateline={$_GET['dateline']}&username={$_GET['username']}&keyword={$_GET['keyword']}&idtype={$_GET['idtype']}&tpp={$_GET['tpp']}&showcensor=$showcensor", 'succeed', ['validates' => $validates, 'ignores' => $ignores, 'deletes' => $deletes]);
	}
