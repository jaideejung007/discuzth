...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
	...
	$pagetmp = $page;
	$modcount = table_common_moderate::t()->count_by_search_for_blog($moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['title']);
	do {
		$start_limit = ($pagetmp - 1) * $tpp;
		$blogarr = table_common_moderate::t()->fetch_all_by_search_for_blog($moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['title'], $start_limit, $tpp);
		$pagetmp = $pagetmp - 1;
	...
	foreach($blogarr as $blog) {
		...
		showtablerow("id=\"mod_{$blog['blogid']}_row1\"", ["id=\"mod_{$blog['blogid']}_row1_op\" rowspan=\"3\" class=\"rowform threadopt\" style=\"width:80px;\"", '', 'width="120"', 'width="120"', 'width="55"'], [
	...
	if($moderate['validate']) {
		$validates = table_home_blog::t()->update($moderate['validate'], ['status' => '0']);
		$query_t = table_home_blog::t()->count_uid_by_blogid($moderate['validate']);
		foreach($query_t as $blog_user) {
			$credit_times = $blog_user['count'];
			updatecreditbyaction('publishblog', $blog_user['uid'], ['blogs' => 1], '', $credit_times);
		}
	...
	if($moderate['ignore']) {
		$ignores = table_home_blog::t()->update($moderate['ignore'], ['status' => '2']);
	...
	if($_GET['fast']) {
		...
		cpmsg('moderate_blogs_succeed', "action=moderate&operation=blogs&page=$page&filter=$filter&dateline={$_GET['dateline']}&username={$_GET['username']}&title={$_GET['title']}&tpp={$_GET['tpp']}&showcensor=$showcensor", 'succeed', ['validates' => $validates, 'ignores' => $ignores, 'recycles' => $recycles, 'deletes' => $deletes]);
	}
