...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
	...
	$pagetmp = $page;
	$modcount = table_common_moderate::t()->count_by_search_for_pic($moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['title']);
	do {
		$start_limit = ($pagetmp - 1) * $tpp;
		$picarr = table_common_moderate::t()->fetch_all_by_search_for_pic($moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['title'], $start_limit, $tpp);
		$pagetmp = $pagetmp - 1;
	...
	foreach($picarr as $pic) {
		...
		showtablerow("id=\"mod_{$pic['picid']}_row1\"", ["id=\"mod_{$pic['picid']}_row1_op\" rowspan=\"3\" class=\"rowform threadopt\" style=\"width:80px;\"", '', 'width="120"', 'width="120"', 'width="55"'], [
	...
	if($moderation['validate']) {
		$validates = table_home_pic::t()->update($moderation['validate'], ['status' => '0']);
		$albumincrease = [];
		foreach(table_home_pic::t()->fetch_all($moderation['validate']) as $pics) {
			$albumincrease[$pics['albumid']]++;
		}
		foreach($albumincrease as $albumid => $albuminc) {
			table_home_album::t()->update_num_by_albumid($albumid, $albuminc);
		}
	...
	if($moderation['ignore']) {
		$ignores = table_home_pic::t()->update($moderation['ignore'], ['status' => '2']);
	...
	if($_GET['fast']) {
		...
		cpmsg('moderate_pictures_succeed', "action=moderate&operation=pictures&page=$page&filter=$filter&dateline={$_GET['dateline']}&username={$_GET['username']}&title={$_GET['title']}&tpp={$_GET['tpp']}&showcensor=$showcensor", 'succeed', ['validates' => $validates, 'ignores' => $ignores, 'recycles' => $recycles, 'deletes' => $deletes]);
	}
