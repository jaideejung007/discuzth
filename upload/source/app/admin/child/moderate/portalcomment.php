...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
	...
	if($operation == 'articlecomments') {
		showtablerow('', ['width="100"', 'width="200"', 'width="100"', 'width="200"', 'width="60"'],
			[
	...
	showtablerow('', $operation == 'articlecomments' ?
		['width="100"', 'width="200"', 'width="100"', 'colspan="3"'] :
	...
	$pagetmp = $page;
	$modcount = table_common_moderate::t()->fetch_all_for_portalcomment($idtype, $tablename, $moderatestatus, $_GET['catid'], $_GET['username'], $dateline, 1, $_GET['keyword']);
	do {
		$start_limit = ($pagetmp - 1) * $tpp;
		$query = table_common_moderate::t()->fetch_all_for_portalcomment($idtype, $tablename, $moderatestatus, $_GET['catid'], $_GET['username'], $dateline, 0, $_GET['keyword'], $start_limit, $tpp);
		$pagetmp = $pagetmp - 1;
	...
	foreach($query as $articlecomment) {
		...
		showtablerow("id=\"mod_{$articlecomment['cid']}_row1\"", ["id=\"mod_{$articlecomment['cid']}_row1_op\" rowspan=\"3\" class=\"rowform threadopt\" style=\"width:80px;\"", '', 'width="120"', 'width="55"'], [
	...
	if($moderation['validate']) {
		$validates = table_portal_comment::t()->update($moderation['validate'], ['status' => '0']);
		updatemoderate($idtype.'_cid', $moderation['validate'], 2);
	}
	if($moderation['delete']) {
		$validates = table_portal_comment::t()->delete($moderation['delete']);
		updatemoderate($idtype.'_cid', $moderation['delete'], 2);
	}
	if($moderation['ignore']) {
		$validates = table_portal_comment::t()->update($ignore_cids, ['status' => '2']);
	...
	if($_GET['fast']) {
		...
		cpmsg('moderate_'.$operation.'_succeed', "action=moderate&operation=$operation&page=$page&filter=$filter&dateline={$_GET['dateline']}&username={$_GET['username']}&keyword={$_GET['keyword']}&catid={$_GET['catid']}&tpp={$_GET['tpp']}&showcensor=$showcensor", 'succeed', ['validates' => $validates, 'ignores' => $ignores, 'deletes' => $deletes]);
	}
