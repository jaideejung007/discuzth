...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
	...
	$pagetmp = $page;
	$modcount = table_common_moderate::t()->count_by_search_for_doing($moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['keyword']);
	do {
		$start_limit = ($pagetmp - 1) * $tpp;
		$doingarr = table_common_moderate::t()->fetch_all_by_search_for_doing($moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['keyword'], $start_limit, $tpp);
		$pagetmp = $pagetmp - 1;
	...
	foreach($doingarr as $doing) {
		...
		showtablerow("id=\"mod_{$doing['doid']}_row1\"", ["id=\"mod_{$doing['doid']}_row1_op\" rowspan=\"3\" class=\"rowform threadopt\" style=\"width:80px;\"", '', 'width="120"'], [
	...
	if($moderation['validate']) {
		table_home_doing::t()->update($moderation['validate'], ['status' => '0']);
		$query_t = table_home_doing::t()->fetch_all($moderation['validate']);
		if(helper_access::check_module('feed')) {
			foreach($query_t as $doing) {
				$feedarr = [
					...
					'title_data' => serialize(['message' => $doing['message']]),
					'body_template' => '',
					'body_data' => '',
					'id' => $doing['doid'],
					'idtype' => 'doid'
				];
				$validates += table_home_feed::t()->insert($feedarr);
			}
		}
	...
	if($moderation['ignore']) {
		$ignores = table_home_doing::t()->update($moderation['ignore'], ['status' => '2']);
	...
	if($_GET['fast']) {
		...
		cpmsg('moderate_doings_succeed', "action=moderate&operation=doings&page=$page&filter=$filter&dateline={$_GET['dateline']}&username={$_GET['username']}&keyword={$_GET['keyword']}&tpp={$_GET['tpp']}&showcensor=$showcensor", 'succeed', ['validates' => $validates, 'ignores' => $ignores, 'deletes' => $deletes]);
	}
