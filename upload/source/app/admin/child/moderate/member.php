...
if($do == 'mod') {
	...
	if(!submitcheck('modsubmit')) {
		...
		$validatenum = table_common_member_validate::t()->count_by_status(0);
		$members = '';
		if($validatenum) {
			...
			$allvalidate = table_common_member_validate::t()->fetch_all_invalidate($start_limit, $_G['setting']['memberperpage']);
			$uids = array_keys($allvalidate);
			$allmember = table_common_member::t()->fetch_all($uids, false, 0);
			$allmemberstatus = table_common_member_status::t()->fetch_all($uids, false, 0);
			$allmemberprofile = table_common_member_profile::t()->fetch_all($uids, false, 0);
			foreach($allvalidate as $uid => $member) {
				...
				$fields = !empty($member['field']) ? dunserialize($member['field']) : [];
				$str = '';
				foreach($_G['cache']['fields_register'] as $field) {
					if(!$field['available'] || in_array($field['fieldid'], ['uid', 'constellation', 'zodiac', 'birthmonth', 'birthyear', 'birthcountry', 'birthprovince', 'birthdist', 'birthcommunity', 'residecountry', 'resideprovince', 'residedist', 'residecommunity'])) {
						continue;
					}
			...
			if(!empty($vuids)) {
				table_common_member_validate::t()->delete($vuids);
			}
		}
		shownav('user', 'nav_modmembers');
		showsubmenu('nav_moderate_users', [
			['nav_moderate_users_mod', 'moderate&operation=members&do=mod', 1],
			['clean', 'moderate&operation=members&do=del', 0]
		]);
...
EOT;
		...
		showsubtitle(['operation', 'members_edit_info', 'moderate_members_message', 'moderate_members_info', 'moderate_members_remark']);
		echo $members;
		...
		$moderation = ['invalidate' => [], 'validate' => [], 'delete' => [], 'ignore' => []];
		...
		$allmembervalidate = $uids ? table_common_member_validate::t()->fetch_all($uids) : table_common_member_validate::t()->range();
		foreach(table_common_member::t()->fetch_all(array_keys($allmembervalidate), false, 0) as $uid => $member) {
		...
		if(!empty($members)) {
			...
			if(!empty($moderation['validate']) && is_array($moderation['validate'])) {
				...
				table_common_member::t()->update($validateuids, ['adminid' => 0, 'groupid' => $_G['setting']['newusergroupid'], 'freeze' => 0]);
				$numvalidated = count($validateuids);
				table_common_member_validate::t()->delete($validateuids);
			} else {
				$moderation['validate'] = [];
			}
			...
			if(!empty($moderation['invalidate']) && is_array($moderation['invalidate'])) {
				...
				foreach($invalidateuids as $uid) {
					table_common_member_validate::t()->update($uid, ['moddate' => $_G['timestamp'], 'admin' => $_G['username'], 'status' => '1', 'remark' => dhtmlspecialchars($_GET['remark'][$uid])]);
				}
			} else {
				$moderation['invalidate'] = [];
			}

			foreach(['validate', 'invalidate'] as $o) {
				foreach($moderation[$o] as $uid) {
					if($_GET['remark'][$uid]) {
						switch($o) {
							case 'validate':
								notification_add($uid, 'mod_member', 'member_moderate_validate', ['remark' => $_GET['remark'][$uid]]);
								break;
							case 'invalidate':
								notification_add($uid, 'mod_member', 'member_moderate_invalidate', ['remark' => $_GET['remark'][$uid]]);
								break;
						}
					} else {
						switch($o) {
			...
			if($_GET['sendemail']) {
				...
				foreach(['delete', 'validate', 'invalidate'] as $o) {
					foreach($moderation[$o] as $uid) {
		...
		cpmsg('moderate_members_op_succeed', "action=moderate&operation=members&page=$page", 'succeed', ['numvalidated' => $numvalidated, 'numinvalidated' => $numinvalidated, 'numdeleted' => $numdeleted]);
	...
	if(!submitcheck('prunesubmit', 1)) {
		...
		shownav('user', 'nav_modmembers');
		showsubmenu('nav_moderate_users', [
			['nav_moderate_users_mod', 'moderate&operation=members&do=mod', 0],
			['clean', 'moderate&operation=members&do=del', 1]
		]);
		showtips('moderate_members_tips');
		...
		$uids = table_common_member_validate::t()->fetch_all_validate_uid($_GET['submitmore'], $_GET['regbefore'], $_GET['modbefore'], $_GET['regip']);
		if((!$membernum = count($uids))) {
			cpmsg('members_search_noresults', '', 'error');
		} elseif(!$_GET['confirmed']) {
			cpmsg('members_delete_confirm', 'action=moderate&operation=members&do=del&submitmore='.rawurlencode($_GET['submitmore']).'&regbefore='.rawurlencode($_GET['regbefore']).'&modbefore='.rawurlencode($_GET['modbefore']).'&regip='.rawurlencode($_GET['regip']).'&prunesubmit=yes', 'form', ['membernum' => $membernum]);
		} else {
			$numdeleted = table_common_member::t()->delete_no_validate(array_keys($uids));
