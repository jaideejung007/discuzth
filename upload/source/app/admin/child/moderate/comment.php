...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
	...
	foreach(['uid', 'blogid', 'picid', 'sid'] as $v) {
		$selected = '';
		if($_GET['idtype'] == $v) {
			$selected = 'selected="selected"';
		}
	...
	$pagetmp = $page;
	$modcount = table_common_moderate::t()->count_by_search_for_commnet($_GET['idtype'], $moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['keyword']);
	do {
		$start_limit = ($pagetmp - 1) * $tpp;
		$commentarr = table_common_moderate::t()->fetch_all_by_search_for_comment($_GET['idtype'], $moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['keyword'], $start_limit, $tpp);
		$pagetmp = $pagetmp - 1;
	...
	foreach($commentarr as $comment) {
		...
		showtablerow("id=\"mod_{$comment['cid']}_row1\"", ["id=\"mod_{$comment['cid']}_row1_op\" rowspan=\"3\" class=\"rowform threadopt\" style=\"width:80px;\"", '', 'width="120"', 'width="120"', 'width="55"', 'width="55"'], [
	...
	if($moderation['validate']) {
		$validates = table_home_comment::t()->update_comment($moderation['validate'], ['status' => '0']);
	}
	...
	if($moderation['ignore']) {
		$ignores = table_home_comment::t()->update_comment($moderation['ignore'], ['status' => '2']);
	}
	...
	if($_GET['fast']) {
		...
		cpmsg('moderate_comments_succeed', "action=moderate&operation=comments&page=$page&filter=$filter&dateline={$_GET['dateline']}&username={$_GET['username']}&keyword={$_GET['keyword']}&idtype={$_GET['idtype']}&tpp={$_GET['tpp']}&showcensor=$showcensor", 'succeed', ['validates' => $validates, 'ignores' => $ignores, 'deletes' => $deletes]);
	}
