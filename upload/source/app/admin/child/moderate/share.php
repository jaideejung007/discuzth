...
if(!submitcheck('modsubmit') && !$_GET['fast']) {
	...
	if(!empty($_GET['keyword'])) {
		$keyword = str_replace(['%', '_'], ['\%', '\_'], $_GET['keyword']);
		$sqlwhere .= " AND s.body_general LIKE '%$keyword%'";
	}
	$modcount = table_common_moderate::t()->count_by_search_for_share($moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['keyword']);
	do {
		$start_limit = ($pagetmp - 1) * $tpp;
		$sharearr = table_common_moderate::t()->fetch_all_by_search_for_share($moderatestatus, $_GET['username'], (($dateline && $dateline != 'all') ? (TIMESTAMP - $dateline) : null), $_GET['keyword'], $start_limit, $tpp);
		$pagetmp = $pagetmp - 1;
	...
	foreach($sharearr as $share) {
		...
		showtablerow("id=\"mod_{$share['sid']}_row1\"", ["id=\"mod_{$share['sid']}_row1_op\" rowspan=\"3\" class=\"rowform threadopt\" style=\"width:80px;\"", '', 'width="120"', 'width="120"', 'width="55"', 'width="55"'], [
	...
	if(!empty($moderation['validate'])) {
		...
		$validates = table_home_share::t()->update($moderation['validate'], ['status' => 0]);
		foreach(table_home_share::t()->fetch_all($moderation['validate']) as $share) {
			switch($share['type']) {
				case 'thread':
	...
	if($ignore_sids = dimplode($moderation['ignore'])) {
		$ignores = table_home_share::t()->update($moderation['ignore'], ['status' => 2]);
	...
	if($_GET['fast']) {
		...
		cpmsg('moderate_shares_succeed', "action=moderate&operation=shares&page=$page&filter=$filter&dateline={$_GET['dateline']}&username={$_GET['username']}&keyword={$_GET['keyword']}&tpp={$_GET['tpp']}&showcensor=$showcensor", 'succeed', ['validates' => $validates, 'ignores' => $ignores, 'deletes' => $deletes]);
	}
