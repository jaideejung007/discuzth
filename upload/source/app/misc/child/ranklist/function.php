...
function getranklist_thread($num = 20, $view = 'replies', $orderby = 'all') {
	...
	$data = [];
	$rank = 0;
	$notfid = $_G['setting']['ranklist']['ignorefid'] ? explode(',', $_G['setting']['ranklist']['ignorefid']) : [];
	foreach(table_forum_thread::t()->fetch_all_rank_thread($timestamp, $notfid, $view, $num) as $thread) {
		++$rank;
		$thread['rank'] = $rank;
...
function getranklist_activity($num = 20, $view = 'heats', $orderby = 'all') {
	...
	$attachtables = [];
	$notfid = $_G['setting']['ranklist']['ignorefid'] ? explode(',', $_G['setting']['ranklist']['ignorefid']) : [];
	foreach(table_forum_thread::t()->fetch_all_rank_activity($timestamp, $notfid, $view, $num) as $thread) {
		++$rank;
		$thread['rank'] = $rank;
...
function getranklist_picture($num = 20, $view = 'hot', $orderby = 'all') {
	...
	$data = [];
	$query = table_home_pic::t()->fetch_all_by_sql($dateline, 'p.'.DB::order($view, 'DESC'), 0, $num);

	require_once libfile('function/home');
	$rank = 0;
	foreach($query as $value) {
		++$rank;
		$picture = ['picid' => $value['picid'], 'uid' => $value['uid'], 'username' => $value['username'], 'title' => $value['title'], 'filepath' => $value['filepath'], 'thumb' => $value['thumb'], 'remote' => $value['remote'], 'hot' => $value['hot'], 'sharetimes' => $value['sharetimes'], 'click1' => $value['click1'], 'click2' => $value['click2'], 'click3' => $value['click3'], 'click4' => $value['click4'], 'click5' => $value['click5'], 'click6' => $value['click6'], 'click7' => $value['click7'], 'click8' => $value['click8'], 'albumid' => $value['albumid'], 'albumname' => $value['albumname'], 'friend' => $value['friend']];
		$picture['rank'] = $rank;
...
function getranklist_pictures_index($num = 20, $dateline = 0, $orderby = 'hot DESC') {
	$picturelist = [];
	$query = table_home_pic::t()->fetch_all_by_sql('p.hot>3', 'p.dateline DESC', 0, $num);
	require_once libfile('function/home');
	foreach($query as $value) {
		$picture = ['picid' => $value['picid'], 'uid' => $value['uid'], 'username' => $value['username'], 'title' => $value['title'], 'filepath' => $value['filepath'], 'thumb' => $value['thumb'], 'remote' => $value['remote'], 'albumid' => $value['albumid'], 'albumname' => $value['albumname'], 'friend' => $value['friend']];
		$picture['url'] = $picture['friend'] == 0 ? pic_get($picture['filepath'], 'album', $picture['thumb'], $picture['remote']) : STATICURL.'image/common/nopublish.svg';;
...
function getranklist_girls($offset = 0, $limit = 20, $orderby = 'ORDER BY s.unitprice DESC, s.credit DESC') {
	return table_common_member::t()->fetch_all_girls_for_ranklist($offset, $limit, $orderby);
}
...
function getranklist_blog($num = 20, $view = 'hot', $orderby = 'all') {
	...
	$data = [];
	$data_blog = table_home_blog::t()->range_blog(0, $num, 'DESC', $view, 0, 0, null, $timestamp);
...
function getranklist_forum($num = 20, $view = 'threads') {
	...
	$query = table_forum_forum::t()->fetch_all_for_ranklist(1, 0, $key, 0, $num, explode(',', $_G['setting']['ranklist']['ignorefid']));
	$i = 1;
	foreach($query as $row) {
		$result = ['fid' => $row['fid'], 'name' => $row['name']];
...
function getranklist_group($num = 20, $view = 'threads') {
	...
	$query = table_forum_forum::t()->fetch_all_for_ranklist(3, 'sub', $key, 0, $num, explode(',', $_G['setting']['ranklist']['ignorefid']));
	$i = 1;
	foreach($query as $row) {
		$result = ['fid' => $row['fid'], 'name' => $row['name']];
...
function getranklist_member_credit($num, $orderby) {
	return table_common_member::t()->fetch_all_order_by_credit_for_ranklist($num, $orderby);
}

function getranklist_member_friendnum($num) {
	return table_common_member::t()->fetch_all_order_by_friendnum_for_ranklist($num);
...
function getranklist_member_invite($num, $orderby) {
	...
	$invite = $invitearray = $inviteuidarray = $invitefieldarray = [];
	foreach(table_common_invite::t()->fetch_all_invitenum_group_by_uid($dateline) as $result) {
		$invitearray[] = $result;
		$inviteuidarray[] = $result['uid'];
	}

	$invitememberfield = table_common_member::t()->fetch_all($inviteuidarray, false, 0);
	if($invitearray) {
...
function getranklist_member_onlinetime($num, $orderby) {
	...
	$onlinetime = $onlinetimearray = $onlinetimeuidarray = $onlinetimefieldarray = [];

	$onlinetimearray = table_common_onlinetime::t()->range_by_field(0, $num, $orderby, 'DESC');
	$onlinetimeuidarray = array_keys($onlinetimearray);
	$onlinetimefieldarray = table_common_member::t()->fetch_all($onlinetimeuidarray, false, 0);
	if($onlinetimearray) {
...
function getranklist_member_gender($gender, $num = 20) {
	...
	if($uids) {
		foreach(table_common_member::t()->fetch_all($uids, false, 0) as $uid => $value) {
...
function getranklist_member_post($num, $orderby) {
	...
	if($timelimit) {
		$posts = table_forum_post::t()->fetch_all_top_post_author(0, $timelimit, $num);
