...
if($_GET['view'] == 'credit') {
	...
	if($_G['uid']) {
		...
		if($_G['cookie'][$cookie_name]) {
			...
			if($now_choose == 'all') {
				$now_pos = table_common_member::t()->count_by_credits($mycredits);
			} else {
				$now_pos = table_common_member_count::t()->count_by_extcredits($now_choose, $mycredits);
			}
			$now_pos++;
	...
	if($_G['uid']) {
		...
		if($_G['cookie'][$cookie_name]) {
			...
			$now_pos = table_common_member_count::t()->count_by_friends($space['friends']);
			$now_pos++;
			dsetcookie($cookie_name, $now_pos);
		}
	} else {
	...
	$extcredits = $_G['setting']['extcredits'];
	$count = table_home_show::t()->count_by_credit();
	$space = [];
	if($count) {
		$space = $_G['member'];
		space_merge($space, 'count');
		...
		$myshowinfo = table_home_show::t()->fetch_by_uid_credit($space['uid']); 
		$myallcredit = intval($myshowinfo['credit']);
		$space['unitprice'] = intval($myshowinfo['unitprice']);
		$now_pos = table_home_show::t()->count_by_credit($space['unitprice']);

		$deluser = false;
		$query = table_home_show::t()->fetch_all_by_unitprice($start, $perpage);
		...
		if($deluser) {
			table_home_show::t()->delete_by_credit(1);
		}
...
$myfuids = [];
$query = table_home_friend::t()->fetch_all($_G['uid']);
foreach($query as $value) {
