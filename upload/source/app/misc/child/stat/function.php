...
function getstatvars_basic() {
	...
	$statvars = [];
	$statvars['members'] = table_common_member::t()->count();
	$members_runtime = table_common_member::t()->fetch_runtime();
	$statvars['membersaddavg'] = $members_runtime > 0.00001 ? (round($statvars['members'] / $members_runtime)) : 0;
	$statvars['memnonpost'] = table_common_member_count::t()->count_by_posts(0);
	$statvars['mempost'] = $statvars['members'] - $statvars['memnonpost'];
	$statvars['admins'] = table_common_member::t()->count_admins();
	$statvars['lastmember'] = table_common_member::t()->count_by_regdate(TIMESTAMP - 86400);
	...
	$bestmember = $bestmember[0] ?? ['username' => ''];
	$bestmember['author'] = $bestmember['username'];
	$statvars['bestmem'] = $bestmember['author'] ?? null;
	$statvars['bestmemposts'] = $bestmember['posts'] ?? null;
	$postsinfo = table_forum_post::t()->fetch_posts(0);
	...
	$statvars['forums'] = table_forum_forum::t()->fetch_all_fids(0, 'forum', 0, 0, 0, 1);

	$hotforum = table_forum_forum::t()->fetch_all_for_ranklist(1, '', 'posts', 0, 1);
	$statvars['hotforum'] = ['posts' => $hotforum[0]['posts'], 'threads' => $hotforum[0]['threads'], 'fid' => $hotforum[0]['fid'], 'name' => $hotforum[0]['name']];

	$statvars['threads'] = table_forum_thread::t()->count_all_thread();

	$statvars['postsaddtoday'] = table_forum_post::t()->count_by_dateline(0, TIMESTAMP - 86400);
...
function getstatvars_trade() {
	global $_G;
	$statvars = [];
	$query = table_forum_trade::t()->fetch_all_statvars('tradesum');
	foreach($query as $data) {
		$tradesums[] = $data;
	}
	$statvars['tradesums'] = $tradesums;
	$query = table_forum_trade::t()->fetch_all_statvars('credittradesum');
	foreach($query as $data) {
		$credittradesums[] = $data;
	}
	$statvars['credittradesums'] = $credittradesums;
	$query = table_forum_trade::t()->fetch_all_statvars('totalitems');
	foreach($query as $data) {
		$totalitems[] = $data;
	}
...
function getstatvars_team() {
	...
	$forums = $moderators = $members = $fuptemp = [];
	$categories = [0 => ['fid' => 0, 'fup' => 0, 'type' => 'group', 'name' => $_G['setting']['bbname']]];

	$uids = [];
	foreach(table_forum_moderator::t()->fetch_all_no_inherited() as $moderator) {
	...
	$admins = [];
	$members = table_common_member::t()->fetch_all($uids) + table_common_member::t()->fetch_all_by_adminid([1, 2]);
	$uids = array_keys($members);
	$onlinetime = $_G['setting']['oltimespan'] ? table_common_onlinetime::t()->fetch_all($uids) : [];
	$member_status = table_common_member_status::t()->fetch_all($uids);
	$member_count = table_common_member_count::t()->fetch_all($uids);
	...
	foreach(table_forum_post::t()->fetch_all_author_posts_by_dateline(0, $uids, $_G['timestamp'] - 86400 * 30) as $post) {
	...
	if($_G['setting']['modworkstatus']) {
		...
		foreach(table_forum_modwork::t()->fetch_all_user_count_by_dateline($starttime) as $member) {
			$members[$member['uid']]['modactions'] = $member['actioncount'];
			$totalmodactions += $member['actioncount'];
		}
	}

	$query = table_forum_forum::t()->fetch_all_by_status(1, 1);
	foreach($query as $val) {
		$forum = ['fid' => $val['fid'], 'fup' => $val['fup'], 'type' => $val['type'], 'name' => $val['name'], 'inheritedmod' => $val['inheritedmod']];
		$moderators[$forum['fid']] = (isset($moderators[$forum['fid']]) && is_array($moderators[$forum['fid']])) ? $moderators[$forum['fid']] : [];
...
function getstatvars_modworks() {
	...
	[$now['year'], $now['month'], $now['day']] = explode('-', dgmdate(TIMESTAMP, 'Y-n-j'));
	...
	$mergeactions = ['OPN' => 'CLS', 'ECL' => 'CLS', 'UEC' => 'CLS', 'EOP' => 'CLS', 'UEO' => 'CLS',
	...
	if($uid) {
		...
		if(empty($member) || !is_array($member) || in_array($_G['adminid'], [0, -1])) {
			showmessage('member_not_found');
		}

		$modactions = $totalactions = [];
		...
		while($starttime_dateline <= $endtime_dateline) {
			$modactions[dgmdate($starttime_dateline, 'Y-m-d')] = [];
			$starttime_dateline += 86400;
		}

		foreach(table_forum_modwork::t()->fetch_all_by_uid_dateline($uid, $starttime, $endtime) as $data) {
		...
		$members = table_common_member::t()->fetch_all_by_adminid([1, 2, 3]);


		foreach(table_forum_modwork::t()->fetch_all_user_count_posts_by_uid_dateline(array_keys($members), $starttime, $endtime) as $data) {
			if(isset($mergeactions[$data['modaction']])) {
				$data['modaction'] = $mergeactions[$data['modaction']];
			}
		...
		if(!empty($before)) {
			table_forum_modwork::t()->delete_by_dateline($expiretime.'-01');
		} else {
...
function getstatvars_memberlist() {
	...
	$statvars['memberlist'] = table_common_member::t()->fetch_all_stat_memberlist($srchmem, $_GET['order'], $_GET['asc'] ? 'ASC' : 'DESC', $start_limit, $_G['setting']['memberperpage']);
	$num = !empty($srchmem) ? table_common_member::t()->count_by_like_username($srchmem) : table_common_member::t()->count();
...
function getstatvars_forumstat($fid) {
	...
	if(!$fid) {
		$query = table_forum_forum::t()->fetch_all_fids();
		$forums = [];
		foreach($query as $val) {
			$forums[] = ['fid' => $val['fid'], 'type' => $val['type'], 'name' => $val['name'], 'posts' => $val['posts']];
		}
		$statvars['forums'] = $forums;
	} else {
		$foruminfo = table_forum_forum::t()->fetch($fid);
		$statvars['foruminfo'] = ['fid' => $foruminfo['fid'], 'name' => $foruminfo['name'], 'posts' => $foruminfo['posts'], 'threads' => $foruminfo['threads'], 'todayposts' => $foruminfo['todayposts']];
		...
		$mindate = table_forum_statlog::t()->fetch_min_logdate_by_fid($fid);
		if(!$mindate) {
			return $statvars;
		}
		[$minyear, $minmonth, $minday] = explode('-', $mindate);
		...
		$monthposts = [];
		foreach(table_forum_statlog::t()->fetch_all_by_fid_type($fid) as $data) {
			[$year, $month, $day] = explode('-', $data['logdate']);
