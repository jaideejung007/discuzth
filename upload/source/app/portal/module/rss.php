...
if($catarray) {
	$alldata = table_portal_rsscache::t()->fetch_all_by_catid($catarray, $num);
	if($alldata) {
...
function updatersscache($num) {
	...
	table_portal_rsscache::t()->truncate();
	require_once libfile('function/post');
	foreach($_G['cache']['portalcategory'] as $catid => $catarray) {
		$query = table_portal_article_title::t()->fetch_all_for_cat($catid, 0, 1, 0, $num);
		...
		foreach($query as $article) {
			...
			$articleattach = table_portal_attachment::t()->fetch_by_aid_image($article['aid']);
			$attachdata = '';
			...
			table_portal_rsscache::t()->insert([
