...
$article_count = table_portal_article_count::t()->fetch($aid);
if($article_count) $article = array_merge($article_count, $article);

if($article_count) {
	table_portal_article_count::t()->increase($aid, ['viewnum' => 1]);
	unset($article_count);
} else {
	table_portal_article_count::t()->insert([
		'aid' => $aid,
		'catid' => $article['catid'],
		'viewnum' => 1]);
}
...
$content = table_portal_article_content::t()->fetch_by_aid_page($aid, $page);
...
if($article['contents'] && $article['showinnernav']) {
	...
	if(empty($contents)) {
		table_portal_article_content::t()->update($aid, ['showinnernav' => '0']);
	}
}
...
if($article['idtype'] == 'tid' || $content['idtype'] == 'pid') {
	...
	if(!empty($thread)) {
		if($content['idtype'] == 'pid') {
			$firstpost = table_forum_post::t()->fetch_post($thread['posttableid'], $content['id']);
		} else {
			$firstpost = table_forum_post::t()->fetch_threadpost_by_tid_invisible($article['id']);
		}
	...
	if(!empty($firstpost) && !empty($thread) && $thread['displayorder'] != -1) {
		...
		table_portal_article_title::t()->update($aid, ['id' => 0, 'idtype' => '']);
		table_portal_article_content::t()->update_by_aid($aid, ['id' => 0, 'idtype' => '']);
	}
} elseif($article['idtype'] == 'blogid') {
	$org = table_home_blog::t()->fetch($article['id']);
	if(empty($org)) {
		table_portal_article_title::t()->update($aid, ['id' => 0, 'idtype' => '']);
		dheader('location: '.fetch_article_url($article));
		exit();
...
if($article['allowcomment']) {
	...
	if($org && empty($article['owncomment'])) {
		...
		if($article['idtype'] == 'blogid') {
			...
			$article['commentnum'] = table_home_comment::t()->count_by_id_idtype($article['id'], 'blogid');
			if($article['commentnum']) {
				$query = table_home_comment::t()->fetch_all_by_id_idtype($article['id'], 'blogid', 0, 20, '', 'DESC');
			...
			$_G['tid'] = $article['id'];
			$article['commentnum'] = getcount($posttable, ['tid' => $article['id'], 'first' => '0']);

			if($article['allowcomment'] && $article['commentnum']) {
				$attachpids = $attachtags = [];
				$_G['group']['allowgetattach'] = $_G['group']['allowgetimage'] = 1;
				foreach(table_forum_post::t()->fetch_all_by_tid('tid:'.$article['id'], $article['id'], true, 'ASC', 0, 20, null, 0) as $value) {
		...
		$query = table_portal_comment::t()->fetch_all_by_id_idtype($aid, 'aid', 'dateline', 'DESC', 0, 20);
		$pricount = 0;
		foreach($query as $value) {
...
$seodata = ['firstcat' => $cats[0], 'secondcat' => $cats[1], 'curcat' => $cat['catname'], 'subject' => $article['title'], 'user' => $article['username'], 'summary' => $article['summary'], 'page' => intval($_GET['page'])];
...
$v = getportalfile($articleprimaltplname, 'portal/view');
if(!$v) {
	$file = 'portal/view';
	$tpldirectory = '';
} else {
...
include_once template('diy:'.$file.':'.$catid, NULL, $tpldirectory);
...
function parseforumattach(&$post, $aids) {
	...
	if(($aids = array_unique($aids))) {
		require_once libfile('function/attachment');
		$finds = $replaces = [];
		foreach(table_forum_attachment_n::t()->fetch_all_by_id('tid:'.$post['tid'], 'aid', $aids) as $attach) {
...
function portal_get_per_next_article(&$article) {
	...
	if($aids) {
		$data = table_portal_article_title::t()->fetch_all($aids);
