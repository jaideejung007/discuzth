...
$hotlist = [];
if(!IS_ROBOT) {
	$feed_users = $feed_list = $user_list = $filter_list = $list = $magic = [];
	if($_GET['view'] != 'app') {
		if($space['self'] && empty($start) && $_G['setting']['feedhotnum'] > 0 && ($_GET['view'] == 'we' || $_GET['view'] == 'all')) {
			...
			$query = table_home_feed::t()->fetch_all_by_hot($hotstarttime);
	...
	if($gid >= 0) {
		$fuids = [];
		$query = table_home_friend::t()->fetch_all_by_uid_gid($_G['uid'], $gid);
		foreach($query as $value) {
			$fuids[] = $value['fuid'];
		}
		if(empty($fuids)) {
	...
	if($need_count) {
		...
		$query = table_home_feed::t()->fetch_all_by_search(1, $uids, $icon, '', '', '', $hot, '', $start, $perpage, $findex);
...
if($space['self'] && empty($start)) {
	...
	if($isnewer && $_G['setting']['homestyle']) {
		...
		$friendlist = [];
		$query = table_home_friend::t()->fetch_all($space['uid']);
		foreach($query as $value) {
			$friendlist[$value['fuid']] = 1;
		}

		foreach(table_home_specialuser::t()->fetch_all_by_status(1) as $value) {
	...
	if($_G['setting']['homestyle']) {
		foreach(table_home_visitor::t()->fetch_all_by_uid($space['uid'], 12) as $value) {
		...
		if($olfcount < 15) {
			$query = table_home_friend::t()->fetch_all_by_uid($space['uid'], 0, 32, true);
			foreach($query as $value) {
		...
		if($space['feedfriend']) {
			$birthdaycache = table_forum_spacecache::t()->fetch_spacecache($_G['uid'], 'birthday');
			if(empty($birthdaycache) || TIMESTAMP > $birthdaycache['expiration']) {
				$birthlist = table_common_member_profile::t()->fetch_all_will_birthday_by_uid($space['feedfriend']);
		...
		if($_G['setting']['taskstatus']) {
			...
			$task = count($taskarr) ? $taskarr[array_rand($taskarr)] : [];
		}
	...
	foreach(table_common_member::t()->range(0, 12, 'DESC') as $uid => $value) {
