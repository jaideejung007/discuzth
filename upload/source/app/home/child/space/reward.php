...
if($_GET['view'] == 'me') {
	$conditions = ['authorid' => $space['uid'], 'special' => 3, 'specialthread' => 1];
} else {
	...
	if($space['feedfriend']) {
		...
		if($fuid && friend_check($fuid, $space['uid'])) {
			$conditions = ['authorid' => $fuid, 'special' => 3, 'specialthread' => 1];
...
if($need_count) {
	...
	$count = table_forum_thread::t()->count_search($conditions);
	if($count) {

		foreach(table_forum_thread::t()->fetch_all_search($conditions, 0, $start, $perpage, 'dateline') as $value) {
