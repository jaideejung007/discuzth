...
if($_GET['view'] == 'me') {
	$viewtype = in_array($_GET['type'], ['orig', 'apply']) ? $_GET['type'] : 'orig';
...
if($need_count) {
	...
	$count = table_forum_activity::t()->fetch_all_for_search($_GET['view'], $_GET['order'], $_GET['searchkey'], $_GET['type'], $frienduid, $space['uid'], $minhot, 1);
	if($count) {
		$query = table_forum_activity::t()->fetch_all_for_search($_GET['view'], $_GET['order'], $_GET['searchkey'], $_GET['type'], $frienduid, $space['uid'], $minhot, 0, $start, $perpage);
		...
		if($tids) {
			...
			foreach($tids as $ptid => $ids) {
				foreach(table_forum_post::t()->fetch_all_by_tid($ptid, $ids, true, '', 0, 0, 1) as $value) {
