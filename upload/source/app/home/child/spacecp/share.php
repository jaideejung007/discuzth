...
if($_GET['op'] == 'delete') {
	if(submitcheck('deletesubmit')) {
		...
		deleteshares([$sid]);
		showmessage('do_success', $_GET['type'] == 'view' ? 'home.php?mod=space&quickforward=1&do=share' : dreferer(), ['sid' => $sid], ['showdialog' => 1, 'showmsg' => true, 'closetime' => true]);
	}
	...
	if($sid) {
		if(!$share = table_home_share::t()->fetch($sid)) {
			showmessage('share_does_not_exist');
		}
	}

	if(submitcheck('hotsubmit')) {
		table_home_share::t()->update($sid, ['hot' => $_POST['hot']]);
		table_home_feed::t()->update_feed($sid, ['hot' => $_POST['hot']], 'sid');
	...
	if(submitcheck('sharesubmit', 0, $seccodecheck, $secqaacheck)) {
		...
		if($type == 'link') {
			...
			$arr['body_data'] = ['link' => "<a href=\"$link\" target=\"_blank\">".$link_text.'</a>', 'data' => $link];
			...
			if(!empty($flashvar)) {
				...
				if($title) {
					$arr['body_data'] = ['link' => "<a href=\"$link\" target=\"_blank\">".$title.'</a>', 'data' => $link];
				}
		...
		$sid = table_home_share::t()->insert($arr, true);
		...
		if($type == 'link' || !(table_home_share::t()->count_by_uid_itemid_type($_G['uid'], $id ? $id : '', $type ? $type : ''))) {
		...
		$needle = $id ? $type.$id : '';
		updatecreditbyaction('createshare', $_G['uid'], ['sharings' => 1], $needle);
		...
		showmessage($showmessagecontent, $redirecturl, $magvalues, ($_G['inajax'] && $_GET['view'] != 'me' ? ['showdialog' => 1, 'showmsg' => true, 'closetime' => true] : []));
	}
...
if($type != 'link') {
	if((table_home_share::t()->count_by_uid_itemid_type($_G['uid'], $id ? $id : '', $type ? $type : ''))) {
		showmessage('spacecp_share_repeat');
	}
}

$share_count = table_home_share::t()->count_by_uid_itemid_type(0, $id ? $id : '', $type ? $type : '');
