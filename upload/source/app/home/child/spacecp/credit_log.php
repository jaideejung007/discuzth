...
$incomeactives = [$_GET['income'] => ' selected="selected"'];
...
if($_GET['suboperation'] == 'creditrulelog') {
	...
	$count = table_common_credit_rule_log::t()->count_by_uid($_G['uid']);
	if($count) {
	...
	$count = table_common_credit_log::t()->count_by_search($_G['uid'], $optype, $beginunixstr, $endunixstr, $exttype, $income, $_G['setting']['extcredits']);
	if($count) {
		$aids = $pids = $tids = $taskids = $uids = $loglist = [];
		loadcache(['magics']);
		foreach(table_common_credit_log::t()->fetch_all_by_search($_G['uid'], $optype, $beginunixstr, $endunixstr, $exttype, $income, $_G['setting']['extcredits'], $start, $perpage) as $log) {
			$credits = $ac_credits = [];
