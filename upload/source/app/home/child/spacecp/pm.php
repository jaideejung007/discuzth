...
if($_GET['op'] == 'checknewpm') {
	...
	if(!empty($_GET['tradeid'])) {
		$trade = table_forum_trade::t()->fetch_goods(0, $_GET['tradeid']);
		if($trade) {
		...
		$comment = table_forum_postcomment::t()->fetch($_GET['commentid']);
		if($comment) {
			$comment['comment'] = str_replace(['[b]', '[/b]', '[/color]'], [''], preg_replace('/\[color=([#\w]+?)\]/i', '', strip_tags($comment['comment'])));
		...
		if($thread) {
			$messageappend = dhtmlspecialchars('[url='.$_G['siteurl'].'forum.php?mod=redirect&goto=findpost&pid='.intval($_GET['pid']).'&ptid='.$thread['tid'].'][b]'.lang('spacecp', 'pm_thread_about', ['subject' => $thread['subject']]).'[/b][/url]');
		}
	}
	...
	if(submitcheck('deletesubmit', 1)) {
		...
		if(!empty($gpmid)) {
			$return = table_common_member_grouppm::t()->update($_G['uid'], $gpmid, ['status' => -1]);
	...
	if($waittime > 0) {
		showmessage('message_can_not_send_2', '', [], ['return' => true]);
	}

	cknewuser();

	if(!checkperm('allowsendpm')) {
		showmessage('no_privilege_sendpm', '', [], ['return' => true]);
	}
	...
	if(submitcheck('pmsubmit')) {
		...
		if(empty($message)) {
			showmessage('unable_to_send_air_news', '', [], ['return' => true]);
		}
		...
		if($touid || $pmid) {
			...
			$newusers = $uidsarr = $membersarr = [];
			if($users) {
				$membersarr = table_common_member::t()->fetch_all_by_username($users);
				foreach($membersarr as $aUsername => $aUser) {
					$uidsarr[] = $aUser['uid'];
				}
			}
			if(empty($membersarr)) {
				showmessage('message_bad_touser', '', [], ['return' => true]);
			}
			if(isset($membersarr[$_G['uid']])) {
				showmessage('message_can_not_send_to_self', '', [], ['return' => true]);
			}
			...
			$coef = count($newusers);
			$return = sendpm(implode(',', $uidsarr), $subject, $message, '', 0, 0, $type);
		} else {
			showmessage('message_can_not_send_9', '', [], ['return' => true]);
		}
		...
		if($return > 0) {
			...
			table_common_member_status::t()->update($_G['uid'], ['lastpost' => TIMESTAMP]);
			...
			if(!empty($newusers)) {
				...
				$newusers = is_array($newusers) ? $newusers : [$newusers];
				showmessage(count($users) ? 'message_send_result' : 'do_success', $returnurl, ['users' => implode(',', $users), 'succeed' => count($newusers)]);
			} else {
				if(!defined('IN_MOBILE')) {
					showmessage('do_success', 'home.php?mod=space&do=pm&subop=view&touid='.$touid, ['pmid' => $return], $_G['inajax'] ? ['msgtype' => 3, 'showmsg' => false] : []);
				} else {
			...
			if(in_array($return, range(-16, -1))) {
				...
				showmessage('message_can_not_send', '', [], ['return' => true]);
			}
		}
	}
	...
	if(submitcheck('ignoresubmit')) {
		...
		if($single) {
			...
			showmessage('do_success', dreferer(), [], ['showdialog' => 1, 'showmsg' => true, 'closetime' => true]);
		} else {
			uc_pm_blackls_set($_G['uid'], $_POST['ignorelist']);
			showmessage('do_success', 'home.php?mod=space&do=pm&view=ignore', [], ['showdialog' => 1, 'showmsg' => true, 'closetime' => true]);
		}
	}
	...
	if($pmid && submitcheck('pmreportsubmit', 1)) {
		...
		$pmreportcontent = lang('spacecp', 'pm_report_content', ['reporterid' => $_G['uid'], 'reportername' => $_G['username'], 'uid' => $pm['authorid'], 'username' => $pm['author'], 'message' => $pm['message']]);
		foreach($pmreportuser as $key => $value) {
			notification_add($value, 'pmreport', 'pmreportcontent', ['pmreportcontent' => $pmreportcontent], 0);
		}
		showmessage('do_success', dreferer(), [], ['showdialog' => 1, 'showmsg' => true, 'closetime' => true]);
	}
	...
	if(submitcheck('pmignoresubmit')) {
		uc_pm_blackls_add($_G['uid'], addslashes($user['loginname']));
		showmessage('do_success', dreferer(), [], ['showdialog' => 1, 'showmsg' => true, 'closetime' => true]);
	}
	...
	$members = [];
	if($memberusername) {
		$members = table_common_member::t()->fetch_all_by_username(explode(',', $memberusername));
	}
	if(empty($members)) {
	...
	if(submitcheck('pmappendmembersubmit')) {
		...
		foreach($members as $member) {
			...
			if($_G['group']['allowsendallpm'] || $member['onlyacceptfriendpm'] == 2 || ($member['onlyacceptfriendpm'] == 1 && friend_check($member['uid']))) {
				...
				$returns[] = ['uid' => $member['uid'], 'username' => $member['username'], 'return' => $return];
			} else {
				$returns[] = ['uid' => $member['uid'], 'username' => $member['username'], 'return' => 0];
			}
		}
		$cannotappend = [];
	...
	if($gpmids) {
		...
		table_common_member_grouppm::t()->update_to_read_by_unread($_G['uid'], $gpmidarr);
	}
	if($plids) {
		$plidarr = explode(',', $plids);
		uc_pm_readstatus($_G['uid'], [], $plidarr, 0);
	}
	showmessage('do_success', '', [], ['msgtype' => 3]);
	...
	$contents .= "\r\n\r\n================================================================\r\n";
	if($touser) {
		$contents .= lang('space', 'pm_export_touser', ['touser' => '<a href="'.$_G['siteurl'].'home.php?mod=space&uid='.$touser[0].'">'.$touser[1].'</a>']);
		$contents .= "\r\n================================================================\r\n";
	} elseif($subject) {
		$contents .= lang('space', 'pm_export_subject', ['subject' => $subject]);
		$contents .= "\r\n================================================================\r\n";
	}
	$contents .= "\r\n";
