...
$myfields = ['uid', 'gender', 'birthyear', 'birthmonth', 'birthday', 'birthcountry', 'birthprovince', 'birthcity', 'residecountry', 'resideprovince', 'residecity', 'residedist', 'residecommunity'];
...
if(!empty($_GET['searchsubmit']) || !empty($_GET['searchmode'])) {
	...
	if($searchkey = stripsearchkey($_GET['searchkey'])) {
		...
		foreach(['uid', 'username', 'avatarstatus'] as $value) {
			if($_GET[$value]) {
	...
	if($wherearr) {
		...
		$space['friends'] = [];
		$query = table_home_friend::t()->fetch_all_by_uid($_G['uid'], 0, 0);
		foreach($query as $value) {
			$space['friends'][$value['fuid']] = $value['fuid'];
		}

		foreach(table_common_member::t()->fetch_all_for_spacecp_search($wherearr, $fromarr, 0, 100) as $value) {
			$value['isfriend'] = ($value['uid'] == $space['uid'] || $space['friends'][$value['uid']]) ? 1 : 0;
			$list[$value['uid']] = $value;
		}
		$follows = table_home_follow::t()->fetch_all_by_uid_followuid($_G['uid'], array_keys($list));
	...
	$sexarr = [$space['sex'] => ' checked=\"checked\"'];
	...
	$bloodhtml = '';
	foreach(['A', 'B', 'O', 'AB'] as $value) {
		if(empty($_GET['all'])) $selectstr = $value == $space['blood'] ? ' selected=\"selected\"' : '';
		$bloodhtml .= "<option value=\"$value\"$selectstr>$value</option>";
	}
	$marryarr = [$space['marry'] => ' selected'];

	include_once libfile('function/profile');
	$birthcityhtml = showdistrict([0, 0, 0], ['birthcountry', 'birthprovince', 'birthcity'], 'birthcitybox', null, 'birth');
	$residecityhtml = showdistrict([0, 0, 0, 0, 0], ['residecountry', 'resideprovince', 'residecity', 'residedist', 'residecommunity'], 'residecitybox', null, 'reside');
...
$actives = [$op => ' class="a"', 'search' => ' class="a"'];
