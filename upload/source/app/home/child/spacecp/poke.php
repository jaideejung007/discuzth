...
if($op == 'send' || $op == 'reply') {
	...
	if($uid) {
		...
		$tospace = table_common_member::t()->fetch_uid_by_username($_POST['username']);
	}
	...
	if(submitcheck('pokesubmit')) {
		...
		table_home_pokearchive::t()->insert($setarr);
		...
		table_home_poke::t()->insert($setarr, false, true);

		require_once libfile('function/friend');
		friend_addnum($tospace['uid']);

		if($op == 'reply') {
			table_home_poke::t()->delete_by_uid_fromuid($_G['uid'], $uid);
			table_common_member::t()->increase($_G['uid'], ['newprompt' => -1]);
		}
		updatecreditbyaction('poke', 0, [], $uid);
		...
		showmessage('poke_success', dreferer(), ['username' => $tospace['username'], 'uid' => $uid, 'from' => $_GET['from']], ['showdialog' => 1, 'showmsg' => true, 'closetime' => true]);
	...
	if(submitcheck('ignoresubmit')) {
		...
		table_home_notification::t()->delete_by_uid_type_authorid($_G['uid'], 'poke', $uid);

		showmessage('has_been_hailed_overlooked', '', ['uid' => $uid, 'from' => $_GET['from']], ['showdialog' => 1, 'showmsg' => true, 'closetime' => true, 'alert' => 'right']);
	}
	...
	$list = [];
	foreach(table_home_poke::t()->fetch_all_by_uid_fromuid($space['uid'], $_GET['uid']) as $value) {
		...
		foreach(table_home_pokearchive::t()->fetch_all_by_pokeuid($pokeuid) as $subvalue) {
	...
	$fuids = $list = [];
	$count = table_home_poke::t()->count_by_uid($space['uid']);
	if($count) {
		foreach(table_home_poke::t()->fetch_all_by_uid($space['uid'], $start, $perpage) as $value) {
			$value['uid'] = $value['fromuid'];
			$value['username'] = $value['fromusername'];
...
$actives = [$op == 'send' ? 'send' : 'poke' => ' class="a"'];
