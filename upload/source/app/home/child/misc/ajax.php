...
if($op == 'comment') {
	...
	$idtype_array = ['picid' => 'album', 'blogid' => 'blog', 'sid' => 'share', 'uid' => 'wall'];

	$cmt = table_home_comment::t()->fetch_comment($cid);
	...
	$list = [];
	$value = table_home_comment::t()->fetch_latest_by_authorid($_G['uid'], $cid);
	$list[] = $value;
	...
	$list = [];
	foreach(table_home_share::t()->fetch_all_by_uid($_G['uid'], 0, 1) as $value) {
		$value = mkshare($value);
		$ajax_edit = 1;
		$list[] = $value;
	}
	...
	$count = table_home_pic::t()->check_albumpic($id, NULL, $_G['uid']);
	$piclist = [];
	$multi = '';
	if($count) {
		$query = table_home_pic::t()->fetch_all_by_albumid($id, $start, $perpage, 0, 0, 1, $_G['uid']);
	...
	$id = intval($_GET['id']);
	if($id) {
		table_home_notification::t()->delete_by_id_uid($id, $_G['uid']);
	}
	...
	$values = [intval($_GET['countryid']), intval($_GET['pid']), intval($_GET['cid']), intval($_GET['did']), intval($_GET['coid'])];
	...
	if($values[0]) {
		...
		if(!empty($district)) {
			foreach(table_common_district::t()->fetch_all_by_name($district) as $value) {
				$key = $value['level'];
				$values[$key] = $value['id'];
			}
			$level++;
	...
	$elems = [];
	if($_GET['country']) {
		$elems = [$_GET['country'], $_GET['province'], $_GET['city'], $_GET['district'], $_GET['community']];
	}
