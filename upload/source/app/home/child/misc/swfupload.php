...
$isupload = empty($_GET['cam']) && empty($_GET['doodle']);
$iscamera = isset($_GET['cam']);
$isdoodle = isset($_GET['doodle']);
$fileurl = '';
if(!empty($_POST['uid'])) {
...
if($op == 'finish') {
	...
	if($isupload) {
		...
		if($iscamera) {
			...
			foreach($directory as $key => $value) {
				...
				if(is_dir($dirstr)) {
					$filearr = dreaddir($dirstr, ['jpg', 'jpeg', 'gif', 'png', 'webp']);
					if(!empty($filearr)) {
			...
			$filearr = dreaddir(DISCUZ_ROOT.'./static/image/doodle/big', ['jpg', 'jpeg', 'gif', 'png', 'webp']);
		}
	}
	...
	if($op == 'doodle') {
		$magic = table_common_magic::t()->fetch_member_magic($_G['uid'], 'doodle');
	...
	if($uploadfiles && is_array($uploadfiles)) {
		...
		$uploadfiles = match ($uploadfiles) {
			-1 => lang('spacecp', 'inadequate_capacity_space'),
			-2 => lang('spacecp', 'only_allows_upload_file_types'),
			-4 => lang('spacecp', 'ftp_upload_file_size'),
			-8 => lang('spacecp', 'has_not_more_doodle'),
			default => lang('spacecp', 'mobile_picture_temporary_failure'),
		};
	}
