...
if(!submitcheck('searchsubmit', 1)) {
	...
	$orderby = $_GET['orderby'] == 'aid' ? $_GET['orderby'] : 'aid';
	...
	if(!empty($searchid)) {
		...
		foreach(table_common_searchindex::t()->fetch_all_search($_G['setting']['search']['portal']['searchctrl'], $_G['clientip'], $_G['uid'], $_G['timestamp'], $searchstring, $srchmod) as $index) {
			if($index['indexvalid'] && $index['dateline'] > $searchindex['dateline']) {
				$searchindex = ['id' => $index['searchid'], 'dateline' => $index['dateline']];
				break;
			} elseif($_G['adminid'] != '1' && $index['flood']) {
				showmessage('search_ctrl', 'search.php?mod=portal', ['searchctrl' => $_G['setting']['search']['portal']['searchctrl']]);
			}
		}
		...
		if($searchindex['id']) {
			...
			if($_G['adminid'] != '1' && $_G['setting']['search']['portal']['maxspm']) {
				if(table_common_searchindex::t()->count_by_dateline($_G['timestamp'], $srchmod) >= $_G['setting']['search']['portal']['maxspm']) {
					showmessage('search_toomany', 'search.php?mod=portal', ['maxspm' => $_G['setting']['search']['portal']['maxspm']]);
				}
			}
			...
			[$srchtxt, $srchtxtsql] = searchkey($keyword, "title LIKE '%{text}%'", true);
			$query = table_portal_article_title::t()->fetch_all_by_sql(' 1 '.$srchtxtsql, 'ORDER BY aid DESC ', 0, $_G['setting']['search']['portal']['maxsearchresults']);
			foreach($query as $article) {
				$ids .= ','.$article['aid'];
				$num++;
			}
