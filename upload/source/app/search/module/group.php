...
if(!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		$threadlist = $grouplist = $posttables = [];
		if($index['ids']['thread'] && ($searchstring[2] || empty($viewgroup))) {
			require_once libfile('function/misc');
			$threads = table_forum_thread::t()->fetch_all_by_tid_fid_displayorder(explode(',', $index['ids']['thread']), null, 0, $orderby, $start_limit, $_G['tpp'], '>=', $ascdesc);
		...
		if($index['ids']['group'] && ($viewgroup || empty($searchstring[2]))) {
			...
			if($viewgroup) {
				$query = table_forum_forum::t()->fetch_all_info_by_fids(explode(',', $index['ids']['group']), 3, $_G['tpp'], 0, 0, 0, 0, 'sub', $start_limit);
			} else {
				$query = table_forum_forum::t()->fetch_all_info_by_fids(explode(',', $index['ids']['group']), 3, 0, 0, 0, 0, 0, 'sub');
			}
		...
		foreach(table_common_searchindex::t()->fetch_all_search($_G['setting']['search']['group']['searchctrl'], $_G['clientip'], $_G['uid'], $_G['timestamp'], $searchstring, $srchmod) as $index) {
			if($index['indexvalid'] && $index['dateline'] > $searchindex['dateline']) {
				$searchindex = ['id' => $index['searchid'], 'dateline' => $index['dateline']];
				break;
			} elseif($_G['adminid'] != '1' && $index['flood']) {
				showmessage('search_ctrl', 'search.php?mod=group', ['searchctrl' => $_G['setting']['search']['group']['searchctrl']]);
			}
		}
		...
		if($searchindex['id']) {
			...
			if($_G['adminid'] != '1' && $_G['setting']['search']['group']['maxspm']) {
				if(table_common_searchindex::t()->count_by_dateline($_G['timestamp'], $srchmod) >= $_G['setting']['search']['group']['maxspm']) {
					showmessage('search_toomany', 'search.php?mod=group', ['maxspm' => $_G['setting']['search']['group']['maxspm']]);
				}
			}
			...
			$threads = table_forum_thread::t()->fetch_all_by_tid_fid(0, $srchfid, 1, '', $keyword, $_G['setting']['search']['group']['maxsearchresults']);
			foreach($threads as $value) {
				$fids[$value['fid']] = $value['fid'];
			}
			$forums = table_forum_forum::t()->fetch_all_by_fid($fids);
			foreach($threads as $thread) {
			...
			if(!$srchfid) {
				...
				$query = table_forum_forum::t()->fetch_all_fid_for_group(0, $_G['setting']['search']['group']['maxsearchresults'], 1, $srchtxtsql);
				foreach($query as $group) {
					$ids .= ','.$group['fid'];
					$num++;
				}
			}
			$allids = ['thread' => $tids, 'group' => $ids];
