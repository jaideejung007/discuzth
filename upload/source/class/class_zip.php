...
class SimpleUnzip {
	...
	function ReadFile($in_FileName) {
		...
		foreach($aE as $vZ) {
			...
			$aI['E'] = 0;
			$aI['EM'] = '';
			$aP = unpack('v1VN/v1GPF/v1CM/v1FT/v1FD/V1CRC/V1CS/V1UCS/v1FNL/v1EXL', $vZ);
			$bE = $aP['GPF'] && 0x0001;
			$nF = $aP['FNL'];
			...
			$aI['P'] = dirname($this->Convert2Utf8($aI['N']));
			$aI['P'] = $aI['P'] == '.' ? '' : $aI['P'];
			$aI['N'] = basename($this->Convert2Utf8($aI['N']));

			$vZ = substr($vZ, 26 + $nF + $aP['EXL']);
