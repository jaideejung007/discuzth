...
class table_forum_thread extends discuz_table {
	...
	public function fetch_thread($tid, $tableid = 0) {
		...
		if($tid && ($data = $this->fetch_cache($tid)) === false) {
			$parameter = [$this->get_table_name($tableid), $tid];
	...
	public function fetch_by_fid_displayorder($fid, $displayorder = 0, $glue = '>=', $order = 'lastpost', $sort = 'DESC') {
		$fid = intval($fid);
		if(!empty($fid)) {
			$parameter = [$this->get_table_name(), $fid, $displayorder];
	...
	public function fetch_next_tid_by_fid_lastpost($fid, $lastpost, $glue = '>', $sort = 'DESC', $tableid = 0) {
		$glue = helper_util::check_glue($glue);
		return DB::result_first("SELECT tid FROM %t WHERE fid=%d AND displayorder>=0 AND closed=0 AND lastpost{$glue}%d  ORDER BY ".DB::order('lastpost', $sort).DB::limit(1), [$this->get_table_name($tableid), $fid, $lastpost]);
	}
	...
	public function fetch_all_by_digest_displayorder($digest, $digestglue = '=', $displayorder = 0, $glue = '>=', $start = 0, $limit = 0, $tableid = 0) {
		$parameter = [$this->get_table_name($tableid), $digest, $displayorder];
	...
	public function fetch_all_by_fid_typeid_displayorder($fid, $typeid = null, $displayorder = null, $glue = '=', $start = 0, $limit = 0) {
		...
		$parameter = [$this->get_table_name(), $fid];
		$wherearr = [];
	...
	public function fetch_all_by_fid_lastpost($fid, $lstart = 0, $lend = 0, $tableid = 0) {
		$parameter = [$this->get_table_name($tableid), $fid];
		$wherearr = [];
		$wherearr[] = is_array($fid) ? 'fid IN(%n)' : 'fid=%d';
	...
	public function fetch_all_by_tid($tids, $start = 0, $limit = 0, $tableid = 0) {
		...
		if(($data = $this->fetch_cache($tids)) === false || count($tids) != count($data)) {
			...
			if($data === false) $data = [];
			if(!empty($tids)) {
				$parameter = [$this->get_table_name($tableid), $tids];
	...
	public function fetch_all_by_tid_displayorder($tids, $displayorder = null, $glue = '>=', $fids = [], $closed = null) {
		$data = [];
		if(!empty($tids)) {
			$data = $this->fetch_all_by_tid((array)$tids);
			$fids = $fids && !is_array($fids) ? [$fids] : $fids;
	...
	public function fetch_all_by_tid_or_fid($fid, $tids = []) {
		$parameter = [$this->get_table_name(), $fid];
		$forumstickytids = '';
		if(!empty($tids)) {
			$tids = dintval($tids, true);
	...
	public function fetch_all_by_displayorder($displayorder = 0, $glue = '>=', $start = 0, $limit = 0, $tableid = 0) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE %i '.DB::limit($start, $limit), [$this->get_table_name($tableid), DB::field('displayorder', $displayorder, $glue)]);
	}

	public function fetch_all_by_authorid($authorid, $start = 0, $limit = 0, $tableid = 0) {
		$authorid = dintval($authorid, true);
		return DB::fetch_all('SELECT * FROM %t WHERE %i ORDER BY dateline DESC '.DB::limit($start, $limit), [$this->get_table_name($tableid), DB::field('authorid', $authorid)], $this->_pk);
	}
	...
	public function fetch_all_by_dateline($starttime, $start = 0, $limit = 0, $order = 'dateline', $sort = 'DESC') {
		if($starttime) {
			...
			$parameter = [$this->get_table_name(), $starttime];
	...
	public function fetch_all_new_thread_by_tid($tid = 0, $start = 0, $limit = 0, $tableid = 0, $glue = '>', $sort = 'ASC') {
		$glue = helper_util::check_glue($glue);
		return DB::fetch_all("SELECT * FROM %t WHERE tid{$glue}%d ORDER BY ".DB::order('tid', $sort).DB::limit($start, $limit), [$this->get_table_name($tableid), $tid], $this->_pk);
	}
	...
	public function fetch_all_group_thread_by_fid_displayorder($fids, $displayorder = null, $dateline = null, $lastpost = null, $digest = null, $order = 'dateline', $start = 0, $limit = 0, $dglue = '>=') {
		$fids = dintval($fids, true);
		$parameter = [$this->get_table_name(), $fids];
		$wherearr = [];
		$wherearr[] = is_array($fids) && $fids ? 'fid IN(%n)' : 'fid=%d';
		if($displayorder !== null) {
			$parameter[] = $displayorder;
	...
	public function fetch_all_by_tid_fid($tids, $fids = [], $isgroup = -1, $author = '', $subject = '', $start = 0, $limit = 0) {
		$data = [];
	...
	public function fetch_all_by_fid($fids, $start = 0, $limit = 0, $tableid = 0) {
		$fids = dintval($fids, true);
		if($fids) {
			return DB::fetch_all('SELECT * FROM %t WHERE fid IN(%n) '.DB::limit($start, $limit), [$this->get_table_name($tableid), (array)$fids]);
		}
		return [];
	}
	...
	public function fetch_all_by_replies($number, $start = 0, $limit = 0, $glue = '>', $tableid = 0) {
		...
		if($number) {
			$glue = helper_util::check_glue($glue);
			return DB::fetch_all("SELECT * FROM %t WHERE replies{$glue}%d ".DB::limit($start, $limit), [$this->get_table_name($tableid), $number]);
		}
		return [];
	}
	...
	public function fetch_all_rank_thread($dateline, $notfid, $order = 'dateline', $start = 0, $limit = 0) {
		...
		if(!empty($fids)) {
			foreach(table_forum_forum::t()->fetch_all_name_by_fid(array_keys($fids)) as $value) {
	...
	public function fetch_all_rank_poll($dateline, $notfid, $order = 'dateline', $start = 0, $limit = 0) {
		$parameter = [$this->get_table_name(), 'forum_poll'];
	...
	public function fetch_all_rank_activity($dateline, $notfid, $order = 'dateline', $start = 0, $limit = 0) {
		$parameter = [$this->get_table_name(), 'forum_activity'];
		$wherearr = ['t.special=4', 't.isgroup=0', 't.closed=0'];
		if($dateline) {
			$parameter[] = $dateline;
			$wherearr[] = 't.dateline>%d';
		}
	...
	public function fetch_all_movedthread($start = 0, $limit = 0) {
		return DB::fetch_all('SELECT t1.tid, t2.tid AS threadexists, f.status, t1.isgroup FROM %t t1
				...
				WHERE t1.closed>1'.DB::limit($start, $limit), [$this->get_table_name(), $this->get_table_name(), 'forum_forum']);
	}

	public function fetch_all_by_fid_cover_lastpost($fid, $cover = null, $starttime = 0, $endtime = 0, $start = 0, $limit = 0) {
		$parameter = [$this->get_table_name(), $fid];
		$wherearr = ['fid=%d', 'displayorder>=0'];
		if($cover !== null) {
			$wherearr[] = 'cover=%d';
			$parameter[] = $cover;
		}
	...
	public function fetch_all_by_posttableid_displayorder($tableid = 0, $posttableid = 0, $displayorder = 0) {
		return DB::fetch_all('SELECT * FROM %t WHERE posttableid=%d AND displayorder>=%d ORDER BY lastpost'.DB::limit(1000), [$this->get_table_name($tableid), $posttableid, $displayorder], $this->_pk);
	}
	...
	private function make_query_condition($tids, $fids = [], $isgroup = -1, $author = '', $subject = '', $displayorder = null, $dateline = null) {
		$parameter = [$this->get_table_name()];
		$wherearr = [];
		if(!empty($tids)) {
			$tids = dintval($tids, true);
	...
	public function count_search($conditions, $tableid = 0, $prefix = false) {
		$prefix = $prefix ? '' : 't';
		return DB::result_first("SELECT COUNT(*) FROM %t $prefix %i", [$this->get_table_name($tableid), $this->search_condition($conditions, $prefix)]);
	}

	public function search_condition($conditions, $prefix = false) {
		$this->_urlparam = $wherearr = [];
		if($prefix) {
			$prefix = 't.';
		}
		...
		if(!empty($conditions['noanony'])) {
			$wherearr[] = $prefix.DB::field('author', '', '<>');
			$this->_urlparam[] = 'authorid='.$conditions['authorid'];
		}
		...
		if(isset($conditions['isgroup']) && in_array($conditions['isgroup'], [0, 1])) {
	...
	public function update_displayorder_by_tid_displayorder($tids, $olddisplayorder, $newdisplayorder) {
		...
		if($tids) {
			...
			return DB::query('UPDATE %t SET displayorder=%d WHERE tid IN (%n) AND displayorder=%d', [$this->get_table_name(), $newdisplayorder, $tids, $olddisplayorder]);
		}
		return 0;
	}
	...
	public function update_status_by_tid($tids, $value, $glue = '|') {
		...
		if($tids) {
			...
			$glue = helper_util::check_glue($glue);
			return DB::query("UPDATE %t SET status=status{$glue}%s WHERE tid IN(%n)", [$this->get_table_name(), $value, (array)$tids]);
		}
		return 0;
	}
	...
	public function update_sortid_by_sortid($sortid, $oldsortid) {
		...
		if($oldsortid) {
			return DB::query('UPDATE %t SET sortid=%d WHERE sortid IN (%n)', [$this->get_table_name(), $sortid, $oldsortid]);
		}
		return 0;
	}
	...
	public function insert_thread_copy_by_tid($tids, $origin = 0, $target = 0) {
		...
		if($tids) {
			...
			DB::query("INSERT INTO %t SELECT * FROM %t WHERE $wheresql", [$this->get_table_name($target), $this->get_table_name($origin), $tids]);
		}
	}
	...
	public function count_by_authorid_displayorder($authorid, $displayorder, $tableid = 0) {
		return DB::result_first('SELECT COUNT(*) FROM %t WHERE authorid=%d AND displayorder=%d', [$this->get_table_name($tableid), $authorid, $displayorder]);
	}
	...
	public function count_by_replies($number, $glue = '>') {
		$glue = helper_util::check_glue($glue);
		return DB::result_first("SELECT COUNT(*) FROM %t WHERE replies{$glue}%d", [$this->get_table_name(), $number]);
	}
	...
	public function count_posts_by_fid($fid, $forcetableid = null) {
		...
		foreach($threadtableids as $tableid) {
			$value = DB::fetch_first('SELECT COUNT(*) AS threads, SUM(replies)+COUNT(*) AS posts FROM %t WHERE fid=%d AND displayorder>=0', [$this->get_table_name($tableid), $fid]);
	...
	public function count_by_fid_displayorder_authorid($fid, $displayorder, $authorid, $tableid = 0) {
		return DB::result_first('SELECT COUNT(*) FROM %t WHERE fid=%d AND displayorder=%d AND authorid=%d', [$this->get_table_name($tableid), $fid, $displayorder, $authorid]);
	}

	public function count_all_thread() {
		$count = 0;
		$settings = table_common_setting::t()->fetch_all_setting('threadtableids', true);
		if(empty($settings['threadtableids']) || !is_array($settings['threadtableids'])) {
			$settings['threadtableids'] = [0];
		}
	...
	public function count_by_posttableid_displayorder($tableid = 0, $posttableid = 0, $displayorder = 0) {
		return DB::result_first('SELECT COUNT(*) FROM %t WHERE posttableid=%d AND displayorder=%d', [$this->get_table_name($tableid), $posttableid, $displayorder]);
	}

	public function count_by_tableid($tableid) {
		return DB::result_first('SELECT COUNT(*) FROM %t', [$this->get_table_name($tableid)]);
	}

	public function count_by_tid_fid($tids, $fids = [], $isgroup = -1, $author = '', $subject = '') {
		$condition = $this->make_query_condition($tids, $fids, $isgroup, $author, $subject);
	...
	public function count_group_thread_by_fid($fid) {
		return DB::fetch_all('SELECT COUNT(*) AS num, authorid FROM %t WHERE fid=%d GROUP BY authorid', [$this->get_table_name(), $fid]);
	}

	public function count_group_by_fid($tableid = 0) {
		return DB::fetch_all('SELECT fid, COUNT(*) AS threads, SUM(replies)+COUNT(*) AS posts FROM %t GROUP BY fid', [$this->get_table_name($tableid)]);
	}
	...
	public function delete_by_tid($tids, $unbuffered = false, $tableid = 0, $limit = 0) {
		...
		if($tids) {
			$this->clear_cache($tids);
			table_forum_newthread::t()->delete_by_tids($tids);
	...
	public function delete_by_fid($fid, $unbuffered = false, $tableid = 0, $limit = 0) {
		...
		if($fid) {
			...
			table_forum_newthread::t()->delete_by_tids($fid);
	...
	public function fetch_all_for_guide($type, $limittid, $tids = [], $heatslimit = 3, $dateline = 0, $start = 0, $limit = 600, $fids = 0) {
		$addsql = match ($type) {
			'hot' => ' AND heats>='.intval($heatslimit),
			'digest' => ' AND digest>0',
			default => '',
		};
		if(getglobal('setting/followforumid')) {
	...
	public function move_thread_by_tid($tids, $source, $target) {
		...
		if($source != $target) {
			DB::query('REPLACE INTO %t SELECT * FROM %t WHERE tid IN (%n)', [$this->get_table_name($target), $this->get_table_name($source), $tids]);
			if(!$source) {
				table_forum_threadhidelog::t()->delete_by_tid($tids);
			}
	...
	public function create_table($maxtableid) {
		if($maxtableid) {
			...
			$db = DB::object();
			$query = DB::query('SHOW CREATE TABLE %t', [$this->get_table_name()]);
	...
	public function drop_table($tableid) {
		$tableid = intval($tableid);
		if($tableid) {
			DB::query('DROP TABLE %t', [$this->get_table_name($tableid)], true);
			return true;
		} else {
			return false;
		}
