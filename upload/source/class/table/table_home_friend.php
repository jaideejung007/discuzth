...
class table_home_friend extends discuz_table {
	...
	public function fetch_all_by_uid_username($uid, $username, $start = 0, $limit = 0) {
		return DB::fetch_all('SELECT * FROM %t WHERE uid=%d AND fusername IN (%n) %i', [$this->_table, $uid, $username, DB::limit($start, $limit)]);
	}
	...
	public function fetch_all_by_uid_fuid($uid, $fuid) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE uid=%d AND fuid IN (%n)', [$this->_table, $uid, $fuid]);
	}

	public function fetch_all_by_uid_gid($uid, $gid, $start = 0, $limit = 100, $order = true) {
		$parameter = [$this->_table, $uid, $gid];
	...
	public function fetch_all_by_uid_common($uid, $fuid) {
		return DB::fetch_all('SELECT * FROM %t WHERE uid=%d OR uid=%d', [$this->_table, $uid, $fuid]);
	}
	...
	public function update_num_by_uid_fuid($incnum, $uid, $fuid) {
		return DB::query('UPDATE %t SET num=num+\'%d\' WHERE uid=%d AND fuid=%d', [$this->_table, $incnum, $uid, $fuid]);
	}
