...
class table_common_member_grouppm extends discuz_table {
	...
	public function count_by_gpmid($gpmid, $type) {
		return $gpmid ? DB::result_first('SELECT COUNT(*) FROM %t WHERE gpmid=%d AND dateline'.($type ? '>' : '=').'0', [$this->_table, $gpmid]) : 0;
	}
	...
	public function fetch_gpm($uid, $gpmid) {
		return $uid && $gpmid ? DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND gpmid=%d', [$this->_table, $uid, $gpmid]) : '';
	}

	public function fetch_all_by_gpmid($gpmid, $type, $start = 0, $limit = 0) {
		return $gpmid ? DB::fetch_all('SELECT * FROM %t WHERE gpmid=%d AND dateline'.($type ? '>' : '=').'0'.DB::limit($start, $limit), [$this->_table, $gpmid], 'uid') : [];
	}

	public function fetch_all_by_uid($uid, $type) {
		return $uid ? DB::fetch_all('SELECT * FROM %t WHERE uid=%d AND `status`'.($type ? '>=' : '=').'0', [$this->_table, $uid], 'gpmid') : [];
	}
	...
	public function update_to_read_by_unread($uid, $gpmid) {
		return ($uid = dintval($uid)) && ($gpmid = dintval($gpmid, true)) ? DB::update($this->_table, ['status' => 1], DB::field('gpmid', $gpmid).' AND '.DB::field('uid', $uid).' AND status=0') : false;
	}
}
