...
class table_common_payment_order extends discuz_table {
	...
	public function update_order_finish($id, $trade_no, $payment_time, $channel) {
		DB::query('UPDATE %t SET `trade_no` = %s,`payment_time` = %d, `channel` = %s, `status` = %d WHERE `id` = %d AND `status` = 0', [$this->_table, $trade_no, $payment_time, $channel, 1, $id]);
	...
	public function fetch_by_biz_no($out_biz_no) {
		return DB::fetch_first('SELECT * FROM %t WHERE out_biz_no = %s', [$this->_table, $out_biz_no]);
	}
	...
	public function fetch_type_all($uid = 0) {
		...
		$query = DB::query("SELECT `type`, `type_name` FROM %t $wherestr GROUP BY `type`", [$this->_table]);
		$result = [];
