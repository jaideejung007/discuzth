...
class table_home_pic extends discuz_table {
	...
	public function update_click($picid, $clickid, $incclick) {
		...
		return DB::query('UPDATE %t SET click'.$clickid.' = click'.$clickid.'+\'%d\' WHERE picid = %d', [$this->_table, $incclick, $picid]);
	}

	public function update_hot($picid, $num = 1) {
		return DB::query('UPDATE %t SET hot=hot+\'%d\' WHERE picid=%d', [$this->_table, $num, $picid]);
	}

	public function update_sharetimes($picid, $num = 1) {
		return DB::query('UPDATE %t SET sharetimes=sharetimes+\'%d\' WHERE picid=%d', [$this->_table, $num, $picid]);
	}
	...
	public function fetch_all_by_uid($uids, $start = 0, $limit = 0, $picids = 0) {
		...
		return DB::fetch_all("SELECT * FROM %t WHERE $picidsql ".DB::field('uid', $uids).DB::limit($start, $limit), [$this->_table]);
	}
	...
	public function fetch_all_by_albumid($albumids, $start = 0, $limit = 0, $picids = 0, $orderbypicid = 0, $orderbydateline = 0, $uid = 0, $count = false) {
		...
		if($count) {
			return DB::result_first("SELECT COUNT(*) FROM %t WHERE $picidsql ".DB::field('albumid', $albumids)." $uidsql", [$this->_table]);
		} else {
			return DB::fetch_all("SELECT * FROM %t WHERE $picidsql ".DB::field('albumid', $albumids)." $uidsql $ordersql".DB::limit($start, $limit), [$this->_table]);
		}
	}
	...
	public function fetch_albumpic($albumid, $uid) {
		return DB::fetch_first('SELECT filepath, thumb FROM %t WHERE albumid=%d AND uid=%d ORDER BY thumb DESC, dateline DESC LIMIT 0,1', [$this->_table, $albumid, $uid]);
	}
	...
	public function count_size_by_uid($uid) {
		return DB::result_first('SELECT SUM(size) FROM %t WHERE uid=%d', [$this->_table, $uid]);
	}
	...
	public function fetch_by_id_idtype($id) {
		...
		return DB::fetch_first('SELECT * FROM %t WHERE %i', [$this->_table, DB::field('picid', $id)]);
	}
	...
	public function update_dateline_by_id_idtype_uid($id, $idtype, $dateline, $uid) {
		...
		return DB::update($this->_table, ['dateline' => intval($dateline)], [$idtype => intval($id), 'uid' => intval($uid)]);
	}
}
