...
class table_common_member_medal extends discuz_table {
	...
	public function fetch_all_by_uid($uid) {
		return !empty($uid) ? DB::fetch_all('SELECT * FROM '.DB::table($this->_table).' WHERE '.DB::field('uid', $uid), '', 'medalid') : [];
	}
	...
	public function count_by_uid_medalid($uid, $medalid) {
		return DB::result_first('SELECT COUNT(*) FROM %t WHERE uid=%d AND medalid=%d', [$this->_table, $uid, $medalid]);
	}
}
