...
class table_forum_collectioncomment extends discuz_table {
	...
	public function delete_by_cid_ctid($cids, $ctid = 0) {
		...
		return DB::query("DELETE FROM %t WHERE cid IN (%n) $ctidsql", [$this->_table, $cids]);
	}
	...
	public function fetch_all_by_ctid($ctid, $start = 0, $limit = 0) {
		return DB::fetch_all('SELECT * FROM %t WHERE ctid=%d ORDER BY dateline DESC '.DB::limit($start, $limit), [$this->_table, $ctid], $this->_pk);
	}
	...
	public function fetch_all_by_uid($uid) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE %i', [$this->_table, DB::field('uid', $uid)], $this->_pk);
	}

	public function fetch_rate_by_ctid_uid($ctid, $uid) {
		return DB::result_first('SELECT rate FROM %t WHERE ctid=%d AND uid=%d AND rate!=0', [$this->_table, $ctid, $uid], $this->_pk);
	}
