...
class table_common_usergroup_field extends discuz_table {
	...
	public function fetch_readaccess_by_readaccess($readaccess) {
		return DB::fetch_all('SELECT groupid,readaccess FROM %t WHERE readaccess>%d ORDER BY readaccess', [$this->_table, $readaccess], $this->_pk);
	}
	...
	public function fetch_all_fields($gid, $fields) {
		...
		return DB::fetch_all('SELECT %i FROM %t %i', [$fieldssql, $this->_table, ($gid ? 'WHERE '.DB::field('groupid', $gid) : '')], $this->_pk);
	}

	public function count_by_field($field, $val, $glue = '=') {
		$allowedfield = ['allowposttrade'];
		if(!in_array($field, $allowedfield)) {
			return null;
		}
		return DB::result_first('SELECT count(*) FROM %t WHERE %i', [$this->_table, DB::field($field, $val, $glue)]);
	}
	...
	public function update_allowsearch() {
		return DB::query('UPDATE %t SET allowsearch = allowsearch | 2 WHERE groupid < 20 AND groupid NOT IN (5, 6)', [$this->_table]);
	}
