...
class table_forum_moderator extends discuz_table {
	...
	public function fetch_all_by_fid($fid, $order = true) {
		return DB::fetch_all('SELECT * FROM %t WHERE fid=%d'.($order ? ' ORDER BY inherited, displayorder' : ''), [$this->_table, $fid], 'uid');
	}

	public function fetch_all_by_fid_inherited($fid, $inherited = 0) {
		return DB::fetch_all('SELECT * FROM %t WHERE fid=%d AND inherited=%d', [$this->_table, $fid, $inherited], 'uid');
	}
	...
	public function fetch_all_by_uid($uid) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE %i', [$this->_table, DB::field('uid', $uid)], 'fid');
	}

	public function fetch_all_by_uid_forum($uid) {
		return DB::fetch_all('SELECT m.fid, f.name, f.recyclebin
			FROM %t m, %t f
			WHERE m.uid=%d AND f.fid=m.fid AND f.status=\'1\' AND f.type<>\'group\'', [$this->_table, 'forum_forum', $uid]);
	}

	public function fetch_uid_by_fid_uid($fid, $uid) {
		return DB::result_first('SELECT uid FROM %t WHERE fid=%d AND uid=%d', [$this->_table, $fid, $uid]);
	}
	...
	public function fetch_uid_by_tid($tid, $uid, $archiveid) {
		...
		return DB::result_first('SELECT uid FROM %t m INNER JOIN %t t ON t.tid=%d AND t.fid=m.fid WHERE m.uid=%d', [$this->_table, $threadtable, $tid, $uid]);
	}
	...
	public function count_by_uid($uid) {
		...
		return DB::result_first('SELECT count(*) FROM %t WHERE %i', [$this->_table, DB::field('uid', $uid)]);
	}

	public function fetch_all_no_inherited_by_fid($fid) {
		return DB::fetch_all('SELECT * FROM %t WHERE fid=%d AND inherited=0 ORDER BY displayorder', [$this->_table, $fid], 'uid');
	}

	public function fetch_all_no_inherited() {
		return DB::fetch_all('SELECT * FROM %t WHERE inherited=0 ORDER BY displayorder', [$this->_table]);
	}
	...
	public function update_by_fid_uid($fid, $uid, $data) {
		...
		return DB::update($this->_table, $data, ['fid' => $fid, 'uid' => $uid]);
	}
