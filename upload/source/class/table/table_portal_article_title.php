...
class table_portal_article_title extends discuz_table {
	...
	public function update_click($cid, $clickid, $incclick) {
		...
		return DB::query('UPDATE %t SET click'.$clickid.' = click'.$clickid.'+\'%d\' WHERE aid = %d', [$this->_table, $incclick, $cid]);
	}
	...
	public function fetch_all_for_cat($catid, $status = null, $orderaid = 0, $start = 0, $limit = 0) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE '.DB::field('catid', $catid).$statussql.$orderaidsql.DB::limit($start, $limit), [$this->_table]);
	}
	...
	public function fetch_all_by_sql($where, $order = '', $start = 0, $limit = 0, $count = 0, $alias = '') {
		...
		if($count) {
			return DB::result_first('SELECT count(*) FROM '.DB::table($this->_table).'  %i %i %i '.DB::limit($start, $limit), [$alias, $where, $order]);
		}
		return DB::fetch_all('SELECT * FROM '.DB::table($this->_table).' %i %i %i '.DB::limit($start, $limit), [$alias, $where, $order]);
	}
	...
	public function fetch_all_aid_by_dateline($dateline, $catids = [], $startid = 0, $endid = 0) {
		$data = [];
		$where = [];
		if($startid) {
	...
	public function fetch_preaid_by_catid_aid($catid, $aid) {
		...
		if(($catid = intval($catid)) && ($aid = intval($aid))) {
			$ret = DB::result_first('SELECT aid FROM %t WHERE catid=%d AND aid<%d ORDER BY aid DESC LIMIT 1', [$this->_table, $catid, $aid]);
		}
		return $ret;
	}
	...
	public function fetch_nextaid_by_catid_aid($catid, $aid) {
		$ret = 0;
		if(($catid = intval($catid)) && ($aid = intval($aid))) {
			$ret = DB::result_first('SELECT aid FROM %t WHERE catid=%d AND aid>%d ORDER BY aid ASC LIMIT 1', [$this->_table, $catid, $aid]);
		}
		return $ret;
	}
}
