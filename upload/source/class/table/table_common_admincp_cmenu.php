...
class table_common_admincp_cmenu extends discuz_table {
	...
	public function count_by_uid($uid) {
		return DB::result_first('SELECT COUNT(*) FROM %t WHERE uid=%d AND sort=1', [$this->_table, $uid]);
	}
	...
	public function delete_cmenu($id, $uid = 0) {
		...
		return DB::query('DELETE FROM %t WHERE '.DB::field('id', $id).' %i', [$this->_table, ($uid ? ' AND '.DB::field('uid', $uid) : '')]);
	}

	public function fetch_id_by_uid_sort_url($uid, $sort, $url) {
		return DB::result_first('SELECT id FROM %t WHERE uid=%d AND sort=%d AND url=%s', [$this->_table, $uid, $sort, $url]);
	}

	public function increase_clicks($id) {
		return DB::query('UPDATE %t SET clicks=clicks+1 WHERE id=%d', [$this->_table, $id]);
	}
