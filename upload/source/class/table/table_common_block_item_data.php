...
class table_common_block_item_data extends discuz_table {
	...
	public function fetch_all_by_bid($bid, $isverified = 1, $start = 0, $limit = 0, $bannedids = [], $format = true) {
		$data = [];
		if(($bid = dintval($bid, true))) {
			$addsql = $bannedids = dintval($bannedids, true) ? ' AND id NOT IN ('.dimplode($bannedids).')' : '';
			$query = DB::query('SELECT * FROM %t WHERE '.DB::field('bid', $bid).' AND isverified=%d'.$addsql.' ORDER BY stickgrade DESC, displayorder DESC, verifiedtime DESC, dataid DESC '.DB::limit($start, $limit), [$this->_table, $isverified]);
	...
	public function count_by_bid($bid, $isverified = 1) {
		return ($bid = dintval($bid, true)) ? DB::result_first('SELECT COUNT(*) FROM %t WHERE '.DB::field('bid', $bid).' AND isverified=%d', [$this->_table, $isverified]) : 0;
	}
	...
	public function delete_by_dataid_bid($dataids, $bid) {
		if(($dataids = dintval($dataids, true)) && ($dataids = DB::fetch_all('SELECT dataid FROM %t WHERE dataid IN (%n) AND bid=%d', [$this->_table, $dataids, $bid], $this->_pk))) {
			$this->delete(array_keys($dataids));
		}
	}

	public function fetch_by_bid_id_idtype($bid, $id, $idtype) {
		return $bid && $id && $idtype ? DB::fetch_first('SELECT * FROM %t WHERE bid=%d AND id=%d AND idtype=%s', [$this->_table, $bid, $id, $idtype]) : [];
	}
}
