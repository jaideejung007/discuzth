...
class table_common_member_stat_field extends discuz_table {
	...
	public function count_by_fieldid($fieldid) {
		return $fieldid ? DB::result_first('SELECT COUNT(*) FROM %t WHERE fieldid=%s', [$this->_table, $fieldid]) : 0;
	}

	public function fetch_all_by_fieldid($fieldid, $start = 0, $limit = 0) {
		return DB::fetch_all('SELECT * FROM %t WHERE fieldid=%s'.DB::limit($start, $limit), [$this->_table, $fieldid]);
	}
