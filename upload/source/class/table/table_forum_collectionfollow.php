...
class table_forum_collectionfollow extends discuz_table {
	...
	public function fetch_by_ctid_uid($ctid, $uid) {
		return DB::fetch_first('SELECT * FROM %t WHERE uid=%d AND ctid=%d', [$this->_table, $uid, $ctid]);
	}
	...
	public function delete_by_ctid_uid($ctid, $uid) {
		return DB::query('DELETE FROM %t WHERE uid=%d AND ctid=%d', [$this->_table, $uid, $ctid]);
	}
	...
	public function count_by_ctid_uid($uid, $ctid) {
		return DB::result_first('SELECT COUNT(*) FROM %t WHERE uid=%d AND ctid=%d', [$this->_table, $uid, $ctid], $this->_pk);
	}
