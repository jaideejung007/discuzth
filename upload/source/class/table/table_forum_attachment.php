...
class table_forum_attachment extends discuz_table {
	...
	public function update_download($aid, $count = 1) {
		...
		return DB::query('UPDATE %t SET downloads=downloads+%d WHERE aid IN (%n)', [$this->_table, $count, (array)$aid], false, true);
	}
	...
	public function fetch_all_by_id($idtype, $ids, $orderby = '') {
		...
		if(in_array($idtype, ['aid', 'tid', 'pid', 'uid']) && $ids) {
			$query = DB::query('SELECT * FROM %t WHERE %i IN (%n) %i', [$this->_table, $idtype, (array)$ids, $orderby]);
	...
	public function delete_by_id($idtype, $ids) {
		if(in_array($idtype, ['aid', 'tid', 'pid', 'uid']) && $ids) {
			DB::query('DELETE FROM %t WHERE %i IN (%n)', [$this->_table, $idtype, (array)$ids], false, true);
		}
	}

	public function update_by_id($idtype, $ids, $newtid) {
		if(in_array($idtype, ['tid', 'pid']) && $ids) {
			DB::query('UPDATE %t SET tid=%d,tableid=%d WHERE %i IN (%n)', [$this->_table, $newtid, getattachtableid($newtid), $idtype, (array)$ids], false, true);
		}
	}

	public function count_by_tid($tid) {
		return $tid ? DB::result_first('SELECT COUNT(*) FROM %t WHERE tid=%d', [$this->_table, $tid]) : 0;
	}
	...
	public function fetch_all_for_manage($tableid, $inforum = '', $authorid = 0, $filename = '', $keyword = '', $sizeless = 0, $sizemore = 0, $dlcountless = 0, $dlcountmore = 0, $daysold = 0, $count = 0, $start = 0, $limit = 0) {
		...
		if($inforum) {
			if(is_array($inforum)) {
				$sql .= ' AND t.fid IN('.dimplode($inforum).')';
			} else {
