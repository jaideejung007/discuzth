...
class table_common_advertisement extends discuz_table {
	...
	public function fetch_all_type() {
		return DB::fetch_all('SELECT type, COUNT(type) AS count FROM %t GROUP BY type', [$this->_table]);
	}

	public function fetch_all_by_type($type) {
		return DB::fetch_all('SELECT * FROM %t WHERE type=%s', [$this->_table, $type]);
	}

	public function fetch_all_old() {
		return DB::fetch_all('SELECT * FROM %t WHERE available>0 AND starttime<=%d ORDER BY displayorder', [$this->_table, TIMESTAMP]);
	}

	public function close_endtime() {
		$return = DB::result_first("SELECT COUNT(*) FROM %t WHERE endtime>0 AND endtime<='".TIMESTAMP."'", [$this->_table]);
		DB::update($this->_table, ['available' => 0], "endtime>0 AND endtime<='".TIMESTAMP."'", 'UNBUFFERED');
		return $return;
	}

	public function fetch_all_endtime($endtime) {
		return DB::fetch_all('SELECT * FROM %t WHERE endtime=%s', [$this->_table, $endtime]);
	}
