...
class table_forum_forumfield extends discuz_table {
	...
	public function fetch_all_by_fid($fids) {
		...
		if(!empty($fids)) {
			return DB::fetch_all('SELECT * FROM %t WHERE fid IN(%n)', [$this->_table, $fids], $this->_pk);
		} else {
			return [];
		}
	}
	...
	public function fetch_info_for_attach($fid, $uid) {
		return DB::fetch_first('SELECT f.fid, f.viewperm, f.getattachperm, a.allowgetattach, a.allowgetimage FROM %t f LEFT JOIN %t a ON a.uid=%d AND a.fid=f.fid WHERE f.fid=%d', ['forum_forumfield', 'forum_access', $uid, $fid]);
	}
