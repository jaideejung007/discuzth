...
const SDK_WEIXIN_PAY_UNIFIEDORDER = 'https://api.mch.weixin.qq.com/pay/unifiedorder';
const SDK_WEIXIN_PAY_ORDERQUERY = 'https://api.mch.weixin.qq.com/pay/orderquery';
const SDK_WEIXIN_PAY_REFUND = 'https://api.mch.weixin.qq.com/secapi/pay/refund';
const SDK_WEIXIN_PAY_REFUNDQUERY = 'https://api.mch.weixin.qq.com/pay/refundquery';
const SDK_WEIXIN_AUTHORIZE = 'https://open.weixin.qq.com/connect/oauth2/authorize';
const SDK_WEIXIN_SNS_ACCESS_TOKEN = 'https://api.weixin.qq.com/sns/oauth2/access_token';

const SDK_WEIXIN_PAY_V3_TRANSACTIONS_NATIVE = 'https://api.mch.weixin.qq.com/v3/pay/transactions/native';
const SDK_WEIXIN_PAY_V3_TRANSACTIONS_H5 = 'https://api.mch.weixin.qq.com/v3/pay/transactions/h5';
const SDK_WEIXIN_PAY_V3_TRANSACTIONS_JSAPI = 'https://api.mch.weixin.qq.com/v3/pay/transactions/jsapi';
const SDK_WEIXIN_PAY_V3_TRANSACTIONS_OUTTRADENO = 'https://api.mch.weixin.qq.com/v3/pay/transactions/out-trade-no/';
const SDK_WEIXIN_PAY_V3_REFUND_DOMESTIC_REFUNDS = 'https://api.mch.weixin.qq.com/v3/refund/domestic/refunds';
const SDK_WEIXIN_PAY_V3_REFUND_DOMESTIC_REFUNDS_QUERY = 'https://api.mch.weixin.qq.com/v3/refund/domestic/refunds/';
const SDK_WEIXIN_PAY_V3_CERTIFICATES = 'https://api.mch.weixin.qq.com/v3/certificates';

class pay_wechat extends pay_base {

	public function __construct() {
		global $_G;
		$this->settings = table_common_setting::t()->fetch_setting('ec_wechat', true);
	...
	public function pay($order) {
		if(!$this->enable()) {
			return ['code' => 500, 'message' => 'Did not open payment'];
		}
	...
	public function transfer($transfer_no, $amount, $realname, $account, $title = '', $desc = '') {
		return ['code' => 500, 'message' => 'not support.'];
	}
	...
	public function wechat_jsapidata($prepay_id) {
		if($this->settings['ec_wechat_version']) {
			...
			$jsapidata = ['appId' => $this->settings['appid'], 'timeStamp' => time().'', 'nonceStr' => $this->wechat_nonce(), 'package' => 'prepay_id='.$prepay_id, 'signType' => 'MD5'];
	...
	public function wechat_sign_verify() {
		...
		if($sign != $data['sign']) {
			return ['code' => 50001, 'data' => $data];
		}
		if($data['return_code'] != 'SUCCESS') {
			return ['code' => 50002, 'data' => $data];
		}
		if($data['result_code'] != 'SUCCESS') {
			return ['code' => 50003, 'data' => $data];
		}
	...
	private function wechat_unifiedorder_pay($order, $type = 'NATIVE', $openid = null) {
		...
		$data = ['appid' => $this->settings['appid'], 'mch_id' => $this->settings['mch_id'], 'nonce_str' => $this->wechat_nonce(), 'sign_type' => 'MD5', 'body' => $order['subject'], 'detail' => $order['description'], 'out_trade_no' => $order['out_biz_no'], 'total_fee' => intval($order['amount']), 'spbill_create_ip' => $_G['clientip'], 'time_expire' => dgmdate(time() + 86400, 'YmdHis'), 'notify_url' => $this->notify_url, 'trade_type' => $type,];
		if($openid) {
			$data['openid'] = $openid;
		}
		...
		if($res['return_code'] != 'SUCCESS') {
			return ['code' => 500, 'message' => $res['return_msg']];
		} elseif($res['result_code'] != 'SUCCESS') {
			return ['code' => 501, 'message' => $res['err_code_des']];
		} else {
			if($res['code_url']) {
				$url = $res['code_url'];
			} elseif($res['mweb_url']) {
	...
	private function wechat_order_query($out_biz_no) {
		...
		if($res['return_code'] != 'SUCCESS') {
			...
			return ['code' => 500, 'message' => $res['err_code_des']];
		} else {
			$pay_time = strtotime(preg_replace('/^(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})$/', '$1-$2-$3 $4:$5:$6', $res['time_end']));
			return ['code' => 200, 'data' => ['trade_no' => $res['transaction_id'], 'payment_time' => $pay_time]];
		}
	}
	...
	private function wechat_refund($refund_no, $trade_no, $total_amount, $refund_amount, $refund_desc) {
		...
		if($res['return_code'] != 'SUCCESS') {
			...
			return ['code' => 200, 'data' => ['refund_time' => time()]];
		}
	}
	...
	private function wechat_refund_status($refund_no) {
		...
		if($res['return_code'] != 'SUCCESS') {
			...
			return ['code' => 500, 'message' => $res['err_code'].'-'.$res['err_code_des']];
		} else {
			return ['code' => 200, 'data' => ['refund_time' => time()]];
		}
	}
	...
	private function v3_wechat_native_pay($order) {
		...
		$data = ['appid' => $this->settings['appid'], 'mchid' => $this->settings['mch_id'], 'description' => $order['subject'].': '.$order['description'], 'out_trade_no' => $order['out_biz_no'], 'notify_url' => $this->notify_url, 'amount' => ['total' => intval($order['amount']), 'currency' => 'CNY']];

		$api = SDK_WEIXIN_PAY_V3_TRANSACTIONS_NATIVE;
		$res = $this->v3_wechat_request_json($api, json_encode($data));
		$res = json_decode($res, true);
		if($res['code_url']) {
			return ['code' => 200, 'url' => $res['code_url']];
		} else {
			if(strtoupper($_G['charset'] != 'UTF-8') && $res['message']) {
				$res['message'] = diconv($res['message'], 'UTF-8', $_G['charset']);
			}
			return ['code' => $res['code'], 'message' => is_array($res['message']) ? $res['code'].'-'.$res['err_code'].'-'.$res['err_code_des'] : $res['message']];
		}
	}
	...
	private function v3_wechat_h5_pay($order) {
		...
		$res = json_decode($res, true);
		if($res['h5_url']) {
			return ['code' => 200, 'url' => $res['h5_url']];
		} else {
			if(strtoupper($_G['charset'] != 'UTF-8') && $res['message']) {
				$res['message'] = diconv($res['message'], 'UTF-8', $_G['charset']);
			}
	...
	private function v3_wechat_h5_jsapi($order, $openid) {
		...
		$res = json_decode($res, true);
		if($res['prepay_id']) {
			return ['code' => 200, 'url' => $res['prepay_id']];
		} else {
			if(strtoupper($_G['charset'] != 'UTF-8') && $res['message']) {
				$res['message'] = diconv($res['message'], 'UTF-8', $_G['charset']);
			}
	...
	private function v3_wechat_query_order($out_biz_no) {
		...
		if($res['trade_state'] && $res['trade_state'] == 'SUCCESS') {
			...
			return ['code' => $res['trade_state'], 'message' => $res['trade_state_desc']];
		} else {
			if(strtoupper($_G['charset'] != 'UTF-8') && $res['message']) {
				$res['message'] = diconv($res['message'], 'UTF-8', $_G['charset']);
			}
	...
	private function v3_wechat_refund($refund_no, $trade_no, $total_amount, $refund_amount, $refund_desc) {
		...
		$data = ['transaction_id' => $trade_no, 'out_refund_no' => $refund_no, 'reason' => $refund_desc, 'amount' => ['refund' => intval($refund_amount), 'total' => intval($total_amount), 'currency' => 'CNY']];
	...
	private function wechat_request($api, $data = []) {
		$client = filesock::open([
			'url' => $api,
			'method' => 'POST',
			'post' => $data
		]);
