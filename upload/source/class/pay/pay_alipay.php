...
const SDK_ALIPAY_GATEWAYURL = 'https://openapi.alipay.com/gateway.do';

class pay_alipay extends pay_base {

	public function __construct() {
		global $_G;
		$this->settings = table_common_setting::t()->fetch_setting('ec_alipay', true);
	...
	private function alipay_trade_query($out_biz_no) {
		...
		$data = [
			...
			'version' => '1.0',
			'biz_content' => json_encode(['out_trade_no' => $out_biz_no])
		];
		if($this->settings['ec_alipay_sign_mode']) {
		...
		if($res['code'] == 10000) {
			if($res['trade_status'] == 'TRADE_SUCCESS') {
				return ['code' => 200, 'data' => ['trade_no' => $res['trade_no'], 'payment_time' => strtotime($res['send_pay_date'])]];
			} else {
				return ['code' => 500, 'message' => $res['trade_status']];
			}
		} else {
			if(strtoupper($_G['charset'] != 'UTF-8')) {
				$res['sub_msg'] = diconv($res['sub_msg'], 'UTF-8', $_G['charset']);
			}
			return ['code' => $res['sub_code'], 'message' => $res['sub_msg']];
		}
	}
	...
	private function alipay_trade_page_pay($order) {
		...
		if(preg_match('/^https?:\/\/.+$/', $res)) {
			...
			if(preg_match('/<div\s+class="Todo">([^<]+)<\/div>/i', $res, $matchers)) {
				return ['code' => 500, 'message' => $matchers[1]];
			} else {
				return ['code' => 501, 'message' => $res];
			}
		}
	}
	...
	private function alipay_fund_trans_uni_transfer($transfer_no, $amount, $realname, $account, $title = '', $desc = '') {
		...
		if(!$this->settings['ec_alipay_sign_mode']) {
			return ['code' => 500, 'message' => 'not support sign mode.'];
		}
		...
		$biz_content = [
			...
			'payee_info' => ['identity' => $account, 'identity_type' => 'ALIPAY_LOGON_ID', 'name' => $realname]
		];
		if($title) {
		...
		if($res['code'] == 10000) {
			if($res['status'] == 'SUCCESS') {
				return ['code' => 200, 'data' => ['transfer_time' => strtotime($res['trans_date'])]];
			} elseif($res['status'] == 'DEALING') {
				return ['code' => 201, 'message' => 'DEALING'];
			} else {
	...
	private function alipay_fund_trans_order_query($transfer_no) {
		global $_G;
		$data = ['method' => 'alipay.fund.trans.order.query', 'format' => 'JSON', 'charset' => 'utf-8', 'sign_type' => 'RSA2', 'timestamp' => dgmdate(time(), 'Y-m-d H:i:s'), 'version' => '1.0',];
		if($this->settings['ec_alipay_sign_mode']) {
			$appid = $this->settings['mode_b_appid'];
			$private_key = $this->settings['mode_b_app_private_key'];
			$data['app_cert_sn'] = $this->alipay_cert_sn($this->settings['mode_b_app_cert']);
		...
		$biz_content = ['out_biz_no' => $transfer_no,];
		$data['biz_content'] = json_encode($biz_content);
	...
	private function alipay_refund($refund_no, $trade_no, $amount, $refund_desc) {
		...
		$data = ['method' => 'alipay.trade.refund', 'format' => 'JSON', 'charset' => 'utf-8', 'sign_type' => 'RSA2', 'timestamp' => dgmdate(time(), 'Y-m-d H:i:s'), 'version' => '1.0', 'biz_content' => json_encode(['trade_no' => $trade_no, 'refund_amount' => $amount / 100, 'out_request_no' => $refund_no, 'refund_reason' => $refund_desc]),];
		if($this->settings['ec_alipay_sign_mode']) {
			$appid = $this->settings['mode_b_appid'];
			$private_key = $this->settings['mode_b_app_private_key'];
			$data['app_cert_sn'] = $this->alipay_cert_sn($this->settings['mode_b_app_cert']);
	...
	private function alipay_refund_status($refund_no, $trade_no) {
		global $_G;
		$data = ['method' => 'alipay.trade.fastpay.refund.query', 'format' => 'JSON', 'charset' => 'utf-8', 'sign_type' => 'RSA2', 'timestamp' => dgmdate(time(), 'Y-m-d H:i:s'), 'version' => '1.0', 'biz_content' => json_encode(['trade_no' => $trade_no, 'out_request_no' => $refund_no,]),];
		if($this->settings['ec_alipay_sign_mode']) {
			$appid = $this->settings['mode_b_appid'];
			$private_key = $this->settings['mode_b_app_private_key'];
			$data['app_cert_sn'] = $this->alipay_cert_sn($this->settings['mode_b_app_cert']);
