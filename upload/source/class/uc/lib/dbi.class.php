...
class ucclient_db {
	...
	function result_first_stmt($sql, $key = [], $value = []) {
		$query = $this->query_stmt($sql, $key, $value);
		return $this->result($query, 0);
	}

	function fetch_first_stmt($sql, $key = [], $value = []) {
		$query = $this->query_stmt($sql, $key, $value);
		return $this->fetch_array($query);
	}

	function fetch_all_stmt($sql, $key = [], $value = [], $id = '') {
		$arr = [];
		$query = $this->query_stmt($sql, $key, $value);
		while($data = $this->fetch_array($query)) {
			$id ? $arr[$data[$id]] = $data : $arr[] = $data;
	...
	function query_stmt($sql, $key = [], $value = [], $type = '', $saveprep = FALSE, $cachetime = FALSE) {
