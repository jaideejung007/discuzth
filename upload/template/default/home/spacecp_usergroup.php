...
<!--{elseif $do == 'switch'}-->
	...
	<form id="switchgroupform_{$groupid}" name="switchgroupform_{$groupid}" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=usergroup&do=switch&groupid=$groupid"{if !empty($_GET['inajax'])} onsubmit="ajaxpost('switchgroupform_{$groupid}', 'return_$_GET['handlekey']', 'return_$_GET['handlekey']', 'onerror');return false;"{/if}>
		...
		<div class="c">
			<table class="list" cellspacing="0" cellpadding="0" style="width:300px">
				<tr>
					<td>{lang memcp_usergroups_main_old}</td><td>$_G[group][grouptitle]{echo g_icon($_G['group']['groupid'], 1, 20);}</td>
				</tr>
				<tr>
					<td>{lang memcp_usergroups_main_new}</td><td>$group[grouptitle]{echo g_icon($group['groupid'], 1, 20);}</td>
				</tr>
			</table>
		</div>
		<p class="o pns">
	...
	<!--{subtemplate home/spacecp_header}-->
			...
			<div class="tdats">
				...
				<table cellpadding="0" cellspacing="0" class="tdat tfx{if !$group}f{/if}">
					...
					<tr>
						<th class="alt">
							<span class="notice">{lang credits}: $space[credits]
							<!--{if $upgroup_credits}-->
							&nbsp; {$upgroup_name}{lang credits}: $upgroup_credits
							<!--{/if}-->
							</span>
						</th>
					</tr>
				...
				<!--{if $group}-->
					<!--{if $switchtype == 'user'}--><!--{eval $cid = 1;$tlang = '{lang usergroup_group1}';}-->
					<!--{elseif $switchtype == 'upgrade'}--><!--{eval $cid = 2;$tlang = '{lang usergroup_group2}';}-->
					<!--{elseif $switchtype == 'admin'}--><!--{eval $cid = 3;$tlang = '{lang usergroup_group3}';}-->
					<!--{else}--><!--{eval $cid = 2;$tlang = '{lang usergroup_group2}';}--><!--{/if}-->
					...
					<div class="tscr">
						<table cellpadding="0" cellspacing="0" class="tdat">
							<tr>
								<th class="alt h">
									<!--{if $group['grouptype'] == 'member' || $group[groupcreditshigher] > 0}-->
										<!--{if $creditleft > 0}-->
											<span class="notice">
												{lang spacecp_usergroup_message1} $creditleft <span title="$upgroup_creditsformulaexp">{$creditstype}{lang credits}</span>
											</span>
										<!--{else}-->
											<span class="notice">
												<span title="$upgroup_creditsformulaexp">{$creditstype}{lang credits}</span> $group[groupcreditshigher]
											</span>
										<!--{/if}-->
									<!--{/if}-->
			...
			<!--{loop $upgroups $upgid $v}-->
				<div id="ug{$upgid}_menu" class="p_pop" style="display:none">$usergroups['up'.$upgid]</div>
			<!--{/loop}-->
		</div>
		<!--{hook/spacecp_usergroup_bottom}-->
	</div>
