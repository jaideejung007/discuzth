	...
	<div class="o cl">
		...
		<!--{if checkperm('allowbanuser') || checkperm('allowedituser')}-->
			<!--{if checkperm('allowedituser')}-->
				<a href="{if $_G[adminid] == 1}?app=admin&action=members&operation=search&username=$encodeusername&submit=yes&frames=yes{else}forum.php?mod=modcp&action=member&op=edit&uid=$space[uid]{/if}" target="_blank" class="xi1">{lang user_edit}</a>
			<!--{/if}-->
			<!--{if checkperm('allowbanuser')}-->
				<a href="{if $_G[adminid] == 1}?app=admin&action=members&operation=ban&username=$encodeusername&frames=yes{else}forum.php?mod=modcp&action=member&op=ban&uid=$space[uid]{/if}" target="_blank" class="xi1">{lang user_ban}</a>
			<!--{/if}-->
