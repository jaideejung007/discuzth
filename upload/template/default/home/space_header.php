	...
	<div id="toptb" class="cl">
		<!--{if $_G['uid']}-->
			<div class="y">
				...
				<!--{if $_G['uid'] && ($_G['group']['radminid'] == 1 || getstatus($_G['member']['allowadmincp'], 1))}--><a href="?app=admin" target="_blank">{lang admincp}</a><!--{/if}-->
				<!--{hook/global_usernav_extra2}-->
				<a href="member.php?mod=logging&action=logout&formhash={FORMHASH}">{lang logout}</a>
				<!--{if $space['self'] && $do == 'index'}--><a id="diy-tg" href="javascript:openDiy();" title="{lang dress_space}">{lang dress_space}</a><!--{/if}-->
			</div>
	...
	<div id="hd" class="wp cl">
		...
		<h2 id="spaceinfoshow">
			...
			<strong id="spacename" class="mbn">
				<!--{if $space[spacename]}-->$space[spacename]<!--{else}-->{lang somebody_space} $space[username]<!--{/if}--><!--discuzth-->
			</strong>
			<span class="xs0 xw0">
