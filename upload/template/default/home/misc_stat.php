...
<!--{eval $_GET['types'] = isset($_GET['types']) && is_array($_GET['types']) ? $_GET['types'] : array();}-->
			...
			<table cellspacing="0" cellpadding="0" class="dt bm mbw">
				...
				<tr class="tbmu">
					...
					<td>
						...
						<label$actives[$value]><input type="checkbox" name="types[]" value="$value" class="pc" {if is_array($_GET['types']) && in_array($value, $_GET['types'])} checked="checked"{/if} /><!--{eval echo lang('spacecp', "do_stat_$value");}--></label> &nbsp;
						<!--{/loop}-->
					</td>
				</tr>
				<tr class="tbmu">
