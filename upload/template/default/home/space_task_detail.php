			...
			<td>
				<table cellpadding="0" cellspacing="0" class="tfm">
					...
					<tr>
						...
						<td class="bbda">
							<!--{if $task[applyperm] || $task[relatedtaskid] || $task[exclusivetaskid] || $task[tasklimits] || $taskvars['apply']}-->
								<ul>
									...
									<!--{if $task[exclusivetaskid]}--><li>{lang task_exclusivetask}: <a href="home.php?mod=task&do=view&id=$task[exclusivetaskid]">$_G['taskexclusive']</a></li><!--{/if}-->
		...
		<tr>
			...
			<td>
				<!--{if $allowapply == '-1'}-->
					...
					<p class="mbw">
						<a href="home.php?mod=task&do=draw&id=$task[taskid]" class="taskbtn {if $task[csc] >=100}taskrw{else}taskda{/if}">{lang task_reward_get}</a>
						<!--{if $task[csc] < 100}--><a href="home.php?mod=task&do=delete&id=$task[taskid]" class="taskbtn">{lang task_quit}</a><!--{/if}-->
					</p>
				<!--{elseif $allowapply == '-2'}-->
				...
				<!--{elseif $allowapply == '-6'}-->
					...
					<a href="javascript:;" onclick="doane(event);showDialog('{$task[t]}{lang task_applyagain}')" class="taskbtn taskda" title="{$task[t]}{lang task_applyagain}">{lang task_newbie_apply}</a>
				<!--{elseif $allowapply == '-7'}-->
					<p class="xg2 mbn">{lang task_lose_on}$task[dateline] &nbsp; {$task[t]}{lang task_reapply}</p>
					<a href="javascript:;" onclick="doane(event);showDialog('{$task[t]}{lang task_reapply}')" class="taskbtn taskda" title="{$task[t]}{lang task_reapply}">{lang task_newbie_apply}</a>
				<!--{elseif $allowapply == '2'}-->
