				...
				<tr>
					...
					<td valign="top" class="xs1">
						...
						<p class="mtm mbm">
							...
							<span class="pipe">|</span><a id="a_manage" href="?app=admin&action=members&operation=search&uid=$space[uid]&submit=1&frames=yes">{lang manage_member}</a>
							<!--{/if}-->
						</p>
						<!--{if $space[spacenote]}-->
						<p>$space[spacenote]</p>
						...
						<div class="mtm pbm mbm bbda cl">
							...
							<ul class="xl xl2 cl">
								<!--{if $space[adminid] > 0}--><li>{lang management_team}: <span style="color:{$space[admingroup][color]}">{$space[admingroup][grouptitle]}</span> {$space[admingroup][icon]}</li><!--{/if}-->
						...
						<!--{if $space['medals']}-->
							<div class="pbm mbm bbda cl">
								...
								<img src="$medal['image']" border="0" alt="$medal['name']" title="$medal['name']" /> &nbsp;
								<!--{/loop}-->
							</div>
						<!--{/if}-->
