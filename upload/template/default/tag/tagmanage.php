...
<!--{elseif ($op == 'manage')}-->
	...
	<div class="c bart">
		...
		<!--{eval $recent_use_tag = recent_use_tag('tid');}-->
		<!--{if $recent_use_tag}-->
...
<!--{/if}-->
	...
	function tagsearch() {
		...
		var url = 'misc.php?mod=tag&op=search&inajax=1&searchkey='+searchkey;
		var x = new Ajax();
	...
	function tagset() {
		...
		var url = 'misc.php?mod=tag&op=set&inajax=1&tags='+tags+'&tid='+tid+'&formhash={FORMHASH}';
		var x = new Ajax();
		x.get(url, function(s){
			if(s) {
