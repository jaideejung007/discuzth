		...
		<div class="tbn">
			...
			<ul>
				...
				<!--{if $modforums['fids'] && $_G['setting']['forumstatus']}-->
					...
					<!--{if $_G['group']['alloweditforum'] && $_G['forum']['jointype'] == 2}--><li{if $_GET[action] == 'forum' && $op == 'member'} class="a"{/if}><span class="y mtn">$joinchecknum</span><a href="{$cpscript}?mod=modcp&action=forum&op=member$forcefid">{lang mod_option_forum_member}</a></li><!--{/if}-->
				<!--{/if}-->
