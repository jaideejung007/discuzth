...
<header class="header">
	<div class="nav">
		...
		<span class="name">{lang pay_attachment}</span>
	</div>
</header>
	...
	<div class="f_c">
		...
		<div class="clew_con">
			<!--{if $status != 1}-->
				...
				<input class="button" type="submit" name="submit" value="<!--{if $status == 0}-->{lang pay_attachment}<!--{else}-->{lang free_buy}<!--{/if}-->" />
			<!--{/if}-->
			<input class="button" type="button" onclick="location.href = 'forum.php?mod=viewthread&tid=$_G['tid']';"value="{lang back}" />
		</div>
	</div>
</form>
