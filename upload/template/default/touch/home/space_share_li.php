...
<!--{if empty($ajax_edit)}--><li id="share_$value[sid]_li"><!--{/if}-->
	<div class="h">
		...
		<!--{if $value[uid] != $_G[uid] && $value[itemid] && helper_access::check_module('share')}-->
			<a href="home.php?mod=spacecp&ac=share&type=$value[type]&id=$value[itemid]" id="k_share" class="dialog">{lang sharemetoo}</a>
			<span class="pipe">|</span>
		<!--{/if}-->
		<!--{hook/space_share_comment_op $k}-->
		<!--{if $value[uid]==$_G[uid]}-->
			<a href="home.php?mod=spacecp&ac=share&op=delete&sid=$value[sid]&handlekey=dellshk_{$value[sid]}" id="s_$value[sid]_delete" class="dialog">{lang delete}</a>
		<!--{/if}-->
		</div>
