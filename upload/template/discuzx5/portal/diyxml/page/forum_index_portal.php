<?PHP exit('Access Denied');?>
<!--[name]พอร์ทัลหน้าแรก[/name]-->
<?xml version="1.0" encoding="ISO-8859-1"?>
<root>
	<item id="spacecss"><![CDATA[#portal_block_201 { margin-top:0px !important;margin-right:0px !important;margin-bottom:15px !important;margin-left:0px !important;}#portal_block_202 { margin-top:0px !important;margin-right:0px !important;margin-bottom:15px !important;margin-left:0px !important;}#portal_block_203 { margin-top:0px !important;margin-right:0px !important;margin-bottom:15px !important;margin-left:0px !important;}]]></item>
	<item id="layoutdata">
		<item id="diynavtop"><![CDATA[]]></item>
		<item id="diy1"><![CDATA[]]></item>
		<item id="diy2">
			<item id="frame`framefYYVYv">
				<item id="attr">
					<item id="name"><![CDATA[framefYYVYv]]></item>
					<item id="moveable"><![CDATA[true]]></item>
					<item id="className"><![CDATA[frame move-span cl frame-1]]></item>
					<item id="titles"><![CDATA[]]></item>
				</item>
				<item id="column`framefYYVYv_left">
					<item id="attr">
						<item id="name"><![CDATA[framefYYVYv_left]]></item>
						<item id="className"><![CDATA[column frame-1-c]]></item>
					</item>
					<item id="block`portal_block_202">
						<item id="attr">
							<item id="name"><![CDATA[portal_block_202]]></item>
							<item id="className"><![CDATA[block move-span]]></item>
							<item id="titles">
								<item id="className">
									<item id="0"><![CDATA[blocktitle title]]></item>
								</item>
								<item id="style"><![CDATA[]]></item>
								<item id="first">
									<item id="text"><![CDATA[คำถามและคำตอบแนะนำ]]></item>
									<item id="href"><![CDATA[]]></item>
									<item id="color"><![CDATA[]]></item>
									<item id="float"><![CDATA[]]></item>
									<item id="margin"><![CDATA[]]></item>
									<item id="font-size"><![CDATA[]]></item>
									<item id="className"><![CDATA[titletext]]></item>
									<item id="src"><![CDATA[]]></item>
								</item>
							</item>
						</item>
					</item>
					<item id="block`portal_block_201">
						<item id="attr">
							<item id="name"><![CDATA[portal_block_201]]></item>
							<item id="className"><![CDATA[block move-span]]></item>
							<item id="titles">
								<item id="className">
									<item id="0"><![CDATA[blocktitle title]]></item>
								</item>
								<item id="style"><![CDATA[]]></item>
								<item id="first">
									<item id="text"><![CDATA[โพสต์แนะนำ]]></item>
									<item id="href"><![CDATA[]]></item>
									<item id="color"><![CDATA[]]></item>
									<item id="float"><![CDATA[]]></item>
									<item id="margin"><![CDATA[]]></item>
									<item id="font-size"><![CDATA[]]></item>
									<item id="className"><![CDATA[titletext]]></item>
									<item id="src"><![CDATA[]]></item>
								</item>
							</item>
						</item>
					</item>
					<item id="block`portal_block_203">
						<item id="attr">
							<item id="name"><![CDATA[portal_block_203]]></item>
							<item id="className"><![CDATA[block move-span]]></item>
							<item id="titles">
								<item id="className">
									<item id="0"><![CDATA[blocktitle title]]></item>
								</item>
								<item id="style"><![CDATA[]]></item>
								<item id="first">
									<item id="text"><![CDATA[สมาชิกที่มีความเคลื่อนไหว]]></item>
									<item id="href"><![CDATA[]]></item>
									<item id="color"><![CDATA[]]></item>
									<item id="float"><![CDATA[]]></item>
									<item id="margin"><![CDATA[]]></item>
									<item id="font-size"><![CDATA[]]></item>
									<item id="className"><![CDATA[titletext]]></item>
									<item id="src"><![CDATA[]]></item>
								</item>
							</item>
						</item>
					</item>
				</item>
			</item>
		</item>
		<item id="diy3"><![CDATA[]]></item>
	</item>
	<item id="blockdata">
		<item id="block">
			<item id="201">
				<item id="bid"><![CDATA[201]]></item>
				<item id="blockclass"><![CDATA[forum_thread]]></item>
				<item id="blocktype"><![CDATA[0]]></item>
				<item id="name"><![CDATA[1]]></item>
				<item id="title"><![CDATA[<div class="blocktitle title"><span class="titletext" style="float:;margin-left:px;font-size:;color: !important;">โพสต์แนะนำ</span></div>]]></item>
				<item id="classname"><![CDATA[]]></item>
				<item id="summary"><![CDATA[]]></item>
				<item id="uid"><![CDATA[1]]></item>
				<item id="username"><![CDATA[admin]]></item>
				<item id="styleid"><![CDATA[9]]></item>
				<item id="blockstyle"><![CDATA[]]></item>
				<item id="picwidth"><![CDATA[0]]></item>
				<item id="picheight"><![CDATA[0]]></item>
				<item id="target"><![CDATA[blank]]></item>
				<item id="dateformat"><![CDATA[Y-m-d]]></item>
				<item id="dateuformat"><![CDATA[0]]></item>
				<item id="script"><![CDATA[thread]]></item>
				<item id="param">
					<item id="tids"><![CDATA[]]></item>
					<item id="uids"><![CDATA[]]></item>
					<item id="keyword"><![CDATA[]]></item>
					<item id="tagkeyword"><![CDATA[]]></item>
					<item id="typeids"><![CDATA[]]></item>
					<item id="reply"><![CDATA[0]]></item>
					<item id="recommend"><![CDATA[0]]></item>
					<item id="viewmod"><![CDATA[0]]></item>
					<item id="rewardstatus"><![CDATA[0]]></item>
					<item id="picrequired"><![CDATA[0]]></item>
					<item id="orderby"><![CDATA[lastpost]]></item>
					<item id="postdateline"><![CDATA[0]]></item>
					<item id="lastpost"><![CDATA[0]]></item>
					<item id="highlight"><![CDATA[0]]></item>
					<item id="titlelength"><![CDATA[40]]></item>
					<item id="summarylength"><![CDATA[80]]></item>
					<item id="startrow"><![CDATA[0]]></item>
					<item id="items"><![CDATA[10]]></item>
				</item>
				<item id="shownum"><![CDATA[10]]></item>
				<item id="cachetime"><![CDATA[3600]]></item>
				<item id="cachetimerange"><![CDATA[]]></item>
				<item id="punctualupdate"><![CDATA[0]]></item>
				<item id="hidedisplay"><![CDATA[0]]></item>
				<item id="dateline"><![CDATA[1752459948]]></item>
				<item id="notinherited"><![CDATA[0]]></item>
				<item id="isblank"><![CDATA[0]]></item>
			</item>
			<item id="202">
				<item id="bid"><![CDATA[202]]></item>
				<item id="blockclass"><![CDATA[forum_thread]]></item>
				<item id="blocktype"><![CDATA[0]]></item>
				<item id="name"><![CDATA[กิจกรรมยอดนิยม]]></item>
				<item id="title"><![CDATA[<div class="blocktitle title"><span class="titletext" style="float:;margin-left:px;font-size:;color: !important;">คำถามและคำตอบแนะนำ</span></div>]]></item>
				<item id="classname"><![CDATA[]]></item>
				<item id="summary"><![CDATA[]]></item>
				<item id="uid"><![CDATA[1]]></item>
				<item id="username"><![CDATA[admin]]></item>
				<item id="styleid"><![CDATA[0]]></item>
				<item id="blockstyle">
					<item id="name"><![CDATA[]]></item>
					<item id="blockclass"><![CDATA[forum_thread]]></item>
					<item id="makethumb"><![CDATA[0]]></item>
					<item id="getpic"><![CDATA[0]]></item>
					<item id="getsummary"><![CDATA[0]]></item>
					<item id="settarget"><![CDATA[1]]></item>
					<item id="moreurl"><![CDATA[0]]></item>
					<item id="fields">
						<item id="0"><![CDATA[url]]></item>
						<item id="1"><![CDATA[title]]></item>
						<item id="2"><![CDATA[views]]></item>
						<item id="3"><![CDATA[dateline]]></item>
					</item>
					<item id="template">
						<item id="raw"><![CDATA[<div class="dz_forumn_qna_list">
<dl>
[loop]
<dd>
   <a href="{url}" title="{title}"{target}>
        <div class="dz_hot_qna_top">
            {title}
        </div>
        <div class="dz_hot_qna_btm">
            มีผู้เข้าชม {views} คน, เผยแพร่เมื่อ {dateline}
        </div>
        <div class="clear"></div>
    </a>
</dd>

[/loop]
</dl>
</div>]]></item>
						<item id="footer"><![CDATA[]]></item>
						<item id="header"><![CDATA[]]></item>
						<item id="indexplus"><![CDATA[
						]]></item>
						<item id="index"><![CDATA[
						]]></item>
						<item id="orderplus"><![CDATA[
						]]></item>
						<item id="order"><![CDATA[
						]]></item>
						<item id="loopplus"><![CDATA[
						]]></item>
						<item id="loop"><![CDATA[<dd>
   <a href="{url}" title="{title}"{target}>
        <div class="dz_hot_qna_top">
            {title}
        </div>
        <div class="dz_hot_qna_btm">
            มีผู้เข้าชม {views} คน, เผยแพร่เมื่อ {dateline}
        </div>
        <div class="clear"></div>
    </a>
</dd>]]></item>
					</item>
					<item id="hash"><![CDATA[630cea75]]></item>
				</item>
				<item id="picwidth"><![CDATA[0]]></item>
				<item id="picheight"><![CDATA[0]]></item>
				<item id="target"><![CDATA[blank]]></item>
				<item id="dateformat"><![CDATA[Y-m-d]]></item>
				<item id="dateuformat"><![CDATA[0]]></item>
				<item id="script"><![CDATA[thread]]></item>
				<item id="param">
					<item id="tids"><![CDATA[]]></item>
					<item id="uids"><![CDATA[]]></item>
					<item id="keyword"><![CDATA[]]></item>
					<item id="tagkeyword"><![CDATA[]]></item>
					<item id="typeids"><![CDATA[]]></item>
					<item id="reply"><![CDATA[0]]></item>
					<item id="recommend"><![CDATA[0]]></item>
					<item id="viewmod"><![CDATA[0]]></item>
					<item id="rewardstatus"><![CDATA[0]]></item>
					<item id="picrequired"><![CDATA[0]]></item>
					<item id="orderby"><![CDATA[lastpost]]></item>
					<item id="postdateline"><![CDATA[0]]></item>
					<item id="lastpost"><![CDATA[0]]></item>
					<item id="highlight"><![CDATA[0]]></item>
					<item id="titlelength"><![CDATA[40]]></item>
					<item id="summarylength"><![CDATA[80]]></item>
					<item id="startrow"><![CDATA[0]]></item>
					<item id="items"><![CDATA[6]]></item>
				</item>
				<item id="shownum"><![CDATA[6]]></item>
				<item id="cachetime"><![CDATA[3600]]></item>
				<item id="cachetimerange"><![CDATA[]]></item>
				<item id="punctualupdate"><![CDATA[0]]></item>
				<item id="hidedisplay"><![CDATA[0]]></item>
				<item id="dateline"><![CDATA[1752459948]]></item>
				<item id="notinherited"><![CDATA[0]]></item>
				<item id="isblank"><![CDATA[0]]></item>
			</item>
			<item id="203">
				<item id="bid"><![CDATA[203]]></item>
				<item id="blockclass"><![CDATA[member_member]]></item>
				<item id="blocktype"><![CDATA[0]]></item>
				<item id="name"><![CDATA[สมาชิกที่มีความเคลื่อนไหว]]></item>
				<item id="title"><![CDATA[<div class="blocktitle title"><span class="titletext" style="float:;margin-left:px;font-size:;color: !important;">สมาชิกที่มีความเคลื่อนไหว</span></div>]]></item>
				<item id="classname"><![CDATA[]]></item>
				<item id="summary"><![CDATA[]]></item>
				<item id="uid"><![CDATA[1]]></item>
				<item id="username"><![CDATA[admin]]></item>
				<item id="styleid"><![CDATA[0]]></item>
				<item id="blockstyle">
					<item id="name"><![CDATA[]]></item>
					<item id="blockclass"><![CDATA[member_member]]></item>
					<item id="makethumb"><![CDATA[0]]></item>
					<item id="getpic"><![CDATA[0]]></item>
					<item id="getsummary"><![CDATA[0]]></item>
					<item id="settarget"><![CDATA[1]]></item>
					<item id="moreurl"><![CDATA[0]]></item>
					<item id="fields">
						<item id="0"><![CDATA[currentorder]]></item>
						<item id="1"><![CDATA[url]]></item>
						<item id="2"><![CDATA[avatar_big]]></item>
						<item id="3"><![CDATA[title]]></item>
						<item id="4"><![CDATA[threads]]></item>
						<item id="5"><![CDATA[credits]]></item>
					</item>
					<item id="template">
						<item id="raw"><![CDATA[<div class="dz_good_author dz_author">
<ul>
[index=1]
	<li>
	
<em class="dz_member_num{currentorder}"></em>
		<div class="dz_index_member_intel">
			<div class="dz_index_member_avator">
				<a href="{url}" c="1" {target}><img src="{avatar_big}"  alt="{title}" onerror="this.onerror=null;this.src='uc_server/data/avatar/noavatar.svg'" /></a>
			</div>
			<div class="dz_index_member_info">
				<h5>
					<a href="{url}" title="{title}" {target}>{title}</a>
					<div class="clear"></div>
				</h5>
				<div class="dz_index_member_info_btm">
					<p>ผลงาน {threads} กระทู้<span class="y">{credits} เครดิต</span></p>
				</div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
	</li>
	[/index]

[index=2]
	<li>
	
<em class="dz_member_num{currentorder}"></em>
		<div class="dz_index_member_intel">
			<div class="dz_index_member_avator">
				<a href="{url}" c="1" {target}><img src="{avatar_big}"  alt="{title}" onerror="this.onerror=null;this.src='uc_server/data/avatar/noavatar.svg'" /></a>
			</div>
			<div class="dz_index_member_info">
				<h5>
					<a href="{url}" title="{title}" {target}>{title}</a>
					<div class="clear"></div>
				</h5>
				<div class="dz_index_member_info_btm">
					<p>ผลงาน {threads} กระทู้<span class="y">{credits} เครดิต</span></p>
				</div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
	</li>
	[/index]

[index=3]
	<li>
	
<em class="dz_member_num{currentorder}"></em>
		<div class="dz_index_member_intel">
			<div class="dz_index_member_avator">
				<a href="{url}" c="1" {target}><img src="{avatar_big}"  alt="{title}" onerror="this.onerror=null;this.src='uc_server/data/avatar/noavatar.svg'"/></a>
			</div>
			<div class="dz_index_member_info">
				<h5>
					<a href="{url}" title="{title}" {target}>{title}</a>
					<div class="clear"></div>
				</h5>
				<div class="dz_index_member_info_btm">
					<p>ผลงาน {threads} กระทู้<span class="y">{credits} เครดิต</span></p>
				</div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
	</li>
	[/index]
	[loop]
	<li>
	
<em class="dz_member_num{currentorder}">{currentorder}</em>
		<div class="dz_index_member_intel">
			<div class="dz_index_member_avator">
				<a href="{url}" c="1" {target}><img src="{avatar_big}"  alt="{title}" onerror="this.onerror=null;this.src='uc_server/data/avatar/noavatar.svg'" /></a>
			</div>
			<div class="dz_index_member_info">
				<h5>
					<a href="{url}" title="{title}" {target}>{title}</a>
					<div class="clear"></div>
				</h5>
				<div class="dz_index_member_info_btm">
					<p>ผลงาน {threads} กระทู้<span class="y">{credits} เครดิต</span></p>
				</div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
	</li>
	[/loop]
</ul>
</div>]]></item>
						<item id="footer"><![CDATA[]]></item>
						<item id="header"><![CDATA[]]></item>
						<item id="indexplus">
						</item>
						<item id="index">
							<item id="1"><![CDATA[<li>
	
<em class="dz_member_num{currentorder}"></em>
		<div class="dz_index_member_intel">
			<div class="dz_index_member_avator">
				<a href="{url}" c="1" {target}><img src="{avatar_big}"  alt="{title}" onerror="this.onerror=null;this.src='uc_server/data/avatar/noavatar.svg'" /></a>
			</div>
			<div class="dz_index_member_info">
				<h5>
					<a href="{url}" title="{title}" {target}>{title}</a>
					<div class="clear"></div>
				</h5>
				<div class="dz_index_member_info_btm">
					<p>ผลงาน {threads} กระทู้<span class="y">{credits} เครดิต</span></p>
				</div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
	</li>]]></item>
							<item id="2"><![CDATA[<li>
	
<em class="dz_member_num{currentorder}"></em>
		<div class="dz_index_member_intel">
			<div class="dz_index_member_avator">
				<a href="{url}" c="1" {target}><img src="{avatar_big}"  alt="{title}" onerror="this.onerror=null;this.src='uc_server/data/avatar/noavatar.svg'" /></a>
			</div>
			<div class="dz_index_member_info">
				<h5>
					<a href="{url}" title="{title}" {target}>{title}</a>
					<div class="clear"></div>
				</h5>
				<div class="dz_index_member_info_btm">
					<p>ผลงาน {threads} กระทู้<span class="y">{credits} เครดิต</span></p>
				</div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
	</li>]]></item>
							<item id="3"><![CDATA[<li>
	
<em class="dz_member_num{currentorder}"></em>
		<div class="dz_index_member_intel">
			<div class="dz_index_member_avator">
				<a href="{url}" c="1" {target}><img src="{avatar_big}"  alt="{title}" onerror="this.onerror=null;this.src='uc_server/data/avatar/noavatar.svg'"/></a>
			</div>
			<div class="dz_index_member_info">
				<h5>
					<a href="{url}" title="{title}" {target}>{title}</a>
					<div class="clear"></div>
				</h5>
				<div class="dz_index_member_info_btm">
					<p>ผลงาน {threads} กระทู้<span class="y">{credits} เครดิต</span></p>
				</div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
	</li>]]></item>
						</item>
						<item id="orderplus">
						</item>
						<item id="order">
						</item>
						<item id="loopplus">
						</item>
						<item id="loop"><![CDATA[<li>
	
<em class="dz_member_num{currentorder}">{currentorder}</em>
		<div class="dz_index_member_intel">
			<div class="dz_index_member_avator">
				<a href="{url}" c="1" {target}><img src="{avatar_big}"  alt="{title}" onerror="this.onerror=null;this.src='uc_server/data/avatar/noavatar.svg'" /></a>
			</div>
			<div class="dz_index_member_info">
				<h5>
					<a href="{url}" title="{title}" {target}>{title}</a>
					<div class="clear"></div>
				</h5>
				<div class="dz_index_member_info_btm">
					<p>ผลงาน {threads} กระทู้<span class="y">{credits} เครดิต</span></p>
				</div>
			</div>
			<div class="clear"></div>
		</div>
		<div class="clear"></div>
	</li>]]></item>
					</item>
					<item id="hash"><![CDATA[2b4b0796]]></item>
				</item>
				<item id="picwidth"><![CDATA[0]]></item>
				<item id="picheight"><![CDATA[0]]></item>
				<item id="target"><![CDATA[blank]]></item>
				<item id="dateformat"><![CDATA[Y-m-d]]></item>
				<item id="dateuformat"><![CDATA[0]]></item>
				<item id="script"><![CDATA[member]]></item>
				<item id="param">
					<item id="uids"><![CDATA[]]></item>
					<item id="special"><![CDATA[]]></item>
					<item id="gender"><![CDATA[]]></item>
					<item id="xbirthcountry"><![CDATA[]]></item>
					<item id="xresidecountry"><![CDATA[]]></item>
					<item id="avatarstatus"><![CDATA[0]]></item>
					<item id="orderby"><![CDATA[credits]]></item>
					<item id="extcredit"><![CDATA[1]]></item>
					<item id="lastpost"><![CDATA[]]></item>
					<item id="startrow"><![CDATA[0]]></item>
					<item id="items"><![CDATA[7]]></item>
				</item>
				<item id="shownum"><![CDATA[7]]></item>
				<item id="cachetime"><![CDATA[3600]]></item>
				<item id="cachetimerange"><![CDATA[]]></item>
				<item id="punctualupdate"><![CDATA[0]]></item>
				<item id="hidedisplay"><![CDATA[0]]></item>
				<item id="dateline"><![CDATA[1752460067]]></item>
				<item id="notinherited"><![CDATA[0]]></item>
				<item id="isblank"><![CDATA[0]]></item>
			</item>
		</item>
		<item id="style">
			<item id="9">
				<item id="styleid"><![CDATA[9]]></item>
				<item id="blockclass"><![CDATA[forum_thread]]></item>
				<item id="name"><![CDATA[[บิลต์อิน] หัวข้อโพสต์]]></item>
				<item id="template">
					<item id="raw"><![CDATA[<div class="module cl xl xl1">
<ul>
[loop]
<li><a href="{url}" title="{title}"{target}>{title}</a></li>
[/loop]
</ul>
</div>]]></item>
					<item id="footer"><![CDATA[]]></item>
					<item id="header"><![CDATA[]]></item>
					<item id="indexplus">
					</item>
					<item id="index">
					</item>
					<item id="orderplus">
					</item>
					<item id="order">
					</item>
					<item id="loopplus">
					</item>
					<item id="loop"><![CDATA[<li><a href="{url}" title="{title}"{target}>{title}</a></li>]]></item>
				</item>
				<item id="hash"><![CDATA[079cd140]]></item>
				<item id="getpic"><![CDATA[0]]></item>
				<item id="getsummary"><![CDATA[0]]></item>
				<item id="makethumb"><![CDATA[0]]></item>
				<item id="settarget"><![CDATA[1]]></item>
				<item id="fields">
					<item id="0"><![CDATA[url]]></item>
					<item id="1"><![CDATA[title]]></item>
				</item>
				<item id="moreurl"><![CDATA[0]]></item>
			</item>
		</item>
	</item>
</root>