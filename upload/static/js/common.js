...
function showloading(display, waiting) {
	...
	var waiting = waiting ? waiting : $L('waiting');
...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	if (locationtime) {
		leftmsg = $L('second_redirect', [locationtime]);
		showDialogST = setTimeout(closefunc, locationtime * 1000);
		showconfirm = 0;
	}
	confirmtxt = confirmtxt ? confirmtxt : confirmtxtdefault;
	canceltxt = canceltxt ? canceltxt : $L('cancel');
...
function showError(msg) {
	...
	msg = msg.replace(p, '');
	if (msg !== '') {
		showDialog(msg, 'alert', $L('error_notice'), null, true, null, '', '', '', 3);
	}
}
...
function simulateSelect(selectId, widthvalue) {
	...
	selectObj.outerHTML += '<a href="javascript:;" id="' + selectId + '_ctrl" style="min-width:' + selectwidth + '">' + defaultopt + '</a>';

	menuObj.id = selectId + '_ctrl_menu';
	menuObj.className = 'sltm';
	menuObj.style.display = 'none';
	menuObj.style.minWidth = selectwidth;
	menuObj.appendChild(ul);
	$('append_parent').appendChild(menuObj);

	$(selectId + '_ctrl').onclick = function (e) {
		$(selectId + '_ctrl_menu').style.minWidth = selectwidth;
		showMenu({
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (!!("placeholder" in document.createElement("input"))) {
		if (searchtxt.value == $L('input_search_content')) {
			searchtxt.value = '';
		}
		searchtxt.placeholder = $L('input_search_content');
	} else {
		...
		searchtxt.onblur = function () {
			if (searchtxt.value == '') {
				searchtxt.value = $L('input_search_content');
				searchtxt.className = tclass + ' xg1';
			}
		};
	}
...
function searchFocus(obj) {
	if (obj.value == $L('input_search_content')) {
		obj.value = '';
	}
...
function sendemailseccode(svctype, email) {
	showWindow('sendemailseccode', 'misc.php?mod=secemailseccode&action=send&svctype=' + svctype + '&email=' + email);
}
...
function addFavorite(url, title) {
	try {
		...
		try {
			...
			showDialog($L('addto_favorite'), 'notice');
		}
	}
}
...
function toggleBlind(dom) {
	if (dom) {
		if (loadUserdata('is_blindman')) {
			...
			dom.title = $L('open_blind');
		} else {
			saveUserdata('is_blindman', '1');
			dom.title = $L('close_blind');
		}
	}
}
