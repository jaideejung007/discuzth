...
var ATTACHORIMAGE = '0';
var STATUSMSG = $L('upload_statusmsg');
STATUSMSG[7] += '(' + imgexts + ')';
...
function validate(theform) {
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		showError($L('message_length_error', [mb_strlen(message), postminchars, postmaxchars]));
		return false;
	}
	if(UPLOADSTATUS == 0) {
		if(!confirm($L('have_no_upload'))) {
			return false;
		}
	} else if(UPLOADSTATUS == 1) {
		showDialog($L('have_uploading'), 'notice');
		AUTOPOST = 1;
		return false;
	}
...
function postsubmit(theform) {
	...
	var submitbtn = theform.replysubmit ? theform.replysubmit : (theform.editsubmit ? theform.editsubmit : theform.topicsubmit);
    if(!$(theform.id+'return')) {
        var div = document.createElement('div');
        div.id = theform.id+'return';
        div.innerHTML = '<input type="hidden" name="handlekey" value="post" />';
        $(theform.id).appendChild(div);
    }
    ajaxpost(theform.id, theform.id+'return', theform.id+'return', 'onerror');
}

function succeedhandle_post(locationhref) {
...
function uploadNextAttach() {
	...
	var sizelimit = '';
	if(arr[4] == 'ban') {
		sizelimit = '(' + $L('attach_type_ban') + ')';
	} else if(arr[4] == 'perday') {
		sizelimit = '(' + $L('attach_size_ban', [arr[5]]) + ')';
	} else if(arr[4] > 0) {
		sizelimit = '(' + $L('attach_size_ban', [arr[4]]) + ')';
	}
...
function uploadAttach(curId, statusid, prefix, sizelimit) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
				showDialog($L('attach_upload_error', [UPLOADCOMPLETE, UPLOADFAILED, FAILEDATTACHS]));
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID[prefix ? 1 : 0] - 1; i++) {
			...
			if(AUTOPOST) {
				...
				showDialog($L('attach_upload_success'), 'right', null, null, 0, null, null, null, null, 3);
			}
	...
	$(prefix + 'cpdel_' + nextId).innerHTML = '<div class="loadicon" title="' + $L('attach_uploading') + '"></div>';
	UPLOADSTATUS = 1;
...
function insertAttach(prefix, id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		...
		showError($L('attach_ext_error'));
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
		showError($L('attach_select_img', [imgexts]));
		return;
	}

	$(prefix + 'cpdel_' + id).innerHTML = '<a href="javascript:;" class="d" onclick="reAddAttach(\'' + prefix + '\', ' + id + ')">' + $L('delete') + '</a>';
...
function updateAttach(aid) {
	...
	if(!objupdate.innerHTML) {
		...
		objupdate.innerHTML = '<input type="file" name="attachupdate[paid' + aid + ']"><a href="javascript:;" onclick="updateAttach(' + aid + ')">' + $L('cancel') + '</a>';
	} else {
		obj.style.display = '';
		objupdate.innerHTML = '';
	}
...
function updateattachnum(type) {
	...
	if(num) {
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = $L('include') + ' ' + num + (type == 'image' ? ' ' + $L('num_images') : ' ' + $L('num_attachs'));
		}
		...
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = type == 'image' ? $L('images') : $L('attachs');
		}
		if($(editorid + '_' + type + 'n')) {
...
function updateDownImageList(msg) {
	...
	if(msg == '') {
		...
		switchImagebutton('imgattachlist');$('imgattach_notice').style.display = '';
		showDialog($L('remote_downloaded'), 'right', null, null, 0, null, null, null, null, 3);
	}
}
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		if(arr[7] == 'ban') {
			...
			sizelimit = '(' + $L('attach_size_ban', [arr[8]]) + ')';
		} else if(arr[7] > 0) {
			sizelimit = '(' + $L('attach_size_ban', [arr[7]]) + ')';
		}
...
function addpolloption() {
	if(curoptions < maxoptions) {
		...
		$('polloption_new').innerHTML = $L('maxvoters', [maxoptions]);
	}
}
...
function attachoption(type, op) {
	if(!op) {
		...
		showDialog('<div id="unusedwin" class="c altw" style="overflow:auto;height:100px;">' + $('unusedlist_' + type).innerHTML + '</div>' +
			'<p class="o pns"><span class="z xg1"><label for="unusedwinchkall"><input id="unusedwinchkall" type="checkbox" onclick="attachoption(\'' + type + '\', 3)" checked="checked" />' + $L('select_all') + '</label></span>' +
			'<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc"><strong>' + $L('use') + '</strong></button></p>', 'info', $L('unused') + (type == 'attach' ? $L('attachs') : $L('images')));
	} else if(op == 3) {
...
function extraCheck(op) {
	if(!op && $('extra_replycredit_chk')) {
        ...
        $('extra_attr_chk').className = $('attrs').value !== '' ? 'a' : '';
    }
}
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">' + $L('reply_credit_over', [real_reply_credit]) + '</b>';
	} else {
		if(have_replycredit > 0 && real_reply_credit < 0) {
			$('replycredit').innerHTML = "<font class='xi1'>" + $L('reply_credit_num', Math.abs(real_reply_credit)) + "</font>";
		} else {
