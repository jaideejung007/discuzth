...
function loadcalendar() {
	...
	s += '<th><a href="###" onclick="refreshcalendar(yy, mm-1)" title="เดือนก่อนหน้า">《</a></th>';
	s += '<th colspan="5" style="text-align: center"><a href="###" onclick="showdiv(\'year\');_cancelBubble(event)" title="เลือกปี" id="year"></a>&nbsp; - &nbsp;<a id="month" title="เลือกเดือน" href="###" onclick="showdiv(\'month\');_cancelBubble(event)"></a></th>';
	s += '<th><A href="###" onclick="refreshcalendar(yy, mm+1)" title="เดือนต่อไป">》</A></th>';
	s += '</tr>';
	...
	s += '<tr id="hourminute"><td colspan="7" align="center"><input type="text" size="2" value="" id="hour" onKeyUp=\'this.value=this.value > 23 ? 23 : zerofill(this.value);controlid.value=controlid.value.replace(/\\d+(\:\\d+)/ig, this.value+"$1")\'> ชั่วโมง <input type="text" size="2" value="" id="minute" onKeyUp=\'this.value=this.value > 59 ? 59 : zerofill(this.value);controlid.value=controlid.value.replace(/(\\d+\:)\\d+/ig, "$1"+this.value)\'> นาที</td></tr>';
	s += '</tbody>';
	s += '</table></div></div>';
